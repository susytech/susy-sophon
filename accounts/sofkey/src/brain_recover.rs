...
fn generate_substitutions(word: &str) -> Vec<&'static str> {
	let mut words = susy_wordlist::WORDS.iter().cloned()
...
impl PhrasesIterator {
	pub fn from_known_phrase(known_phrase: &str, expected_words: usize) -> Self {
		let known_words = susy_wordlist::WORDS.iter().cloned().collect::<HashSet<_>>();
		...
		if words.len() < expected_words {
			...
			for _ in 0..to_add {
				words.push(susy_wordlist::WORDS.iter().cloned().collect());
			}
		}
