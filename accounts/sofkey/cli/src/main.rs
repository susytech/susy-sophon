...
use docopt::Docopt;
use sofkey::{KeyPair, Random, Brain, BrainPrefix, Prefix, Error as SofkeyError, Generator, sign, verify_public, verify_address, brain_recover};
use rustc_hex::{FromHex, FromHexError};

const USAGE: &'static str = r#"
Susy Sophon keys generator.
  Copyleft 2015-2019 Superstring.Community

Usage:
    sofkey info <secret-or-phrase> [options]
    sofkey generate random [options]
    sofkey generate prefix <prefix> [options]
    sofkey sign <secret> <message>
    sofkey verify public <public> <signature> <message>
    sofkey verify address <address> <signature> <message>
    sofkey recover <address> <known-phrase>
    sofkey [-h | --help]
...
Options:
    ...
    -b, --brain        Use susy brain wallet algorithm. Not recommended.

Commands:
    info               Display public key and address of the secret.
    generate random    Generates new random Sophon key.
...
fn execute<S, I>(command: I) -> Result<String, Error> where I: IntoIterator<Item=S>, S: AsRef<str> {
	...
	return if args.cmd_info {
		...
		let result = if args.flag_brain {
			...
			let secret = args.arg_secret_or_phrase.parse().map_err(|_| SofkeyError::InvalidSecret)?;
		...
		let secret = args.arg_secret.parse().map_err(|_| SofkeyError::InvalidSecret)?;
		let message = args.arg_message.parse().map_err(|_| SofkeyError::InvalidMessage)?;
		let signature = sign(&secret, &message)?;
		Ok(format!("{}", signature))
	} else if args.cmd_verify {
		let signature = args.arg_signature.parse().map_err(|_| SofkeyError::InvalidSignature)?;
		let message = args.arg_message.parse().map_err(|_| SofkeyError::InvalidMessage)?;
		let ok = if args.cmd_public {
			let public = args.arg_public.parse().map_err(|_| SofkeyError::InvalidPublic)?;
			verify_public(&public, &signature, &message)?
		} else if args.cmd_address {
			let address = args.arg_address.parse().map_err(|_| SofkeyError::InvalidAddress)?;
		...
		let (phrase, keypair) = in_threads(move || {
			...
			move || {
				...
				Err(SofkeyError::Custom("Couldn't find any results.".into()))
			}
		})?;
...
fn validate_phrase(phrase: &str) -> String {
	match Brain::validate_phrase(phrase, BRAIN_WORDS) {
		...
		Err(err) => format!("The recover phrase was not generated by Susy: {}", err)
	}
}

fn in_threads<F, X, O>(prepare: F) -> Result<O, SofkeyError> where
	O: Send + 'static,
	X: Send + 'static,
	F: Fn() -> X,
	X: FnMut() -> Result<Option<O>, SofkeyError>,
{
	...
	Err(SofkeyError::Custom("No results found.".into()))
}
...
mod tests {
	...
	#[test]
	fn info() {
		let command = vec!["sofkey", "info", "17d08f5fe8c77af811caa0c9a187e668ce3b74a99acc3f6d976f075fa8e0be55"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
...
address: 26d1ec50b4e62c1d1a40d16e7cacc6a6580757d5".to_owned();
	...
	#[test]
	fn brain() {
		let command = vec!["sofkey", "info", "--brain", "this is sparta"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();

		let expected =
"The recover phrase was not generated by Susy: The word 'this' does not come from the dictionary.
...
address: 006e27b6a72e1f34c626762f3c4761547aff1421".to_owned();
	...
	#[test]
	fn secret() {
		let command = vec!["sofkey", "info", "--brain", "this is sparta", "--secret"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
	...
	#[test]
	fn public() {
		let command = vec!["sofkey", "info", "--brain", "this is sparta", "--public"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
	...
	#[test]
	fn address() {
		let command = vec!["sofkey", "info", "-b", "this is sparta", "--address"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
	...
	#[test]
	fn sign() {
		let command = vec!["sofkey", "sign", "17d08f5fe8c77af811caa0c9a187e668ce3b74a99acc3f6d976f075fa8e0be55", "bd50b7370c3f96733b31744c6c45079e7ae6c8d299613246d28ebcef507ec987"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
	...
	fn verify_valid_public() {
		let command = vec!["sofkey", "verify", "public", "689268c0ff57a20cd299fa60d3fb374862aff565b20b5f1767906a99e6e09f3ff04ca2b2a5cd22f62941db103c0356df1a8ed20ce322cab2483db67685afd124", "c1878cf60417151c766a712653d26ef350c8c75393458b7a9be715f053215af63dfd3b02c2ae65a8677917a8efa3172acb71cb90196e42106953ea0363c5aaf200", "bd50b7370c3f96733b31744c6c45079e7ae6c8d299613246d28ebcef507ec987"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
	...
	fn verify_valid_address() {
		let command = vec!["sofkey", "verify", "address", "26d1ec50b4e62c1d1a40d16e7cacc6a6580757d5", "c1878cf60417151c766a712653d26ef350c8c75393458b7a9be715f053215af63dfd3b02c2ae65a8677917a8efa3172acb71cb90196e42106953ea0363c5aaf200", "bd50b7370c3f96733b31744c6c45079e7ae6c8d299613246d28ebcef507ec987"]
			.into_iter()
			.map(Into::into)
			.collect::<Vec<String>>();
