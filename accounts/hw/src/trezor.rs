...
use trezor_sys::messages::{SophonAddress, PinMatrixAck, MessageType, SophonTxRequest, SophonSignTx, SophonGetAddress, SophonTxAck, ButtonAck};
...
const SOF_DERIVATION_PATH: [u32; 5] = [0x8000_002C, 0x8000_003C, 0x8000_0000, 0, 0]; // m/44'/60'/0'/0/0
...
impl Manager {
	...
	pub fn pin_matrix_ack(&self, device_path: &str, pin: &str) -> Result<bool, Error> {
		let unlocked = {
			...
			match resp_type {
				...
				MessageType::MessageType_SophonAddress => Ok(true),
	...
	fn signing_loop(&self, handle: &hidapi::HidDevice, chain_id: &Option<u64>, data: &[u8]) -> Result<Signature, Error> {
		...
		match resp_type {
			...
			MessageType::MessageType_SophonTxRequest => {
				let resp: SophonTxRequest = protobuf::core::parse_from_bytes(&bytes)?;
				if resp.has_data_length() {
					let mut msg = SophonTxAck::new();
					let len = resp.get_data_length() as usize;
					msg.set_data_chunk(data[..len].to_vec());
					self.send_device_message(handle, MessageType::MessageType_SophonTxAck, &msg)?;
					...
					if let Some(c_id) = *chain_id {
						...
						// part of the signature that is already adjusted for SIP-155,
...
impl<'a> Wallet<'a> for Manager {
	...
	fn sign_transaction(&self, address: &Address, t_info: Self::Transaction) ->
		...
		let msg_type = MessageType::MessageType_SophonSignTx;
		let mut message = SophonSignTx::new();
		match *self.key_path.read() {
			KeyPath::Sophon => message.set_address_n(SOF_DERIVATION_PATH.to_vec()),
			KeyPath::SophonClassic => message.set_address_n(ETC_DERIVATION_PATH.to_vec()),
		}
	...
	fn get_address(&self, device: &hidapi::HidDevice) -> Result<Option<Address>, Error> {
		let typ = MessageType::MessageType_SophonGetAddress;
		let mut message = SophonGetAddress::new();
		match *self.key_path.read() {
			KeyPath::Sophon => message.set_address_n(SOF_DERIVATION_PATH.to_vec()),
			KeyPath::SophonClassic => message.set_address_n(ETC_DERIVATION_PATH.to_vec()),
		}
		...
		match resp_type {
			MessageType::MessageType_SophonAddress => {
				let response: SophonAddress = protobuf::core::parse_from_bytes(&bytes)?;
