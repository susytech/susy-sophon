...
//! See <https://github.com/LedgerHQ/blue-app-sof/blob/master/doc/sofapp.asc> for protocol details.
...
const SOF_DERIVATION_PATH_BE: [u8; 17] = [4, 0x80, 0, 0, 44, 0x80, 0, 0, 60, 0x80, 0, 0, 0, 0, 0, 0, 0]; // 44'/60'/0'/0
...
mod commands {
	...
	pub const GET_SOF_PUBLIC_ADDRESS: u8 = 0x02;
	pub const SIGN_SOF_TRANSACTION: u8 = 0x04;
	pub const SIGN_SOF_PERSONAL_MESSAGE: u8 = 0x08;
}

/// Hardware wallet error.
#[derive(Debug)]
pub enum Error {
	/// Sophon wallet protocol error.
	Protocol(&'static str),
	/// Hidapi error.
	Usb(hidapi::HidError),
	/// Libusb error
...
impl Manager {
	...
	pub fn new(usb: Arc<Mutex<hidapi::HidApi>>) -> Arc<Self> {
		Arc::new(Self {
			...
			key_path: RwLock::new(KeyPath::Sophon),
		})
	}
	...
	fn read(handle: &hidapi::HidDevice) -> Result<Vec<u8>, Error> {
		...
		match status {
			...
			0x6d00 => Err(Error::Protocol("Not implemented. Make sure the Ledger Sophon Wallet app is running.")),
	...
	fn get_derivation_path(&self) -> &[u8] {
		match *self.key_path.read() {
			KeyPath::Sophon => &SOF_DERIVATION_PATH_BE,
			KeyPath::SophonClassic => &ETC_DERIVATION_PATH_BE,
		}
	}
	...
	fn signer_helper(&self, address: &Address, data: &[u8], command: u8) -> Result<Signature, Error> {
		...
		if command == commands::SIGN_SOF_PERSONAL_MESSAGE {
	...
	pub fn sign_message(&self, address: &Address, msg: &[u8]) -> Result<Signature, Error> {
		self.signer_helper(address, msg, commands::SIGN_SOF_PERSONAL_MESSAGE)
	}
}
...
impl<'a> Wallet<'a> for Manager {
	...
	fn sign_transaction(&self, address: &Address, transaction: Self::Transaction) -> Result<Signature, Self::Error> {
		self.signer_helper(address, transaction, commands::SIGN_SOF_TRANSACTION)
	}
	...
	fn get_address(&self, device: &hidapi::HidDevice) -> Result<Option<Address>, Self::Error> {
		...
		let key_and_address = Self::send_apdu(device, commands::GET_SOF_PUBLIC_ADDRESS, 0, 0, derivation_path)?;
...
mod tests {
	...
	/// This test can't be run without an actual ledger device connected with the `Ledger Wallet Sophon application` running
	#[test]
	#[ignore]
	fn sign_personal_message() {
		...
		// Update device list
		ledger.update_devices(DeviceDirection::Arrived).expect("No Ledger found, make sure you have a unlocked Ledger connected with the Ledger Wallet Sophon running");

		// Fetch the sophon address of a connected ledger device
