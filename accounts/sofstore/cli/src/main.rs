...
use docopt::Docopt;
use sofstore::accounts_dir::{KeyDirectory, RootDiskDirectory};
use sofstore::sofkey::{Address, Password};
use sofstore::{SofStore, SimpleSecretStore, SecretStore, import_accounts, PresaleWallet, SecretVaultRef, StoreAccountRef};
...
Usage:
    ...
    sofstore import [<password>] [--src DIR] [--dir DIR]
...
enum Error {
	Sofstore(sofstore::Error),
	Docopt(docopt::Error),
}

impl From<sofstore::Error> for Error {
	fn from(err: sofstore::Error) -> Self {
		Error::Sofstore(err)
	}
}
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			Error::Sofstore(ref err) => fmt::Display::fmt(err, f),
...
fn key_dir(location: &str, password: Option<Password>) -> Result<Box<KeyDirectory>, Error> {
	let dir: RootDiskDirectory = match location {
		"graviton" => RootDiskDirectory::create(dir::graviton(false))?,
		"graviton-test" => RootDiskDirectory::create(dir::graviton(true))?,
		path if path.starts_with("susy") => {
			let chain = path.split('-').nth(1).unwrap_or("sophon");
			let path = dir::susy(chain);
...
fn open_args_vault(store: &SofStore, args: &Args) -> Result<SecretVaultRef, Error> {
...
fn open_args_vault_account(store: &SofStore, address: Address, args: &Args) -> Result<StoreAccountRef, Error> {
...
fn load_password(path: &str) -> Result<Password, Error> {
	let mut file = fs::File::open(path).map_err(|e| sofstore::Error::Custom(format!("Error opening password file '{}': {}", path, e)))?;
	let mut password = String::new();
	file.read_to_string(&mut password).map_err(|e| sofstore::Error::Custom(format!("Error reading password file '{}': {}", path, e)))?;
	// drop EOF
	let _ = password.pop();
	Ok(password.into())
}
...
fn execute<S, I>(command: I) -> Result<String, Error> where I: IntoIterator<Item=S>, S: AsRef<str> {
	...
	let store = SofStore::open(key_dir(&args.flag_dir, None)?)?;

	return if args.cmd_insert {
		let secret = args.arg_secret.parse().map_err(|_| sofstore::Error::InvalidSecret)?;
		...
		let address = args.arg_address.parse().map_err(|_| sofstore::Error::InvalidAccount)?;
		...
		let address = args.arg_address.parse().map_err(|_| sofstore::Error::InvalidAccount)?;
		let message = args.arg_message.parse().map_err(|_| sofstore::Error::InvalidMessage)?;
		let password = load_password(&args.arg_password)?;
		let account_ref = open_args_vault_account(&store, address, &args)?;
