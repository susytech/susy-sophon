...
use sofkey::{self, KeyPair, sign, Address, Password, Signature, Message, Public, Secret};
...
impl SafeAccount {
	...
	pub fn decrypt(&self, password: &Password, shared_mac: &[u8], message: &[u8]) -> Result<Vec<u8>, Error> {
		let secret = self.crypto.secret(password)?;
		sofkey::crypto::ecies::decrypt(&secret, shared_mac, message).map_err(From::from)
	}
...
#[cfg(test)]
mod tests {
	use sofkey::{Generator, Random, verify_public, Message};
