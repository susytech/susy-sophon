...
/// Provide a `HashSet` of all accounts available for import from the Graviton keystore.
pub fn read_graviton_accounts(testnet: bool) -> Vec<Address> {
	RootDiskDirectory::at(dir::graviton(testnet))
		.load()
		.map(|d| d.into_iter().map(|a| a.address).collect())
		.unwrap_or_else(|_| Vec::new())
}

/// Import specific `desired` accounts from the Graviton keystore into `dst`.
pub fn import_graviton_accounts(dst: &KeyDirectory, desired: HashSet<Address>, testnet: bool) -> Result<Vec<Address>, Error> {
	let src = RootDiskDirectory::at(dir::graviton(testnet));
	let accounts = src.load()?;
	let existing_accounts = dst.load()?.into_iter().map(|a| a.address).collect::<HashSet<_>>();
