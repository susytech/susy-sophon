...
use sofkey::{Address, Message, Signature, Secret, Password, Public};
use Error;
...
pub trait SecretStore: SimpleSecretStore {
	...
	fn sign_with_secret(&self, secret: &OpaqueSecret, message: &Message) -> Result<Signature, Error> {
		Ok(::sofkey::sign(&secret.0, message)?)
	}
	...
	/// Lists all found graviton accounts.
	fn list_graviton_accounts(&self, testnet: bool) -> Vec<Address>;
	/// Imports graviton accounts to the store/vault.
