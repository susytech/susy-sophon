...
mod test {
	...
	fn make_vault_dir_path_succeeds() {
		...
		assert_eq!(&make_vault_dir_path("/home/user/susy", "vault", true).unwrap(), &Path::new("/home/user/susy/vault"));
		assert_eq!(&make_vault_dir_path("/home/user/susy", "*bad-name*", false).unwrap(), &Path::new("/home/user/susy/*bad-name*"));
	}

	#[test]
	fn make_vault_dir_path_fails() {
		assert!(make_vault_dir_path("/home/user/susy", "*bad-name*", true).is_err());
	}
