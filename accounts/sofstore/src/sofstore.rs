...
use random::Random;
use sofkey::{self, Signature, Password, Address, Message, Secret, Public, KeyPair, ExtendedKeyPair};
...
impl SofStore {
	...
	pub fn open_with_iterations(directory: Box<KeyDirectory>, iterations: NonZeroU32) -> Result<Self, Error> {
		Ok(SofStore {
			store: SofMultiStore::open_with_iterations(directory, iterations)?,
		})
	}
	...
	///
	/// By default refreshing is disabled, so only accounts created using this instance of `SofStore` are taken into account.
...
impl SimpleSecretStore for SofStore {
...
impl SecretStore for SofStore {
	...
	fn list_graviton_accounts(&self, testnet: bool) -> Vec<Address> {
		import::read_graviton_accounts(testnet)
	}

	fn import_graviton_accounts(&self, vault: SecretVaultRef, desired: Vec<Address>, testnet: bool) -> Result<Vec<StoreAccountRef>, Error> {
		let imported_addresses = match vault {
			SecretVaultRef::Root => import::import_graviton_accounts(&*self.store.dir, desired.into_iter().collect(), testnet),
			SecretVaultRef::Vault(vault_name) => {
				if let Some(vault) = self.store.vaults.lock().get(&vault_name) {
					import::import_graviton_accounts(vault.as_key_directory(), desired.into_iter().collect(), testnet)
				} else {
...
/// Similar to `SofStore` but may store many accounts (with different passwords) for the same `Address`
pub struct SofMultiStore {
			...
			Derivation::Hierarchical(path) => {
				for path_item in path {
					extended = extended.derive(
						if path_item.soft { sofkey::Derivation::Soft(path_item.index) }
						else { sofkey::Derivation::Hard(path_item.index) }
					)?;
				}
			},
			Derivation::SoftHash(h256) => { extended = extended.derive(sofkey::Derivation::Soft(h256))?; }
			Derivation::HardHash(h256) => { extended = extended.derive(sofkey::Derivation::Hard(h256))?; }
		}
		Ok(extended)
	}
}

impl SimpleSecretStore for SofMultiStore {
	fn insert_account(&self, vault: SecretVaultRef, secret: Secret, password: &Password) -> Result<StoreAccountRef, Error> {
	    ...
	    -> Result<Address, Error>
	{
		...
		for account in accounts {
			let extended = self.generate(account.crypto.secret(password)?, derivation)?;
			return Ok(sofkey::public_to_address(extended.public().public()));
		}
		Err(Error::InvalidPassword)
	}
		...
		-> Result<Signature, Error>
	{
		...
		for account in accounts {
			...
			return Ok(sofkey::sign(&secret, message)?)
		}
		Err(Error::InvalidPassword)
	}
...
mod tests {
	...
	fn store() -> SofStore {
		SofStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory always load successfuly; qed")
	}

	fn multi_store() -> SofMultiStore {
		SofMultiStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory always load successfuly; qed")
	}
	...
	fn should_create_and_open_vaults() {
		...
		let store = SofStore::open(dir.key_dir.take().unwrap()).unwrap();
