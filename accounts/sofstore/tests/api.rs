...
use sofstore::{SofStore, SimpleSecretStore, SecretVaultRef, StoreAccountRef};
use sofstore::sofkey::{Random, Generator, Secret, KeyPair, verify_address};
...
fn secret_store_create() {
	...
	let _ = SofStore::open(Box::new(dir)).unwrap();
}
...
fn secret_store_create_account() {
	let dir = TransientDir::create().unwrap();
	let store = SofStore::open(Box::new(dir)).unwrap();
...
fn test_path() -> &'static str {
	match ::std::fs::metadata("sofstore") {
		Ok(_) => "sofstore/tests/res/graviton_keystore",
 		Err(_) => "tests/res/graviton_keystore",
	}
}

fn pat_path() -> &'static str {
	match ::std::fs::metadata("sofstore") {
		Ok(_) => "sofstore/tests/res/pat",
		Err(_) => "tests/res/pat",
	}
}
...
fn ciphertext_path() -> &'static str {
	match ::std::fs::metadata("sofstore") {
		Ok(_) => "sofstore/tests/res/ciphertext",
		Err(_) => "tests/res/ciphertext",
	}
}

#[test]
fn secret_store_laod_graviton_files() {
