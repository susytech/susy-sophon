...
use sofkey::{Address, Message, Public, Secret, Password, Random, Generator};
use sofstore::accounts_dir::MemoryDirectory;
use sofstore::{
	SimpleSecretStore, SecretStore, SofStore, SofMultiStore,
	random_string, SecretVaultRef, StoreAccountRef, OpaqueSecret,
};
use log::{warn, debug};
use parking_lot::RwLock;

pub use sofkey::Signature;
pub use sofstore::{Derivation, IndexDerivation, KeyFile, Error};
...
pub struct AccountProvider {
	...
	transient_sstore: SofMultiStore,
...
fn transient_sstore() -> SofMultiStore {
	SofMultiStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory load always succeeds; qed")
}
...
impl AccountProvider {
	...
	pub fn new(sstore: Box<SecretStore>, settings: AccountProviderSettings) -> Self {
		...
		if settings.enable_hardware_wallets {
			match HardwareWalletManager::new() {
				Ok(manager) => {
					manager.set_key_path(if settings.hardware_wallet_classic_key { KeyPath::SophonClassic } else { KeyPath::Sophon });
	...
	pub fn transient_provider() -> Self {
		AccountProvider {
			...
			sstore: Box::new(SofStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory load always succeeds; qed")),
			transient_sstore: transient_sstore(),
			hardware_store: None,
	...
	pub fn list_graviton_accounts(&self, testnet: bool) -> Vec<Address> {
		self.sstore.list_graviton_accounts(testnet).into_iter().map(|a| Address::from(a).into()).collect()
	}

	/// Returns the underlying `SecretStore` reference if one exists.
	pub fn import_graviton_accounts(&self, desired: Vec<Address>, testnet: bool) -> Result<Vec<Address>, Error> {
		self.sstore.import_graviton_accounts(SecretVaultRef::Root, desired, testnet)
	...
	pub fn sign_transaction_with_hardware(&self, address: &Address, transaction: &Transaction, chain_id: Option<u64>, srlp_encoded_transaction: &[u8]) -> Result<Signature, SignError> {
		...
		match self.hardware_store.as_ref().map(|s| s.sign_transaction(&address, &t_info, srlp_encoded_transaction)) {
			None | Some(Err(HardwareError::KeyNotFound)) => Err(SignError::NotFound),
			Some(Err(e)) => Err(From::from(e)),
			Some(Ok(s)) => Ok(s),
		}
...
mod tests {
	...
	use sofkey::{Generator, Random, Address};
	use sofstore::{StoreAccountRef, Derivation};
