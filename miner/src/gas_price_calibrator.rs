	...
	pub(crate) fn recalibrate<F: FnOnce(U256) + Sync + Send + 'static>(&mut self, set_price: F) {
		...
		if Instant::now() >= self.next_calibration {
			...
			self.price_info.get(move |price: PriceInfo| {
				...
				let usd_per_sof = price.sofusd;
				let wei_per_usd: f32 = 1.0e18 / usd_per_sof;
				let gas_per_tx: f32 = 21000.0;
				let wei_per_gas: f32 = wei_per_usd * usd_per_tx / gas_per_tx;
				//info!(target: "miner", "Updated conversion rate to Ξ1 = {} ({} wei/gas)", Colour::White.bold().paint(format!("US${:.2}", usd_per_sof)), Colour::Yellow.bold().paint(format!("{}", wei_per_gas)));
