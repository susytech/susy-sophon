...
impl TransactionEntry {
	fn into_pending(self) -> Option<PendingTransaction> {
		let tx: UnverifiedTransaction = match Rlp::new(&self.srlp_bytes).as_val() {
			Err(e) => {
...
impl From<PendingTransaction> for TransactionEntry {
	fn from(pending: PendingTransaction) -> Self {
		TransactionEntry {
			srlp_bytes: ::srlp::encode(&pending.transaction),
...
mod tests {
	...
	#[test]
	fn twice_empty() {
		let db = Arc::new(::susykv_memorydb::create(0));
