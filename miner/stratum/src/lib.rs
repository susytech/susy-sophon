...
//! Stratum protocol implementation for susy sophon/bitcoin clients
...
use susy_jsonrpc_core::{MetaIoHandler, Params, to_value, Value, Metadata, Compatibility, IoDelegate};
use std::sync::Arc;
...
type RpcResult = Result<susy_jsonrpc_core::Value, susy_jsonrpc_core::Error>;
...
impl StratumImpl {
	...
	fn subscribe(&self, _params: Params, meta: SocketMetadata) -> RpcResult {
		...
		Ok(match self.dispatcher.initial() {
			Some(initial) => match susy_jsonrpc_core::Value::from_str(&initial) {
				Ok(val) => Ok(val),
				Err(e) => {
...
pub struct SocketMetadata {
	addr: SocketAddr,
	// with the new version of susy-jsonrpc-core, SocketMetadata
