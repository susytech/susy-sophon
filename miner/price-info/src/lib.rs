...
//! A simple client to get the current SOF price using an external API.
...
/// Current SOF price information.
#[derive(Debug)]
pub struct PriceInfo {
	/// Current SOF price in USD.
	pub sofusd: f32,
}
...
/// A client to get the current SOF price using an external API.
...
impl<F: Fetch> Client<F> {
	...
	pub fn new(fetch: F, pool: Executor) -> Client<F> {
		let api_endpoint = "https://api.sophyscan.io/api?module=stats&action=sofprice".to_owned();
		Client { pool, api_endpoint, fetch }
	}

	/// Gets the current SOF price and calls `set_price` with the result.
	pub fn get<G: FnOnce(PriceInfo) + Sync + Send + 'static>(&self, set_price: G) {
		let future = self.fetch.get(&self.api_endpoint, fetch::Abort::default())
			...
			.and_then(move |body| {
				...
				let sofusd = value
					.as_ref()
					.and_then(|value| value.pointer("/result/sofusd"))
					.and_then(|obj| obj.as_str())
					.and_then(|s| s.parse().ok());

				match sofusd {
					Some(sofusd) => {
						set_price(PriceInfo { sofusd });
						Ok(())
					},
			...
			.map_err(|err| {
				//warn!("Failed to auto-update latest SOF price: {:?}", err);
			});
...
mod test {
	...
	fn should_get_price_info() {
		...
		let response = r#"{
			...
			"result": {
				"sofbtc": "0.0891",
				"sofbtc_timestamp": "1499894236",
				"sofusd": "209.55",
				"sofusd_timestamp": "1499894229"
			}
		}"#;
