...
use sofcore::client::{BlockId, BlockChainClient, ChainNotify, NewBlocks};
use sophon_types::H256;
use hash_fetch::{self as fetch, HashFetch};
use susy_path::restrict_permissions_owner;
use service::Service;
...
impl Updater {
	...
	fn update_file_name(v: &VersionInfo) -> String {
		format!("susy-{}.{}.{}-{:x}", v.version.major, v.version.minor, v.version.patch, v.hash)
	}
}
...
impl<O: OperationsClient, F: HashFetch, T: TimeProvider, R: GenRange> Updater<O, F, T, R> {
	...
	/// Returns release track of the susy node.
	...
	fn updater_step(&self, mut state: MutexGuard<UpdaterState>) {
		...
		if let Some(latest) = state.latest.clone() {
			let fetch = |latest, binary| {
				info!(target: "updater", "Attempting to get susy binary {}", binary);
...
pub mod tests {
	...
	use semver::Version;
	use tempdir::TempDir;
	use sofcore::client::{TestBlockChainClient, EachBlockWith};
	...
	fn should_update_on_new_release() {
		...
		let update_file = tempdir.path().join("susy");
