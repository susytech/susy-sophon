...
use sofcore::client::{self, SvmTestClient, SvmTestError, TransactResult};
use sofcore::{state, state_db, trace, spec, pod_state, TrieSpec};
use sofjson;
...
pub fn run_transaction<T: Informant>(
	...
	let spec = match SvmTestClient::spec_from_json(spec) {
		Some(spec) => {
		...
		match result {
			TransactResult::Ok { state_root, gas_left, output, vm_trace, end_state, .. } => {
				if state_root != post_root {
					(Err(SvmTestError::PostCondition(format!(
...
pub fn run<'a, F, X>(
	...
	F: FnOnce(SvmTestClient) -> (Result<Vec<u8>, SvmTestError>, H256, Option<pod_state::PodState>, Option<U256>, Option<X>),
{
	let do_dump = trie_spec == TrieSpec::Fat;

	let mut test_client = SvmTestClient::from_pod_state_with_trie(spec, pre_state.clone(), trie_spec)
...
pub mod tests {
		...
		F: FnOnce(Option<I::Output>, &str),
	{
		...
		let spec = ::sofcore::sophon::new_foundation(&tempdir.path());
	...
	fn should_call_account_from_spec() {
		...
		let spec = ::sofcore::sophon::load(None, include_bytes!("../res/testchain.json"));
