...
use sip_712::SIP712;
use sophon_types::{H160, H256, H520, U128};
use susy_jsonrpc_core::types::Value;
use susy_jsonrpc_core::{BoxFuture, Result};
use susy_jsonrpc_derive::rpc;
use v1::types::{Bytes, TransactionRequest, RichRawTransaction as RpcRichRawTransaction, SIP191Version};
...
pub trait Personal {
	...
	/// Produces an SIP-712 compliant signature with given account using the given password to unlock the
	/// account during the request.
	#[rpc(name = "personal_signTypedData")]
	fn sign_typed_data(&self, SIP712, H160, String) -> BoxFuture<H520>;

	/// Signs an arbitrary message based on the version specified
	#[rpc(name = "personal_sign191")]
	fn sign_191(&self, SIP191Version, Value, H160, String) -> BoxFuture<H520>;
