...
use sofkey::{recover, public_to_address, Signature};
use sophon_types::{H256, U64};
...
/// helper method for susy_verifySignature
...
mod tests {
	use super::*;
	use sofkey::Generator;
	use sophon_types::{H160, U64};
	...
	fn sign(should_prefix: bool, data: Vec<u8>, signing_chain_id: Option<u64>) -> (H160, [u8; 32], [u8; 32], U64) {
		let hash = if should_prefix { sof_data_hash(data) } else { keccak(data) };
		let account = sofkey::Random.generate().unwrap();
		let address = account.address();
		let sig = sofkey::sign(account.secret(), &hash).unwrap();
