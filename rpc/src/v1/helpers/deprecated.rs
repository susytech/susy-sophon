...
mod tests {
	...
	fn should_throttle_printing() {
		...
		notice.print("sof_test", details.clone());
		// printer shouldn't be called
		notice.print("sof_test", None);
		assert_eq!(saved.read().clone().unwrap(), ("sof_test", details.as_ref().map(|x| x.to_string())));
		// but calling a different method is fine
		notice.print("sof_test2", None);
		assert_eq!(saved.read().clone().unwrap(), ("sof_test2", None));

		// wait and call again
		*now.write() = Instant::now() + PRINT_INTERVAL;
		notice.print("sof_test", None);
		assert_eq!(saved.read().clone().unwrap(), ("sof_test", None));
	}
}
