...
use sofcore::executed::ExecutionError;

use susy_jsonrpc_core::{Result, Error};
...
use sophon_types::{Address, U256};
use hash::H256;
use parking_lot::{Mutex, RwLock};
use fastmap::H256FastMap;
use std::collections::BTreeMap;
use types::transaction::{Action, Transaction as SofTransaction, PendingTransaction, SignedTransaction, LocalizedTransaction};
	...
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static
{
	...
	pub fn proved_read_only_execution(
		...
		Box::new(nonce_fut.join(gas_price_fut).and_then(move |(nonce, gas_price)| {
			future::done(
				Ok((req.gas.is_some(), SofTransaction {
	...
	pub fn logs_no_tx_hash(&self, filter: SofcoreFilter) -> impl Future<Item = Vec<Log>, Error = Error> + Send {
		use susy_jsonrpc_core::futures::stream::{self, Stream};
	...
	pub fn logs(&self, filter: SofcoreFilter) -> impl Future<Item = Vec<Log>, Error = Error> + Send {
		use susy_jsonrpc_core::futures::stream::{self, Stream};
	...
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static
{
	...
	hdr: encoded::Header,
	env_info: ::vm::EnvInfo,
	engine: Arc<::sofcore::engines::SofEngine>,
