...
use sofcore::error::{Error as SofcoreError, ErrorKind, CallError};
use sofcore::client::BlockId;
use susy_jsonrpc_core::{futures, Result as RpcResult, Error, ErrorCode, Value};
use srlp::DecoderError;
use types::transaction::Error as TransactionError;
use sofcore_private_tx::Error as PrivateTransactionError;
...
pub fn cannot_restart() -> Error {
	Error {
		...
		message: "Susy could not be restarted. This feature is disabled in development mode and if the binary name isn't susy.".into(),
		data: None,
	}
}
...
pub fn no_author() -> Error {
	Error {
		...
		message: "Author not configured. Run Susy with --author to configure.".into(),
		data: None,
	}
}
...
pub fn cannot_submit_work(err: SofcoreError) -> Error {
	Error {
	...
	where C: BlockChainClient,
{
	move |response| {
		if response.is_none() && !allow_missing_blocks {
			...
			// are gaps in the database sofcore/src/blockchain/blockchain.rs
...
pub fn transaction_message(error: &TransactionError) -> String {
	...
	match *error {
		...
		InvalidRlp(ref descr) => format!("Invalid SRLP data: {}", descr),
	}
}

pub fn transaction<T: Into<SofcoreError>>(error: T) -> Error {
...
pub fn decode<T: Into<SofcoreError>>(error: T) -> Error {
	let error = error.into();
	match *error.kind() {
		ErrorKind::Decoder(ref dec_err) => srlp(dec_err.clone()),
		_ => Error {
			code: ErrorCode::InternalError,
...
pub fn srlp(error: DecoderError) -> Error {
	Error {
		code: ErrorCode::InvalidParams,
		message: "Invalid SRLP.".into(),
		data: Some(Value::String(format!("{:?}", error))),
	}
}
...
pub fn call(error: CallError) -> Error {
	match error {
		...
		CallError::TransactionNotFound => internal("{}, this should not be the case with sof_call, most likely a bug.", CallError::TransactionNotFound),
	}
}
...
pub fn filter_block_not_found(id: BlockId) -> Error {
	Error {
		code: ErrorCode::ServerError(codes::UNSUPPORTED_REQUEST), // Specified in SIP-234.
...
pub fn require_experimental(allow_experimental_rpcs: bool, sip: &str) -> Result<(), Error> {
	if allow_experimental_rpcs {
		Ok(())
	} else {
		Err(Error {
			code: ErrorCode::ServerError(codes::EXPERIMENTAL_RPC),
			message: format!("This method is not part of the official RPC API yet (SIP-{}). Run with `--susy-jsonrpc-experimental` to enable it.", sip),
			data: Some(Value::String(format!("See SIP: https://sips.superstring.io/SIPS/sip-{}", sip))),
		})
	}
}
