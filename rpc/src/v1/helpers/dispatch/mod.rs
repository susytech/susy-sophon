...
use sophon_types::{H520, H256, U256, Address};
use sofkey::{Password, Signature};
use hash::keccak;
use types::transaction::{SignedTransaction, PendingTransaction};

use susy_jsonrpc_core::{BoxFuture, Result, Error};
use susy_jsonrpc_core::futures::{future, Future, IntoFuture};
...
use v1::types::{
	...
	SofSignRequest as RpcSofSignRequest,
	SIP191SignRequest as RpcSignRequest,
...
pub enum SignMessage {
	/// Sof-sign kind data (requires prefixing)
	Data(Bytes),
	/// Prefixed data hash
	Hash(H256),
}
...
pub fn execute<D: Dispatcher + 'static>(
	...
	match payload {
		...
		ConfirmationPayload::SofSignMessage(address, data) => {
	...
	where D: Dispatcher
{
	match payload {
		...
		RpcConfirmationPayload::SofSignMessage(RpcSofSignRequest { address, data }) => {
			Box::new(future::ok(ConfirmationPayload::SofSignMessage(address.into(), data.into())))
		},
		RpcConfirmationPayload::SIP191SignMessage(RpcSignRequest { address, data }) => {
