...
//! Susy-specific metadata extractors.
...
mod tests {
	...
	fn should_extract_rpc_origin() {
		...
		let meta2 = extractor.read_metadata(None, Some("http://superstring.io".to_owned()));
		let meta3 = extractor.read_metadata(None, Some("http://superstring.io".to_owned()));

		// then
		assert_eq!(meta1.origin, Origin::Rpc("unknown origin / unknown agent".into()));
		assert_eq!(meta2.origin, Origin::Rpc("unknown origin / http://superstring.io".into()));
		assert_eq!(meta3.origin, Origin::Rpc("unknown origin / http://superstring.io".into()));
	}
}
