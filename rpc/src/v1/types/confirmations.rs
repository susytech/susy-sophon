...
impl fmt::Display for ConfirmationPayload {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			...
			ConfirmationPayload::SofSignMessage(ref sign) => write!(f, "{}", sign),
			ConfirmationPayload::SIP191SignMessage(ref sign) => write!(f, "{}", sign),
			ConfirmationPayload::Decrypt(ref decrypt) => write!(f, "{}", decrypt),
		}
	}
}

/// Sophon-prefixed Sign request
#[derive(Debug, Clone, Eq, PartialEq, Hash, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct SofSignRequest {
	/// Address
	pub address: H160,
	/// Hash to sign
	pub data: Bytes,
...
impl From<(H160, H256)> for SIP191SignRequest {
...
impl fmt::Display for SIP191SignRequest {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(
			f,
			"sign 0x{} with {}",
...
impl From<(H160, Bytes)> for SofSignRequest {
...
impl fmt::Display for SofSignRequest {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(
			f,
			"sign 0x{} with {}",
...
impl Serialize for ConfirmationResponse {
		...
		where S: Serializer
	{
		match *self {
			...
			ConfirmationResponse::SignTransaction(ref srlp) => srlp.serialize(serializer),
...
pub enum ConfirmationPayload {
	...
	/// Signature
	#[serde(rename = "sign")]
	SofSignMessage(SofSignRequest),
	/// signature without prefix
	SIP191SignMessage(SIP191SignRequest),
	/// Decryption
...
impl From<helpers::ConfirmationPayload> for ConfirmationPayload {
	fn from(c: helpers::ConfirmationPayload) -> Self {
		match c {
			...
			helpers::ConfirmationPayload::SofSignMessage(address, data) => ConfirmationPayload::SofSignMessage(SofSignRequest {
				address: address.into(),
				data: data.into(),
			}),
			helpers::ConfirmationPayload::SignMessage(address, data) => ConfirmationPayload::SIP191SignMessage(SIP191SignRequest {
				address: address.into(),
				data: data.into(),
			}),
...
mod tests {
	...
	fn should_serialize_sign_confirmation() {
		...
		let request = helpers::ConfirmationRequest {
			id: 15.into(),
			payload: helpers::ConfirmationPayload::SofSignMessage(1.into(), vec![5].into()),
