...
pub fn format_sophy(i: U256) -> String {
...
impl fmt::Display for TransactionRequest {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let sof = self.value.unwrap_or(U256::from(0));
		match self.to {
			Some(ref to) => write!(
				f,
				"{} SOF from {} to 0x{:?}",
				Colour::White.bold().paint(format_sophy(sof)),
			...
			None => write!(
				f,
				"{} SOF from {} for contract creation",
				Colour::White.bold().paint(format_sophy(sof)),
				Colour::White.bold().paint(
					self.from.as_ref()
						.map(|f| format!("0x{:?}", f))
...
mod tests {
	...
	#[test]
	fn test_format_sophy() {
		assert_eq!(&format_sophy(U256::from(1000000000000000000u64)), "1");
		assert_eq!(&format_sophy(U256::from(500000000000000000u64)), "0.5");
		assert_eq!(&format_sophy(U256::from(50000000000000000u64)), "0.05");
		assert_eq!(&format_sophy(U256::from(5000000000000000u64)), "0.005");
		assert_eq!(&format_sophy(U256::from(2000000000000000000u64)), "2");
		assert_eq!(&format_sophy(U256::from(2500000000000000000u64)), "2.5");
		assert_eq!(&format_sophy(U256::from(10000000000000000000u64)), "10");
	}
}
