...
use sophon_types::{H160, H256, U256, Bloom as H2048};
use serde::ser::Error;
use serde::{Serialize, Serializer};
use types::encoded::Header as SofHeader;
...
impl From<SofHeader> for Header {
	fn from(h: SofHeader) -> Self {
		(&h).into()
	}
}

impl<'a> From<&'a SofHeader> for Header {
	fn from(h: &'a SofHeader) -> Self {
		Header {
			hash: Some(h.hash().into()),
			size: Some(h.srlp().as_raw().len().into()),
...
mod tests {
	use std::collections::BTreeMap;
	use sophon_types::{H64, H160, H256, U256, Bloom as H2048};
	use serde_json;
