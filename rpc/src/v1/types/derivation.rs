...
impl Derive {
	...
	#[cfg(any(test, feature = "accounts"))]
	pub fn to_derivation(self) -> Result<sofstore::Derivation, ConvertError> {
		Ok(match self {
			Derive::Hierarchical(drv) => {
				sofstore::Derivation::Hierarchical({
					let mut members = Vec::<sofstore::IndexDerivation>::new();
					for h in drv {
						if h.index > ::std::u32::MAX as u64 { return Err(ConvertError::IndexOverlfow(h.index)); }
						members.push(match h.d_type {
							DerivationType::Soft => sofstore::IndexDerivation { soft: true, index: h.index as u32 },
							DerivationType::Hard => sofstore::IndexDerivation { soft: false, index: h.index as u32 },
						});
					}
					members
			   })
			...
			Derive::Hash(drv) => {
				match drv.d_type {
					DerivationType::Soft => sofstore::Derivation::SoftHash(drv.hash.into()),
					DerivationType::Hard => sofstore::Derivation::HardHash(drv.hash.into()),
				}
			},
		})
	}
