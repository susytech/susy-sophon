...
use susy_jsonrpc_core::{Error as RpcError};
...
impl Filter {
	pub fn try_into(self) -> Result<SofFilter, RpcError> {
...
mod tests {
	...
	fn filter_conversion() {
		...
		let sof_filter: SofFilter = filter.try_into().unwrap();
		assert_eq!(sof_filter, SofFilter {
