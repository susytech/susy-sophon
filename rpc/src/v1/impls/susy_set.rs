...
use sofcore::client::{BlockChainClient, Mode};
...
pub mod accounts {
	...
	/// Susy-specific account-touching RPC interfaces.
	pub struct SusySetAccountsClient<M> {
		miner: Arc<M>,
		accounts: Arc<AccountProvider>,
		deprecation_notice: DeprecationNotice,
	}

	impl<M> SusySetAccountsClient<M> {
		/// Creates new SusySetAccountsClient
		pub fn new(
			accounts: &Arc<AccountProvider>,
			miner: &Arc<M>,
		) -> Self {
			SusySetAccountsClient {
	...
	impl<M: MinerService + 'static> SusySetAccounts for SusySetAccountsClient<M> {
		fn set_engine_signer(&self, address: H160, password: String) -> Result<bool> {
			self.deprecation_notice.print(
				"susy_setEngineSigner",
				"use `susy_setEngineSignerSecret` instead. See #9997 for context."
			);
...
/// Susy-specific rpc interface for operations altering the settings.
pub struct SusySetClient<C, M, U, F = fetch::Client> {
	client: Arc<C>,
	miner: Arc<M>,
	updater: Arc<U>,
...
impl<C, M, U, F> SusySetClient<C, M, U, F>
	where C: BlockChainClient + 'static,
{
	/// Creates new `SusySetClient` with given `Fetch`.
	pub fn new(
		client: &Arc<C>,
		miner: &Arc<M>,
		updater: &Arc<U>,
...
impl<C, M, U, F> SusySet for SusySetClient<C, M, U, F> where
	...
	F: Fetch + 'static,
{
	...
	fn set_engine_signer_secret(&self, secret: H256) -> Result<bool> {
		let keypair = sofkey::KeyPair::from_secret(secret.into()).map_err(|e| errors::account("Invalid secret", e))?;
		self.miner.set_author(miner::Author::Sealer(sofcore::engines::signer::from_keypair(keypair)));
		Ok(true)
	}
