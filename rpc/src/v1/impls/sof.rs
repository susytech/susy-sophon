...
use srlp::Rlp;
use sophon_types::{Address, H64, H160, H256, U64, U256};
use parking_lot::Mutex;

use sofash::{self, SeedHashCompute};
use sofcore::client::{BlockChainClient, BlockId, TransactionId, UncleId, StateOrBlock, StateClient, StateInfo, Call, EngineInfo, ProvingBlockChainClient};
...
use types::BlockNumber as SofBlockNumber;
use types::encoded;
use types::filter::Filter as SofcoreFilter;
...
use v1::types::{
	...
	Transaction, CallRequest, Index, Filter, Log, Receipt, Work, SofAccount, StorageProof,
	block_number_to_id
};
...
impl SofClientOptions {
	/// Creates new default `SofClientOptions` and allows alterations
...
impl Default for SofClientOptions {
	fn default() -> Self {
		SofClientOptions {
...
/// Sof rpc implementation.
pub struct SofClient<C, SN: ?Sized, S: ?Sized, M, EM> where
...
pub fn base_logs<C, M, T: StateInfo + 'static> (client: &C, miner: &M, filter: Filter) -> BoxFuture<Vec<Log>> where
	...
	let filter: SofcoreFilter = match filter.try_into() {
		Ok(value) => value,
...
impl<C, SN: ?Sized, S: ?Sized, M, EM, T: StateInfo + 'static> SofClient<C, SN, S, M, EM> where
			...
			BlockNumberOrId::Number(BlockNumber::Pending) => {
				...
				match self.miner.pending_block(info.best_block_number) {
					Some(pending_block) => {
						...
						(Some(encoded::Block::new(pending_block.srlp_bytes())), Some(difficulty), Some(extra), true)
					},
					None => {
			...
			(Some(block), Some(total_difficulty)) => {
				...
				Ok(Some(RichBlock {
					inner: Block {
						...
						size: Some(block.srlp().as_raw().len().into()),
...
pub fn pending_logs<M>(miner: &M, best_block: SofBlockNumber, filter: &SofcoreFilter) -> Vec<Log> where M: MinerService {
...
impl<C, SN: ?Sized, S: ?Sized, M, EM, T: StateInfo + 'static> Sof for SofClient<C, SN, S, M, EM> where
	...
	EM: ExternalMinerService + 'static,
{
	...
	fn accounts(&self) -> Result<Vec<H160>> {
		self.deprecation_notice.print("sof_accounts", deprecated::msgs::ACCOUNTS);
	...
	fn proof(&self, address: H160, values: Vec<H256>, num: Option<BlockNumber>) -> BoxFuture<SofAccount> {
		...
		let res = match self.client.prove_account(key1, id) {
			Some((proof, account)) => Ok(SofAccount {
				address: address,
	...
	fn work(&self, no_new_work_timeout: Option<u64>) -> Result<Work> {
		...
		let target = sofash::difficulty_to_boundary(&difficulty);
	...
	fn compile_polynomial(&self, _: String) -> Result<Bytes> {
		Err(errors::deprecated("Compilation of Polynomial via RPC is deprecated".to_string()))
	}
}
