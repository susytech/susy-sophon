...
//! Susy-specific PUB-SUB rpc implementation.

use std::sync::Arc;
use std::time::Duration;
use parking_lot::RwLock;

use susy_jsonrpc_core::{self as core, Result, MetaIoHandler};
use susy_jsonrpc_core::futures::{future, Future, Stream, Sink};
use susy_jsonrpc_pubsub::typed::Subscriber;
use susy_jsonrpc_pubsub::SubscriptionId;
use tokio_timer;
...
impl<S: core::Middleware<Metadata>> PubSub for PubSubClient<S> {
	...
	fn susy_subscribe(&self, mut meta: Metadata, subscriber: Subscriber<core::Value>, method: String, params: Option<core::Params>) {
	...
	fn susy_unsubscribe(&self, _: Option<Self::Metadata>, id: SubscriptionId) -> Result<bool> {
