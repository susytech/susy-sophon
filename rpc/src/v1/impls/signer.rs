...
use susy_jsonrpc_core::{Result, BoxFuture, Error};
...
use v1::helpers::dispatch::{self, Dispatcher, WithToken, sof_data_hash};
...
impl<D: Dispatcher + 'static> SignerClient<D> {
	...
	{
		let signed_transaction = Rlp::new(&bytes.0).as_val().map_err(errors::srlp)?;
		...
		self.signer.take(&id).map(|sender| {
			...
			let result = match payload {
				...
				ConfirmationPayload::SofSignMessage(address, data) => {
					let expected_hash = sof_data_hash(data);
					let signature = sofkey::Signature::from_electrum(&bytes.0);
					match sofkey::verify_address(&address, &signature, &expected_hash) {
				...
				ConfirmationPayload::SignMessage(address, hash) => {
					let signature = sofkey::Signature::from_electrum(&bytes.0);
					match sofkey::verify_address(&address, &signature, &hash) {
						Ok(true) => Ok(ConfirmationResponse::Signature(bytes.0.as_slice().into())),
						Ok(false) => Err(errors::invalid_params("Sender address does not match the signature.", ())),
						Err(err) => Err(errors::invalid_params("Invalid signature received.", err)),
					}
