...
pub trait Filterable {
	...
	fn pending_logs(&self, block_number: u64, filter: &SofcoreFilter) -> Vec<Log>;
	...
	fn removed_logs(&self, block_hash: H256, filter: &SofcoreFilter) -> (Vec<Log>, u64);
}

/// Sof filter rpc implementation for a full node.
pub struct SofFilterClient<C, M> {
	client: Arc<C>,
	miner: Arc<M>,
	polls: Mutex<PollManager<SyncPollFilter>>,
}

impl<C, M> SofFilterClient<C, M> {
	/// Creates new Sof filter client.
...
impl<C, M> Filterable for SofFilterClient<C, M> where
...
impl<T: Filterable + Send + Sync + 'static> SofFilter for T {
