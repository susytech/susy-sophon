...
/// Susy implementation for light client.
pub struct SusyClient<S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static> {
		...
		let from_encoded = move |encoded: encoded::Header| {
			...
			Ok(RichHeader {
				inner: Header {
					...
					size: Some(encoded.srlp().as_raw().len().into()),
