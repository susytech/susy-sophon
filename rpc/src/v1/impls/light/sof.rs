...
//! Sof RPC interface for the light client.
...
use v1::impls::sof_filter::Filterable;
...
use v1::types::{
	...
	SyncInfo as RpcSyncInfo, Transaction, CallRequest, Index, Filter, Log, Receipt, Work, SofAccount
};
...
/// Light client `SOF` (and filter) RPC.
pub struct SofClient<C, S: LightSyncProvider + LightNetworkDispatcher + 'static> {
	sync: Arc<S>,
	client: Arc<C>,
...
impl<C, S> Clone for SofClient<C, S>
where
...
impl<C, S> SofClient<C, S>
where
	C: LightChainClient + 'static,
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static
{
	/// Create a new `SofClient` with a handle to the light sync instance, client,
...
impl<C, S> Sof for SofClient<C, S>
where
	C: LightChainClient + 'static,
	S: LightSyncInfo + LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static
{
	...
	fn proof(&self, _address: H160, _values:Vec<H256>, _num: Option<BlockNumber>) -> BoxFuture<SofAccount> {
		Box::new(future::err(errors::unimplemented(None)))
	}
...
// This trait implementation triggers a blanked impl of `SofFilter`.
impl<C, S> Filterable for SofClient<C, S>
where
	C: LightChainClient + 'static,
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static
{
	...
	fn logs(&self, filter: SofcoreFilter) -> BoxFuture<Vec<Log>> {
		Box::new(self.fetcher().logs(filter)) as BoxFuture<_>
	}

	fn pending_logs(&self, _block_number: u64, _filter: &SofcoreFilter) -> Vec<Log> {
	...
	fn removed_logs(&self, _block_hash: ::sophon_types::H256, _filter: &SofcoreFilter) -> (Vec<Log>, u64) {
		(Default::default(), 0)
	}
}
