...
//! Susy-specific rpc implementation.
use std::sync::Arc;
use std::str::FromStr;
use std::collections::BTreeMap;

use crypto::DEFAULT_MAC;
use sophon_types::{Address, H64, H160, H256, H512, U64, U256};
use sofcore::client::{BlockChainClient, StateClient, Call};
use sofcore::miner::{self, MinerService};
use sofcore::snapshot::{SnapshotService, RestorationStatus};
use sofcore::state::StateInfo;
use sofcore_logger::RotatingLogger;
use sofkey::{crypto::ecies, Brain, Generator};
use sofstore::random_phrase;
...
impl<C, M, U> SusyClient<C, M, U> where
	C: BlockChainClient,
{
	/// Creates new `SusyClient`.
	pub fn new(
		client: Arc<C>,
		miner: Arc<M>,
		sync: Arc<SyncProvider>,
...
impl<C, M, U, S> Susy for SusyClient<C, M, U> where
	S: StateInfo + 'static,
	...
	U: UpdateService + 'static,
{
	...
	fn future_transactions(&self) -> Result<Vec<Transaction>> {
		Err(errors::deprecated("Use `susy_allTransaction` instead."))
	}
