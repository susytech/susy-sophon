...
use sophon_types::{Address, H160, H256, H520, U256};
...
impl<D: Dispatcher + 'static> SofSigning for SigningUnsafeClient<D>
{
	...
	fn sign(&self, _: Metadata, address: H160, data: RpcBytes) -> BoxFuture<H520> {
		...
		Box::new(self.handle(RpcConfirmationPayload::SofSignMessage((address.clone(), data).into()), address.into())
			.then(|res| match res {
...
impl<D: Dispatcher + 'static> SusySigning for SigningUnsafeClient<D> {
	type Metadata = Metadata;
