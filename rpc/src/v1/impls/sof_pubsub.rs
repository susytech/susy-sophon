...
//! Sof PUB-SUB rpc implementation.

use std::sync::{Arc, Weak};
use std::collections::BTreeMap;

use susy_jsonrpc_core::{BoxFuture, Result, Error};
use susy_jsonrpc_core::futures::{self, Future, IntoFuture};
use susy_jsonrpc_pubsub::{SubscriptionId, typed::{Sink, Subscriber}};

use v1::helpers::{errors, limit_logs, Subscribers};
use v1::helpers::light_fetch::LightFetch;
use v1::metadata::Metadata;
use v1::traits::SofPubSub;
use v1::types::{pubsub, RichHeader, Log};

use sofcore::client::{BlockChainClient, ChainNotify, NewBlocks, ChainRouteType, BlockId};
...
use types::encoded;
use types::filter::Filter as SofFilter;

type Client = Sink<pubsub::Result>;

/// Sof PubSub implementation.
pub struct SofPubSubClient<C> {
	handler: Arc<ChainNotificationHandler<C>>,
	heads_subscribers: Arc<RwLock<Subscribers<Client>>>,
	logs_subscribers: Arc<RwLock<Subscribers<(Client, SofFilter)>>>,
	transactions_subscribers: Arc<RwLock<Subscribers<Client>>>,
}

impl<C> SofPubSubClient<C> {
	/// Creates new `SofPubSubClient`.
	...
	/// Creates new `SofPubSubCient` with deterministic subscription ids.
	#[cfg(test)]
...
impl<S> SofPubSubClient<LightFetch<S>>
where
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static
{
	/// Creates a new `SofPubSubClient` for `LightClient`.
	pub fn light(
		...
		SofPubSubClient::new(Arc::new(fetch), executor)
	}
}
...
pub trait LightClient: Send + Sync {
	...
	/// Fetch logs.
	fn logs(&self, filter: SofFilter) -> BoxFuture<Vec<Log>>;
}
...
impl<C: Send + Sync + 'static> SofPubSub for SofPubSubClient<C> {
