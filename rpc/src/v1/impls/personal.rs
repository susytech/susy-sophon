...
use sophon_types::{H160, H256, H520, U128, Address};
use sofkey::{public_to_address, recover, Signature};
use types::transaction::{PendingTransaction, SignedTransaction};

use susy_jsonrpc_core::futures::{future, Future};
use susy_jsonrpc_core::types::Value;
use susy_jsonrpc_core::{BoxFuture, Result};
use v1::helpers::deprecated::{self, DeprecationNotice};
use v1::helpers::dispatch::{self, sof_data_hash, Dispatcher, SignWith, PostSign, WithToken};
use v1::helpers::{errors, sip191};
...
impl<D: Dispatcher + 'static> Personal for PersonalClient<D> {
	...
	fn sign(&self, data: RpcBytes, account: H160, password: String) -> BoxFuture<H520> {
		...
		let payload = RpcConfirmationPayload::SofSignMessage((account.clone(), data).into());
	...
	fn sign_191(&self, version: SIP191Version, data: Value, account: H160, password: String) -> BoxFuture<H520> {
		...
		let data = try_bf!(sip191::hash_message(version, data));
		let dispatcher = self.dispatcher.clone();
		let accounts = Arc::new(dispatch::Signer::new(self.accounts.clone())) as _;

		let payload = RpcConfirmationPayload::SIP191SignMessage((account.clone(), data.into()).into());
	...
	fn sign_typed_data(&self, typed_data: SIP712, account: H160, password: String) -> BoxFuture<H520> {
