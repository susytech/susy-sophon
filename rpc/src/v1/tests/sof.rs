...
use sofcore::client::{BlockChainClient, Client, ClientConfig, ChainInfo, ImportBlock};
use sofcore::sophon;
use sofcore::miner::Miner;
use sofcore::spec::{Genesis, Spec};
use sofcore::test_helpers;
use sofcore::verification::VerifierType;
use sofcore::verification::queue::kind::blocks::Unverified;
use sophon_types::{Address, H256, U256};
use sofjson::blockchain::BlockChain;
...
use v1::impls::{SofClient, SofClientOptions, SigningUnsafeClient};
use v1::metadata::Metadata;
use v1::tests::helpers::{TestSnapshotService, TestSyncProvider, Config};
use v1::traits::{Sof, SofSigning};
...
fn make_spec(chain: &BlockChain) -> Spec {
	...
	let mut spec = sophon::new_frontier_test();
...
impl SofTester {
	fn from_chain(chain: &BlockChain) -> Self {
		...
		let tester = if ::sofjson::blockchain::Engine::NoProof == chain.engine {
	...
	fn from_spec_conf(spec: Spec, config: ClientConfig) -> Self {
		...
		let sof_sign = SigningUnsafeClient::new(
			&signer,
			dispatcher,
		);

		let mut handler = IoHandler::default();
		handler.extend_with(sof_client.to_delegate());
		handler.extend_with(sof_sign.to_delegate());
...
fn harness_works() {
	...
	let _ = SofTester::from_chain(&chain);
}

#[test]
fn sof_get_balance() {
	let chain = extract_chain!("BlockchainTests/bcWalletTest/wallet2outOf3txs");
	let tester = SofTester::from_chain(&chain);
	// final account state
	let req_latest = r#"{
		"jsonrpc": "2.0",
...
fn sof_get_block() {
	let chain = extract_chain!("BlockchainTests/bcGasPricerTest/RPC_API_Test");
	let tester = SofTester::from_chain(&chain);
	let req_block = r#"{"method":"sof_getBlockByNumber","params":["0x0",false],"id":1,"jsonrpc":"2.0"}"#;
...
#[test]
fn sof_get_block_by_hash() {
	let chain = extract_chain!("BlockchainTests/bcGasPricerTest/RPC_API_Test");
	let tester = SofTester::from_chain(&chain);

	// We're looking for block number 4 from "RPC_API_Test_Frontier"
	let req_block = r#"{"method":"sof_getBlockByHash","params":["0xaddb9e39795e9e041c936b88a2577802569f34afded0948707b074caa3163a87",false],"id":1,"jsonrpc":"2.0"}"#;
...
#[test]
fn sof_transaction_count() {
	let secret = "8a283037bb19c4fed7b1c569e40c7dcff366165eb869110a1b11532963eb9cb2".parse().unwrap();
	let tester = SofTester::from_spec(Spec::load(&env::temp_dir(), TRANSACTION_COUNT_SPEC).expect("invalid chain spec"));
...
fn verify_transaction_counts(name: String, chain: BlockChain) {
	...
	let mut id = 1;
	for b in chain.blocks_srlp().into_iter().filter_map(|b| Unverified::from_srlp(b).ok()) {
...
fn starting_nonce_test() {
	let tester = SofTester::from_spec(Spec::load(&env::temp_dir(), POSITIVE_NONCE_SPEC).expect("invalid chain spec"));
...
register_test!(sof_transaction_count_1, verify_transaction_counts, "BlockchainTests/bcWalletTest/wallet2outOf3txs");
register_test!(sof_transaction_count_2, verify_transaction_counts, "BlockchainTests/bcTotalDifficultyTest/sideChainWithMoreTransactions");
register_test!(sof_transaction_count_3, verify_transaction_counts, "BlockchainTests/bcGasPricerTest/RPC_API_Test");
