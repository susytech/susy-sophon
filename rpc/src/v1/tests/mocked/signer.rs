...
use std::sync::Arc;
use std::str::FromStr;
use sophon_types::{H520, U256, Address};
use bytes::ToPretty;
...
use v1::helpers::dispatch::{self, FullDispatcher, sof_data_hash};
...
fn should_return_list_of_items_to_confirm() {
	...
	let _sign_future = tester.signer.add_request(ConfirmationPayload::SofSignMessage(1.into(), vec![5].into()), Origin::Unknown).unwrap();
...
fn should_not_remove_sign_if_password_is_invalid() {
	// given
	let tester = signer_tester();
	let _confirmation_future = tester.signer.add_request(ConfirmationPayload::SofSignMessage(0.into(), vec![5].into()), Origin::Unknown).unwrap();
	assert_eq!(tester.signer.requests().len(), 1);
...
#[test]
fn should_confirm_transaction_with_srlp() {
	// given
	let tester = signer_tester();
	let address = tester.accounts.new_account(&"test".into()).unwrap();
	let recipient = Address::from_str("d46e8dd67c5d32be8058bb8eb970870f07244567").unwrap();
	...
	let request = r#"{
		...
		"params":["0x1", "0x"#.to_owned() + &srlp.to_hex() + r#""],
		"id":1
	}"#;
	let response = r#"{"jsonrpc":"2.0","result":""#.to_owned() + format!("0x{:x}", t.hash()).as_ref() + r#"","id":1}"#;
...
#[test]
fn should_confirm_sign_transaction_with_srlp() {
	// given
	let tester = signer_tester();
	let address = tester.accounts.new_account(&"test".into()).unwrap();
	let recipient = Address::from_str("d46e8dd67c5d32be8058bb8eb970870f07244567").unwrap();
...
fn should_confirm_data_sign_with_signature() {
	...
	let tester = signer_tester();
	let address = tester.accounts.new_account(&"test".into()).unwrap();
	let _confirmation_future = tester.signer.add_request(ConfirmationPayload::SofSignMessage(
		address,
		vec![1, 2, 3, 4].into(),
	), Origin::Unknown).unwrap();
	assert_eq!(tester.signer.requests().len(), 1);

	let data_hash = sof_data_hash(vec![1, 2, 3, 4].into());
