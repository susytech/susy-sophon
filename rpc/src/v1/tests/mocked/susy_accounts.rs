...
use sofstore::SofStore;
use sofstore::accounts_dir::RootDiskDirectory;
use tempdir::TempDir;

use susy_jsonrpc_core::IoHandler;
use v1::{SusyAccounts, SusyAccountsInfo, SusyAccountsClient};

struct SusyAccountsTester {
...
fn accounts_provider_with_vaults_support(temp_path: &str) -> Arc<AccountProvider> {
	...
	let secret_store = SofStore::open(Box::new(root_keys_dir)).unwrap();
	Arc::new(AccountProvider::new(Box::new(secret_store), AccountProviderSettings::default()))
}

fn setup_with_accounts_provider(accounts_provider: Arc<AccountProvider>) -> SusyAccountsTester {
	let opt_ap = accounts_provider.clone();
	let susy_accounts = SusyAccountsClient::new(&opt_ap);
	let susy_accounts2 = SusyAccountsClient::new(&opt_ap);
	let mut io = IoHandler::default();
	io.extend_with(SusyAccounts::to_delegate(susy_accounts));
	io.extend_with(SusyAccountsInfo::to_delegate(susy_accounts2));

	let tester = SusyAccountsTester {
...
fn rpc_susy_accounts_info() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_accountsInfo", "params": [], "id": 1}"#;
	let response = format!("{{\"jsonrpc\":\"2.0\",\"result\":{{\"0x{:x}\":{{\"name\":\"Test\"}}}},\"id\":1}}", address);
	assert_eq!(io.handle_request_sync(request), Some(response));
}

#[test]
fn rpc_susy_default_account() {
	let tester = setup();
	let io = tester.io;

	// Check empty
	let address = Address::default();
	let request = r#"{"jsonrpc": "2.0", "method": "susy_defaultAccount", "params": [], "id": 1}"#;
...
fn should_be_able_to_get_account_info() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_allAccountsInfo", "params": [], "id": 1}"#;
...
fn should_be_able_to_set_name() {
	...
	let request = format!(r#"{{"jsonrpc": "2.0", "method": "susy_setAccountName", "params": ["0x{:x}", "Test"], "id": 1}}"#, address);
...
fn should_be_able_to_set_meta() {
	...
	let request = format!(r#"{{"jsonrpc": "2.0", "method": "susy_setAccountMeta", "params": ["0x{:x}", "{{foo: 69}}"], "id": 1}}"#, address);
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));
...
fn should_be_able_to_kill_account() {
	...
	let request = format!(r#"{{"jsonrpc": "2.0", "method": "susy_killAccount", "params": ["0xf00baba2f00baba2f00baba2f00baba2f00baba2"], "id": 1}}"#);
	let response = r#"{"jsonrpc":"2.0","error":{"code":-32602,"message":"Invalid params: invalid length 1, expected a tuple of size 2."},"id":1}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));

	let request = format!(r#"{{"jsonrpc": "2.0", "method": "susy_killAccount", "params": ["0x{:x}", "password"], "id": 1}}"#, address);
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));
...
fn should_be_able_to_remove_address() {
	...
	// add an address
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setAccountName", "params": ["0x000baba1000baba2000baba3000baba4000baba5", "Test"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));

	// verify it exists
	let request = r#"{"jsonrpc": "2.0", "method": "susy_allAccountsInfo", "params": [], "id": 2}"#;
	let res = tester.io.handle_request_sync(request);
	let response = r#"{"jsonrpc":"2.0","result":{"0x000baba1000baba2000baba3000baba4000baba5":{"meta":"{}","name":"Test"}},"id":2}"#;
	assert_eq!(res, Some(response.into()));

	// remove the address
	let request = r#"{"jsonrpc": "2.0", "method": "susy_removeAddress", "params": ["0x000baba1000baba2000baba3000baba4000baba5"], "id": 3}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":3}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));

	// verify empty
	let request = r#"{"jsonrpc": "2.0", "method": "susy_allAccountsInfo", "params": [], "id": 4}"#;
	let res = tester.io.handle_request_sync(request);
...
#[test]
fn rpc_susy_new_vault() {
	let tempdir = TempDir::new("").unwrap();
	let tester = setup_with_vaults_support(tempdir.path().to_str().unwrap());

	let request = r#"{"jsonrpc": "2.0", "method": "susy_newVault", "params":["vault1", "password1"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_susy_open_vault() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_openVault", "params":["vault1", "password1"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_susy_close_vault() {
	let tempdir = TempDir::new("").unwrap();
	let tester = setup_with_vaults_support(tempdir.path().to_str().unwrap());

	assert!(tester.accounts.create_vault("vault1", &"password1".into()).is_ok());

	let request = r#"{"jsonrpc": "2.0", "method": "susy_closeVault", "params":["vault1"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_susy_change_vault_password() {
	let tempdir = TempDir::new("").unwrap();
	let tester = setup_with_vaults_support(tempdir.path().to_str().unwrap());

	assert!(tester.accounts.create_vault("vault1", &"password1".into()).is_ok());

	let request = r#"{"jsonrpc": "2.0", "method": "susy_changeVaultPassword", "params":["vault1", "password2"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_susy_change_vault() {
	...
	let request = format!(r#"{{"jsonrpc": "2.0", "method": "susy_changeVault", "params":["0x{:x}", "vault1"], "id": 1}}"#, address);
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_susy_vault_adds_vault_field_to_acount_meta() {
	let tempdir = TempDir::new("").unwrap();
	let tester = setup_with_vaults_support(tempdir.path().to_str().unwrap());
...
fn rpc_susy_list_vaults() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_listVaults", "params":[], "id": 1}"#;
...
#[test]
fn rpc_susy_list_opened_vaults() {
	let tempdir = TempDir::new("").unwrap();
	let tester = setup_with_vaults_support(tempdir.path().to_str().unwrap());
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_listOpenedVaults", "params":[], "id": 1}"#;
...
#[test]
fn rpc_susy_get_set_vault_meta() {
	let tempdir = TempDir::new("").unwrap();
	let tester = setup_with_vaults_support(tempdir.path().to_str().unwrap());
	...
	// when no meta set
	let request = r#"{"jsonrpc": "2.0", "method": "susy_getVaultMeta", "params":["vault1"], "id": 1}"#;
	...
	// change meta
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setVaultMeta", "params":["vault1", "updated_vault1_meta"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
// name: susy_deriveAddressHash
// example: {"jsonrpc": "2.0", "method": "susy_deriveAddressHash", "params": ["0xc171033d5cbff7175f29dfd3a63dda3d6f8f385e", "password1", { "type": "soft", "hash": "0x0c0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0c0c" }, true ], "id": 3}
#[test]
fn derive_key_hash() {
	let tester = setup();
	...
	// derive by hash
	let request = r#"{"jsonrpc": "2.0", "method": "susy_deriveAddressHash", "params": ["0xc171033d5cbff7175f29dfd3a63dda3d6f8f385e", "password1", { "type": "soft", "hash": "0x0c0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0c0c" }, true ], "id": 3}"#;
	let response = r#"{"jsonrpc":"2.0","result":"0xf28c28fcddf4a9b8f474237278d3647f9c0d1b3c","id":3}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));
}

// name: susy_deriveAddressIndex
// example: {"jsonrpc": "2.0", "method": "susy_deriveAddressIndex", "params": ["0xc171033d5cbff7175f29dfd3a63dda3d6f8f385e", "password1", [{ "type": "soft", "index": 0 }, { "type": "soft", "index": 1 }], false ], "id": 3}
#[test]
fn derive_key_index() {
	let tester = setup();
	let hash = tester.accounts
	...
	// derive by hash
	let request = r#"{"jsonrpc": "2.0", "method": "susy_deriveAddressIndex", "params": ["0xc171033d5cbff7175f29dfd3a63dda3d6f8f385e", "password1", [{ "type": "soft", "index": 0 }, { "type": "soft", "index": 1 }], false ], "id": 3}"#;
...
fn should_export_account() {
	// given
	let tester = setup();
	let wallet = r#"{"id":"6a186c80-7797-cff2-bc2e-7c1d6a6cc76e","version":3,"crypto":{"cipher":"aes-128-ctr","cipherparams":{"iv":"a1c6ff99070f8032ca1c4e8add006373"},"ciphertext":"df27e3db64aa18d984b6439443f73660643c2d119a6f0fa2fa9a6456fc802d75","kdf":"pbkdf2","kdfparams":{"c":10240,"dklen":32,"prf":"hmac-sha256","salt":"ddc325335cda5567a1719313e73b4842511f3e4a837c9658eeb78e51ebe8c815"},"mac":"3dc888ae79cbb226ff9c455669f6cf2d79be72120f2298f6cb0d444fddc0aa3d"},"address":"0042e5d2a662eeaca8a7e828c174f98f35d8925b","name":"susy-export-test","meta":"{\"passwordHint\":\"susy-export-test\",\"timestamp\":1490017814987}"}"#;
	tester.accounts.import_wallet(wallet.as_bytes(), &"susy-export-test".into(), false).unwrap();
	let accounts = tester.accounts.accounts().unwrap();
	assert_eq!(accounts.len(), 1);

	// invalid password
	let request = r#"{"jsonrpc":"2.0","method":"susy_exportAccount","params":["0x0042e5d2a662eeaca8a7e828c174f98f35d8925b","123"],"id":1}"#;
	let response = r#"{"jsonrpc":"2.0","error":{"code":-32023,"message":"Could not export account.","data":"InvalidPassword"},"id":1}"#;
	let res = tester.io.handle_request_sync(&request);
	assert_eq!(res, Some(response.into()));

	// correct password
	let request = r#"{"jsonrpc":"2.0","method":"susy_exportAccount","params":["0x0042e5d2a662eeaca8a7e828c174f98f35d8925b","susy-export-test"],"id":1}"#;

	let response = r#"{"jsonrpc":"2.0","result":{"address":"0042e5d2a662eeaca8a7e828c174f98f35d8925b","crypto":{"cipher":"aes-128-ctr","cipherparams":{"iv":"a1c6ff99070f8032ca1c4e8add006373"},"ciphertext":"df27e3db64aa18d984b6439443f73660643c2d119a6f0fa2fa9a6456fc802d75","kdf":"pbkdf2","kdfparams":{"c":10240,"dklen":32,"prf":"hmac-sha256","salt":"ddc325335cda5567a1719313e73b4842511f3e4a837c9658eeb78e51ebe8c815"},"mac":"3dc888ae79cbb226ff9c455669f6cf2d79be72120f2298f6cb0d444fddc0aa3d"},"id":"6a186c80-7797-cff2-bc2e-7c1d6a6cc76e","meta":"{\"passwordHint\":\"susy-export-test\",\"timestamp\":1490017814987}","name":"susy-export-test","version":3},"id":1}"#;
...
fn should_import_wallet() {
	...
	let request = r#"{"jsonrpc":"2.0","method":"susy_newAccountFromWallet","params":["{\"id\":\"<ID>\",\"version\":3,\"crypto\":{\"cipher\":\"aes-128-ctr\",\"cipherparams\":{\"iv\":\"478736fb55872c1baf01b27b1998c90b\"},\"ciphertext\":\"fe5a63cc0055d7b0b3b57886f930ad9b63f48950d1348145d95996c41e05f4e0\",\"kdf\":\"pbkdf2\",\"kdfparams\":{\"c\":10240,\"dklen\":32,\"prf\":\"hmac-sha256\",\"salt\":\"658436d6738a19731149a98744e5cf02c8d5aa1f8e80c1a43cc9351c70a984e4\"},\"mac\":\"c7384b26ecf25539d942030230062af9b69de5766cbcc4690bffce1536644631\"},\"address\":\"00bac56a8a27232baa044c03f43bf3648c961735\",\"name\":\"hello world\",\"meta\":\"{}\"}", "himom"],"id":1}"#;
...
fn should_sign_message() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_signMessage", "params": ["0xc171033d5cbff7175f29dfd3a63dda3d6f8f385e", "password1", "0xbc36789e7a1e281436464229828f817d6612f7b477d66591ff96a9e064bcc98a"], "id": 3}"#;
