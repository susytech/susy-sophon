...
use sophon_types::{Address, H520, U256};
...
use v1::helpers::{nonce, sip191};
use v1::helpers::dispatch::{sof_data_hash, FullDispatcher};
use v1::tests::helpers::TestMinerService;
use v1::types::{SIP191Version, PresignedTransaction};
use rustc_hex::ToHex;
...
fn ec_recover() {
	...
	let hash = sof_data_hash(data.clone());
	let signature = H520(tester.accounts.sign(address, Some("password123".into()), hash).unwrap().into_electrum());
	let signature = format!("{:?}", signature);
...
fn sign_sip191_with_validator() {
	...
	let result = sip191::hash_message(SIP191Version::PresignedTransaction, with_validator).unwrap();
...
#[test]
fn sign_sip191_structured_data() {
	let tester = setup();
...
fn should_disable_experimental_apis() {
	...
	// then
	let expected = r#"{"jsonrpc":"2.0","error":{"code":-32071,"message":"This method is not part of the official RPC API yet (SIP-191). Run with `--susy-jsonrpc-experimental` to enable it.","data":"See SIP: https://sips.superstring.io/SIPS/sip-191"},"id":1}"#;
	assert_eq!(r1, expected);

	let expected = r#"{"jsonrpc":"2.0","error":{"code":-32071,"message":"This method is not part of the official RPC API yet (SIP-712). Run with `--susy-jsonrpc-experimental` to enable it.","data":"See SIP: https://sips.superstring.io/SIPS/sip-712"},"id":1}"#;
