...
use sophon_types::{U256, Address};

use sofcore::miner::MinerService;
use sofcore::client::TestBlockChainClient;
use sync::ManageNetwork;

use susy_jsonrpc_core::IoHandler;
use v1::{SusySet, SusySetClient};
...
pub type TestSusySetClient = SusySetClient<TestBlockChainClient, TestMinerService, TestUpdater, FakeFetch<usize>>;
...
#[test]
fn rpc_susy_execute_upgrade() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	let mut io = IoHandler::new();
	io.extend_with(susy_set_client(&client, &miner, &updater, &network).to_delegate());

	let request = r#"{"jsonrpc": "2.0", "method": "susy_executeUpgrade", "params": [], "id": 1}"#;
...
fn rpc_susy_upgrade_ready() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_upgradeReady", "params": [], "id": 1}"#;
...
fn rpc_susy_set_min_gas_price() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setMinGasPrice", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_susy_set_min_gas_price_with_automated_calibration_enabled() {
	let miner = miner_service();
	*miner.min_gas_price.write() = None;
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setMinGasPrice", "params":["0xdeadbeef"], "id": 1}"#;
...
#[test]
fn rpc_susy_set_gas_floor_target() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setGasFloorTarget", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_susy_set_extra_data() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setExtraData", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_susy_set_author() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setAuthor", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_susy_set_transactions_limit() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_setTransactionsLimit", "params":[10240240], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":false,"id":1}"#;
...
fn rpc_susy_set_hash_content() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_hashContent", "params":["http://superstring.io/assets/images/sofcore-black-horizontal.png"], "id": 1}"#;
...
#[test]
fn rpc_susy_remove_transaction() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "susy_removeTransaction", "params":[""#.to_owned() + &format!("0x{:x}", hash) + r#""], "id": 1}"#;
...
#[test]
fn rpc_susy_set_engine_signer() {
	use accounts::AccountProvider;
	use bytes::ToPretty;
	use v1::impls::SusySetAccountsClient;
	use v1::traits::SusySetAccounts;
	...
	let mut io = IoHandler::new();
	io.extend_with(
		SusySetAccountsClient::new(&account_provider, &miner).to_delegate()
	);

	let request = r#"{"jsonrpc": "2.0", "method": "susy_setEngineSigner", "params":["0xcd2a3d9f938e13cd947ec05abc7fe734df8dd826", "password"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
