...
use susy_jsonrpc_core::{IoHandler, Success};
use susy_jsonrpc_core::futures::Future;
use v1::impls::SigningQueueClient;
use v1::metadata::Metadata;
use v1::traits::{SofSigning, SusySigning, Susy};
...
use susy_runtime::{Runtime, Executor};
...
impl Default for SigningTester {
	fn default() -> Self {
		...
		io.extend_with(SofSigning::to_delegate(rpc));
		let rpc = SigningQueueClient::new(&signer, dispatcher, executor, &account_signer);
		io.extend_with(SusySigning::to_delegate(rpc));
...
fn should_add_sign_transaction_to_the_queue() {
	...
	let srlp = srlp::encode(&t);

	let response = r#"{"jsonrpc":"2.0","result":{"#.to_owned() +
		r#""raw":"0x"# + &srlp.to_hex() + r#"","# +
		r#""tx":{"# +
...
fn should_decrypt_message_if_account_is_unlocked() {
	// given
	let mut tester = sof_signing();
	let susy = susy::Dependencies::new();
	tester.io.extend_with(susy.client(None).to_delegate());
	...
	let request = format!("{}0x{:x}{}",
		r#"{"jsonrpc": "2.0", "method": "susy_encryptMessage", "params":[""#,
		public,
		r#"", "0x01020304"], "id": 1}"#
	);
	...
	let request = format!("{}{:x}{}{}{}",
		r#"{"jsonrpc": "2.0", "method": "susy_decryptMessage", "params":["0x"#,
		address,
		r#"","#,
		encrypted.result,
		r#"], "id": 1}"#
...
fn should_compose_transaction() {
	...
	let request = r#"{
		"jsonrpc": "2.0",
		"method": "susy_composeTransaction",
