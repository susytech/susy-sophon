...
use v1::{SofClientOptions, SofSigning, SigningUnsafeClient};
...
#[test]
fn rpc_sof_send_transaction_with_bad_to() {
	let tester = SofTester::default();
	let address = tester.accounts_provider.new_account(&"".into()).unwrap();
	let request = r#"{
		"jsonrpc": "2.0",
...
#[test]
fn rpc_sof_send_transaction_error() {
	let tester = SofTester::default();
	let address = tester.accounts_provider.new_account(&"".into()).unwrap();
	let request = r#"{
		"jsonrpc": "2.0",
