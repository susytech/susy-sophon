...
fn should_subscribe_to_a_method() {
	...
	// Subscribe
	let request = r#"{"jsonrpc": "2.0", "method": "susy_subscribe", "params": ["hello", []], "id": 1}"#;
	...
	let response =
		r#"{"jsonrpc":"2.0","method":"susy_subscription","params":{"result":"hello","subscription":"0x416d77337e24399d"}}"#;
	assert_eq!(res, Some(response.into()));

	let (res, receiver) = receiver.into_future().wait().unwrap();
	let response =
		r#"{"jsonrpc":"2.0","method":"susy_subscription","params":{"result":"world","subscription":"0x416d77337e24399d"}}"#;
	assert_eq!(res, Some(response.into()));

	// And unsubscribe
	let request = r#"{"jsonrpc": "2.0", "method": "susy_unsubscribe", "params": ["0x416d77337e24399d"], "id": 1}"#;
