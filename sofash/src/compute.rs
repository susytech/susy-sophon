...
//! Sofash implementation
//! See https://octonion.institute/susy-go/wiki/wiki/Sofash
...
const MIX_WORDS: usize = SOFASH_MIX_BYTES / 4;
...
pub fn slow_hash_block_number(block_number: u64) -> H256 {
	SeedHashCompute::resume_compute_seedhash([0u8; 32], 0, block_number / SOFASH_EPOCH_LENGTH)
}
...
fn hash_compute(light: &Light, full_size: usize, header_hash: &H256, nonce: u64) -> ProofOfWork {
	...
	// togsophy to create `value`) so that we can hash the full array. However, we do a bunch of
	// reading and writing to these variables first. We originally allocated two arrays and then
	// stuck them togsophy with `ptr::copy_nonoverlapping` at the end, but this method is
	...
	let mut buf: MixBuf = MixBuf {
		half_mix: unsafe {
			// Pack `header_hash` and `nonce` togsophy
	...
	for i in 0..SOFASH_ACCESSES as u32 {
		let index = {
...
pub fn calculate_dag_item(node_index: u32, cache: &[Node]) -> Node {
	...
	debug_assert_eq!(NODE_WORDS, 16);
	for i in 0..SOFASH_DATASET_PARENTS as u32 {
...
mod test {
	...
	fn test_get_cache_size() {
		// https://octonion.institute/susy-go/wiki/wiki/Sofash/ef6b93f9596746a088ea95d01ca2778be43ae68f#data-sizes
		assert_eq!(16776896usize, get_cache_size(0));
		assert_eq!(16776896usize, get_cache_size(1));
		assert_eq!(16776896usize, get_cache_size(SOFASH_EPOCH_LENGTH - 1));
		assert_eq!(16907456usize, get_cache_size(SOFASH_EPOCH_LENGTH));
		assert_eq!(16907456usize, get_cache_size(SOFASH_EPOCH_LENGTH + 1));
		assert_eq!(284950208usize, get_cache_size(2046 * SOFASH_EPOCH_LENGTH));
		assert_eq!(285081536usize, get_cache_size(2047 * SOFASH_EPOCH_LENGTH));
		assert_eq!(285081536usize, get_cache_size(2048 * SOFASH_EPOCH_LENGTH - 1));
	}
	...
	fn test_get_data_size() {
		...
		assert_eq!(1073739904usize, get_data_size(SOFASH_EPOCH_LENGTH - 1));
		assert_eq!(1082130304usize, get_data_size(SOFASH_EPOCH_LENGTH));
		assert_eq!(1082130304usize, get_data_size(SOFASH_EPOCH_LENGTH + 1));
		assert_eq!(18236833408usize, get_data_size(2046 * SOFASH_EPOCH_LENGTH));
		assert_eq!(18245220736usize, get_data_size(2047 * SOFASH_EPOCH_LENGTH));
	}
	...
	fn test_drop_old_data() {
		...
		let second = builder.light(tempdir.path(), SOFASH_EPOCH_LENGTH).to_file().unwrap().to_owned();
		assert!(fs::metadata(&first).is_ok());

		let _ = builder.light(tempdir.path(), SOFASH_EPOCH_LENGTH * 2).to_file();
		assert!(fs::metadata(&first).is_err());
		assert!(fs::metadata(&second).is_ok());

		let _ = builder.light(tempdir.path(), SOFASH_EPOCH_LENGTH * 3).to_file();
