...
use compute::Light;
use sophon_types::{U256, U512};
use keccak::H256;
use parking_lot::Mutex;
pub use seed_compute::SeedHashCompute;
pub use shared::SOFASH_EPOCH_LENGTH;
use std::mem;
...
impl SofashManager {
	/// Create a new new instance of sofash manager
	pub fn new<T: Into<Option<OptimizeFor>>>(cache_dir: &Path, optimize_for: T, progpow_transition: u64) -> SofashManager {
		SofashManager {
	...
	pub fn compute_light(&self, block_number: u64, header_hash: &H256, nonce: u64) -> ProofOfWork {
		let epoch = block_number / SOFASH_EPOCH_LENGTH;
		let light = {
...
/// Convert an Sofash boundary to its original difficulty. Basically just `f(x) = 2^256 / x`.
pub fn boundary_to_difficulty(boundary: &sophon_types::H256) -> U256 {
	difficulty_to_boundary_aux(&**boundary)
}

/// Convert an Sofash difficulty to the target boundary. Basically just `f(x) = 2^256 / x`.
pub fn difficulty_to_boundary(difficulty: &U256) -> sophon_types::H256 {
	difficulty_to_boundary_aux(difficulty).into()
}

fn difficulty_to_boundary_aux<T: Into<U512>>(difficulty: T) -> sophon_types::U256 {
...
fn test_lru() {
	...
	let sofash = SofashManager::new(tempdir.path(), None, u64::max_value());
	let hash = [0u8; 32];
	sofash.compute_light(1, &hash, 1);
	sofash.compute_light(50000, &hash, 1);
	assert_eq!(sofash.cache.lock().recent_epoch.unwrap(), 1);
	assert_eq!(sofash.cache.lock().prev_epoch.unwrap(), 0);
	sofash.compute_light(1, &hash, 1);
	assert_eq!(sofash.cache.lock().recent_epoch.unwrap(), 0);
	assert_eq!(sofash.cache.lock().prev_epoch.unwrap(), 1);
	sofash.compute_light(70000, &hash, 1);
	assert_eq!(sofash.cache.lock().recent_epoch.unwrap(), 2);
	assert_eq!(sofash.cache.lock().prev_epoch.unwrap(), 0);
}
...
fn test_difficulty_to_boundary_regression() {
	...
	// https://octonion.institute/susytech/susy-sophon/issues/8397
...
fn test_boundary_to_difficulty_panics_on_zero() {
	boundary_to_difficulty(&sophon_types::H256::from(0));
}
