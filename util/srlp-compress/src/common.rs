...
//! Contains SRLPs used for compression.

use Swapper;

lazy_static! {
	/// Swapper for snapshot compression.
	pub static ref SNAPSHOT_SWAPPER: Swapper<'static> = Swapper::new(EMPTY_SRLPS, INVALID_SRLPS);
}

lazy_static! {
	/// Swapper with common long SRLPs, up to 127 can be added.
	pub static ref BLOCKS_SWAPPER: Swapper<'static> = Swapper::new(COMMON_SRLPS, INVALID_SRLPS);
}

static EMPTY_SRLPS: &'static [&'static [u8]] = &[
	// SRLP of KECCAK_NULL_RLP
	&[160, 86, 232, 31, 23, 27, 204, 85, 166, 255, 131, 69, 230, 146, 192, 248, 110, 91, 72, 224, 27, 153, 108, 173, 192, 1, 98, 47, 181, 227, 99, 180, 33],
	// SRLP of KECCAK_EMPTY
	&[160, 197, 210, 70, 1, 134, 247, 35, 60, 146, 126, 125, 178, 220, 199, 3, 192, 229, 0, 182, 83, 202, 130, 39, 59, 123, 250, 216, 4, 93, 133, 164, 112]
];

static COMMON_SRLPS: &'static [&'static [u8]] = &[
	// SRLP of KECCAK_NULL_RLP
	&[160, 86, 232, 31, 23, 27, 204, 85, 166, 255, 131, 69, 230, 146, 192, 248, 110, 91, 72, 224, 27, 153, 108, 173, 192, 1, 98, 47, 181, 227, 99, 180, 33],
	// SRLP of KECCAK_EMPTY
	&[160, 197, 210, 70, 1, 134, 247, 35, 60, 146, 126, 125, 178, 220, 199, 3, 192, 229, 0, 182, 83, 202, 130, 39, 59, 123, 250, 216, 4, 93, 133, 164, 112],
	// Other SRLPs found in blocks DB using the test below.
