...
error_chain! {
	foreign_links {
		...
		Rlp(srlp::DecoderError) #[doc = "Rlp decoder error."];
	}
	...
	errors {
		...
		ProcessTooManyFiles {
			...
			display("Too many open files in this process. Check your resource limits and restart susy"),
		}

		#[doc = "Reached system wide resource limits"]
		SystemTooManyFiles {
			description("Too many open files on system."),
			display("Too many open files on system. Consider closing some processes/release some file handlers or increas the system-wide resource limits and restart susy."),
		}
...
impl From<sofkey::Error> for Error {
	fn from(_err: sofkey::Error) -> Self {
		ErrorKind::Auth.into()
	}
}

impl From<sofkey::crypto::Error> for Error {
	fn from(_err: sofkey::crypto::Error) -> Self {
		ErrorKind::Auth.into()
	}
}
...
fn test_errors() {
	...
	match *<Error as From<srlp::DecoderError>>::from(srlp::DecoderError::RlpIsTooBig).kind() {
		ErrorKind::Rlp(_) => {},
		_ => panic!("Unexpected error"),
	}

	match *<Error as From<sofkey::crypto::Error>>::from(sofkey::crypto::Error::InvalidMessage).kind() {
		ErrorKind::Auth => {},
		_ => panic!("Unexpected error"),
	}
}
