...
use susykv::{KeyValueDB, DBTransaction, DBValue};
use memory_db::*;
	...
	fn read_refs(db: &KeyValueDB, col: Option<u32>) -> (Option<u64>, HashMap<H256, RefInfo>) {
		...
		if let Some(val) = db.get(col, &LATEST_ERA_KEY).expect("Low-level database error.") {
			...
			loop {
				...
				while let Some(srlp_data) = db.get(col, &encode(&db_key)).expect("Low-level database error.") {
					let inserts = DatabaseValueView::from_srlp(&srlp_data).inserts().expect("srlp read from db; qed");
...
impl JournalDB for EarlyMergeDB {
	...
	fn mark_canonical(&mut self, batch: &mut DBTransaction, end_era: u64, canon_id: &H256) -> io::Result<u32> {
		...
		while let Some(srlp_data) = {
			last = encode(&db_key);
			self.backing.get(self.column, &last)
		}? {
			let view = DatabaseValueView::from_srlp(&srlp_data);
			let inserts = view.inserts().expect("srlp read from db; qed");

			if canon_id == &view.id().expect("srlp read from db; qed") {
				// Collect keys to be removed. Canon block - remove the (enacted) deletes.
				let deletes = view.deletes().expect("srlp read from db; qed");
...
mod tests {
	...
	fn new_db() -> EarlyMergeDB {
		let backing = Arc::new(susykv_memorydb::create(0));
		EarlyMergeDB::new(backing, None)
	}

	#[test]
	fn reopen() {
		let shared_db = Arc::new(susykv_memorydb::create(0));
		let bar = H256::random();
