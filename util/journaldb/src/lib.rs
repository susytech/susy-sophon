...
pub fn new(backing: Arc<::susykv::KeyValueDB>, algorithm: Algorithm, col: Option<u32>) -> Box<JournalDB> {
	match algorithm {
...
const DB_PREFIX_LEN : usize = ::susykv::PREFIX_LEN;
const LATEST_ERA_KEY : [u8; ::susykv::PREFIX_LEN] = [ b'l', b'a', b's', b't', 0, 0, 0, 0, 0, 0, 0, 0 ];

fn error_key_already_exists(hash: &sophon_types::H256) -> io::Error {
	io::Error::new(io::ErrorKind::AlreadyExists, hash.to_string())
}

fn error_negatively_reference_hash(hash: &sophon_types::H256) -> io::Error {
	io::Error::new(io::ErrorKind::Other, format!("Entry {} removed from database more times than it was added.", hash))
}

pub fn new_memory_db() -> memory_db::MemoryDB<keccak_hasher::KeccakHasher, susykv::DBValue> {
	memory_db::MemoryDB::from_null_node(&srlp::NULL_RLP, srlp::NULL_RLP.as_ref().into())
}
