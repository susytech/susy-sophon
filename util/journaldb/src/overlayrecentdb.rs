...
use srlp::{Rlp, RlpStream, encode, decode, DecoderError, Decodable, Encodable};
...
impl Decodable for DatabaseValue {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		let id = srlp.val_at(0)?;
		let inserts = srlp.at(1)?.iter().map(|r| {
			let k = r.val_at(0)?;
			let v = DBValue::from_slice(r.at(1)?.data()?);
			Ok((k, v))
		}).collect::<Result<Vec<_>, _>>()?;
		let deletes = srlp.list_at(2)?;
		...
		if let Some(val) = db.get(col, &LATEST_ERA_KEY).expect("Low-level database error.") {
			...
			loop {
				...
				while let Some(srlp_data) = db.get(col, &encode(&db_key)).expect("Low-level database error.") {
					...
					let value = decode::<DatabaseValue>(&srlp_data).expect(&format!("read_overlay: Error decoding DatabaseValue era={}, index{}", era, db_key.index));
