...
lazy_static! {
	// match polynomial identifier with the addition of '[(\d)*]*'
...
pub struct SIP712 {
	...
	pub(crate) domain: SIP712Domain,
}

impl Validate for SIP712 {
...
mod tests {
	...
	fn test_deserialization() {
		...
		let _ = from_str::<SIP712>(string).unwrap();
	}
	...
	fn test_failing_deserialization() {
		...
		let data = from_str::<SIP712>(string).unwrap();
