...
//! SIP712 Encoder
use sofabi::{encode, Token as SofAbiToken};
use sophon_types::{Address as SofAddress, U256, H256};
...
use crate::sip712::{SIP712, MessageTypes};
...
) -> Result<Vec<u8>>
{
	let encoded = match message_type {
		...
		Type::Custom(ref ident) if message_types.get(&*ident).is_some() => {
			...
			let mut tokens = encode(&[SofAbiToken::FixedBytes(type_hash)]);
		...
		Type::Bytes => {
			...
			encode(&[SofAbiToken::FixedBytes(bytes)])
		}
		...
		Type::String => {
			...
			encode(&[SofAbiToken::FixedBytes(hash)])
		}

		Type::Bool => encode(&[SofAbiToken::Bool(value.as_bool().ok_or_else(|| serde_error("bool", field_name))?)]),
		...
		Type::Address => {
			...
			let address = SofAddress::from_str(&addr[2..]).map_err(|err| ErrorKind::HexParseError(format!("{}", err)))?;
			encode(&[SofAbiToken::Address(address)])
		}
		...
		Type::Uint | Type::Int => {
			...
			let token = if *message_type == Type::Uint {
				SofAbiToken::Uint(uint)
			} else {
				SofAbiToken::Int(uint)
			};
			encode(&[token])
		}
...
/// encodes and hashes the given SIP712 struct
pub fn hash_structured_data(typed_data: SIP712) -> Result<H256> {
	// validate input
	typed_data.validate()?;
	// SIP-191 compliant
	...
	let (domain_hash, data_hash) = (
		encode_data(&parser, &Type::Custom("SIP712Domain".into()), &typed_data.types, &domain, None)?,
...
mod tests {
	...
	#[test]
	fn test_hash_data() {
		let typed_data = from_str::<SIP712>(JSON).expect("alas error!");
		assert_eq!(
	...
	fn test_unequal_array_lengths() {
		...
		let typed_data = from_str::<SIP712>(TEST).expect("alas error!");
		assert_eq!(
