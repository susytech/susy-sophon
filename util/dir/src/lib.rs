...
pub fn default_data_path() -> String {
	...
	get_app_root(AppDataType::UserData, &app_info).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|_| "$HOME/.susy".to_owned())
}

/// Default local path
pub fn default_local_path() -> String {
	let app_info = AppInfo { name: PRODUCT, author: AUTHOR };
	get_app_root(AppDataType::UserCache, &app_info).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|_| "$HOME/.susy".to_owned())
}

/// Default hypervisor path
pub fn default_hypervisor_path() -> PathBuf {
	let app_info = AppInfo { name: PRODUCT_HYPERVISOR, author: AUTHOR };
	get_app_root(AppDataType::UserData, &app_info).unwrap_or_else(|_| "$HOME/.susy-hypervisor".into())
}
...
/// Graviton path
pub fn graviton(testnet: bool) -> PathBuf {
	let mut base = graviton_base();
	if testnet {
		base.push("testnet");
	}
	base.push("keystore");
...
/// Susy path for specific chain
pub fn susy(chain: &str) -> PathBuf {
	let mut base = susy_base();
	base.push(chain);
	base
}
...
mod platform {
	...
	pub const AUTHOR: &str = "Susy";
	pub const PRODUCT: &str = "io.susy.sophon";
	pub const PRODUCT_HYPERVISOR: &str = "io.susy.sophon-updates";

	pub fn susy_base() -> PathBuf {
		let mut home = super::home();
		home.push("Library");
		home.push("Application Support");
		home.push("io.susy.sophon");
		home.push("keys");
		home
	}

	pub fn graviton_base() -> PathBuf {
		let mut home = super::home();
		home.push("Library");
		home.push("Sophon");
		home
...
mod platform {
	use std::path::PathBuf;
	pub const AUTHOR: &str = "Susy";
	pub const PRODUCT: &str = "Sophon";
	pub const PRODUCT_HYPERVISOR: &str = "SophonUpdates";
