...
use hash::keccak;
use sophon_types::{H256, H520};
use srlp::{Rlp, RlpStream};
use node_table::*;
use network::{Error, ErrorKind};
use sofkey::{Secret, KeyPair, sign, recover};
...
struct PingRequest {
	...
	echo_hash: H256,
	// The hash Susy used to respond with (until rev 01f825b0e1f1c4c420197b51fc801cbe89284b29)
	#[deprecated()]
...
impl<'a> Discovery<'a> {
	...
	fn ping(&mut self, node: &NodeEntry, reason: PingReason) -> Result<(), Error> {
		...
		srlp.append(&PROTOCOL_VERSION);
		self.public_endpoint.to_srlp_list(&mut srlp);
		node.endpoint.to_srlp_list(&mut srlp);
		append_expiration(&mut srlp);
		let old_susy_hash = keccak(srlp.as_raw());
		let hash = self.send_packet(PACKET_PING, &node.endpoint.udp_address(), &srlp.drain())?;

		self.in_flight_pings.insert(node.id, PingRequest {
			sent_at: Instant::now(),
			node: node.clone(),
			echo_hash: hash,
			deprecated_echo_hash: old_susy_hash,
			reason: reason
		});
	...
	fn send_find_node(&mut self, node: &NodeEntry, target: &NodeId) -> Result<(), Error> {
		...
		srlp.append(target);
		append_expiration(&mut srlp);
		self.send_packet(PACKET_FIND_NODE, &node.endpoint.udp_address(), &srlp.drain())?;
	...
	pub fn on_packet(&mut self, packet: &[u8], from: SocketAddr) -> Result<Option<TableUpdates>, Error> {
		...
		let srlp = Rlp::new(&signed[1..]);
		match packet_id {
			PACKET_PING => self.on_ping(&srlp, &node_id, &from, &hash_signed),
			PACKET_PONG => self.on_pong(&srlp, &node_id, &from),
			PACKET_FIND_NODE => self.on_find_node(&srlp, &node_id, &from),
			PACKET_NEIGHBOURS => self.on_neighbours(&srlp, &node_id, &from),
			_ => {
	...
	fn on_ping(&mut self, srlp: &Rlp, node_id: &NodeId, from: &SocketAddr, echo_hash: &[u8]) -> Result<Option<TableUpdates>, Error> {
		trace!(target: "discovery", "Got Ping from {:?}", &from);
		let ping_from = NodeEndpoint::from_srlp(&srlp.at(1)?)?;
		let ping_to = NodeEndpoint::from_srlp(&srlp.at(2)?)?;
		let timestamp: u64 = srlp.val_at(3)?;
		...
		// WARNING: this field _should not be used_, but old Susy versions
		// use it in order to get the node's address.
		// So this is a temporary fix so that older Susy versions don't brake completely.
		ping_to.to_srlp_list(&mut response);
		// pong_to.to_srlp_list(&mut response);
	...
	fn on_pong(&mut self, srlp: &Rlp, node_id: &NodeId, from: &SocketAddr) -> Result<Option<TableUpdates>, Error> {
		trace!(target: "discovery", "Got Pong from {:?} ; node_id={:#x}", &from, node_id);
		let _pong_to = NodeEndpoint::from_srlp(&srlp.at(0)?)?;
		let echo_hash: H256 = srlp.val_at(1)?;
		let timestamp: u64 = srlp.val_at(2)?;
		self.check_timestamp(timestamp)?;
				...
				let expected_node = {
					...
					if request.echo_hash != echo_hash && request.deprecated_echo_hash != echo_hash {
						...
						if request.deprecated_echo_hash == echo_hash {
							trace!(target: "discovery", "Got Pong from an old susy-sophon version.");
						}
	...
	fn on_find_node(&mut self, srlp: &Rlp, node_id: &NodeId, from: &SocketAddr) -> Result<Option<TableUpdates>, Error> {
		trace!(target: "discovery", "Got FindNode from {:?}", &from);
		let target: NodeId = srlp.val_at(0)?;
		let timestamp: u64 = srlp.val_at(1)?;
		self.check_timestamp(timestamp)?;
	...
	fn prepare_neighbours_packets(nearest: &[NodeEntry]) -> Vec<Bytes> {
		...
		let packets = chunks.map(|c| {
			let mut srlp = RlpStream::new_list(2);
			srlp.begin_list(c.len());
			for n in c {
				srlp.begin_list(4);
				n.endpoint.to_srlp(&mut srlp);
				srlp.append(&n.id);
			}
			append_expiration(&mut srlp);
			srlp.out()
	...
	fn on_neighbours(&mut self, srlp: &Rlp, node_id: &NodeId, from: &SocketAddr) -> Result<Option<TableUpdates>, Error> {
		let results_count = srlp.at(0)?.item_count()?;
		...
		for r in srlp.at(0)?.iter() {
			let endpoint = NodeEndpoint::from_srlp(&r)?;
...
mod tests {
	...
	fn test_ping() {
		...
		let srlp = Rlp::new(&data[1..]);
		assert_eq!(ep1, NodeEndpoint::from_srlp(&srlp.at(1).unwrap()).unwrap());
		assert_eq!(ep2, NodeEndpoint::from_srlp(&srlp.at(2).unwrap()).unwrap());
		...
		let srlp = Rlp::new(&data[1..]);
		assert_eq!(ping_data.payload[0..32], srlp.val_at::<Vec<u8>>(1).unwrap()[..]);

		// Create a pong packet with incorrect echo hash and assert that it is rejected.
		let mut incorrect_pong_srlp = RlpStream::new_list(3);
		ep1.to_srlp_list(&mut incorrect_pong_srlp);
		incorrect_pong_srlp.append(&H256::default());
		append_expiration(&mut incorrect_pong_srlp);
		let incorrect_pong_data = assemble_packet(
			PACKET_PONG, &incorrect_pong_srlp.drain(), &discovery2.secret
		).unwrap();
		...
		assert_eq!(data[0], PACKET_PING);
		let srlp = Rlp::new(&data[1..]);
		assert_eq!(ep2, NodeEndpoint::from_srlp(&srlp.at(1).unwrap()).unwrap());
		assert_eq!(ep1, NodeEndpoint::from_srlp(&srlp.at(2).unwrap()).unwrap());

		// Deliver an unexpected PONG message to discover1.
		let mut unexpected_pong_srlp = RlpStream::new_list(3);
		ep3.to_srlp_list(&mut unexpected_pong_srlp);
		unexpected_pong_srlp.append(&H256::default());
		append_expiration(&mut unexpected_pong_srlp);
		let unexpected_pong = assemble_packet(
			PACKET_PONG, &unexpected_pong_srlp.drain(), key3.secret()
		).unwrap();
