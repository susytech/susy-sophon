...
use srlp::{Rlp, RlpStream, EMPTY_LIST_RLP};
...
impl Session {
	...
	pub fn send_packet<Message>(&mut self, io: &IoContext<Message>, protocol: Option<[u8; 3]>, packet_id: u8, data: &[u8]) -> Result<(), Error>
		...
		srlp.append_raw(payload, 1);
		self.send(io, &srlp.drain())
	}
	...
	where Message: Send + Sync + Clone {
		...
		match packet_id {
			PACKET_HELLO => {
				let srlp = Rlp::new(&data); //TODO: validate srlp expected size
				self.read_hello(io, &srlp, host)?;
				Ok(SessionData::Ready)
			},
			PACKET_DISCONNECT => {
				let srlp = Rlp::new(&data);
				let reason: u8 = srlp.val_at(0)?;
				if self.had_hello {
	...
	fn write_hello<Message>(&mut self, io: &IoContext<Message>, host: &HostInfo) -> Result<(), Error> where Message: Send + Sync + Clone {
		let mut srlp = RlpStream::new();
		srlp.append_raw(&[PACKET_HELLO as u8], 0);
		srlp.begin_list(5)
	...
	fn read_hello<Message>(&mut self, io: &IoContext<Message>, srlp: &Rlp, host: &HostInfo) -> Result<(), Error>
	where Message: Send + Sync + Clone {
		let protocol = srlp.val_at::<u32>(0)?;
		let client_version_string = srlp.val_at::<String>(1)?;
		let client_version = ClientVersion::from(client_version_string);
		let peer_caps: Vec<PeerCapabilityInfo> = srlp.list_at(2)?;
		let id = srlp.val_at::<NodeId>(4)?;
	...
	pub fn disconnect<Message>(&mut self, io: &IoContext<Message>, reason: DisconnectReason) -> Error where Message: Send + Sync + Clone {
		if let State::Session(_) = self.state {
			let mut srlp = RlpStream::new();
			srlp.begin_list(1);
			srlp.append(&(reason as u32));
			self.send_packet(io, None, PACKET_DISCONNECT, &srlp.drain()).ok();
		}
