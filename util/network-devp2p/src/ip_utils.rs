...
// Based on original work by David Levy https://raw.githubussrcontent.com/dlevy47/rust-interfaces
			...
			match search_gateway_from_timeout(local_ip, Duration::new(5, 0)) {
				...
				Ok(gateway) => {
					match gateway.get_external_ip() {
						...
						Ok(external_addr) => {
							match gateway.add_any_port(PortMappingProtocol::TCP, SocketAddrV4::new(local_ip, local_port), 0, "Susy Node/TCP") {
								Err(ref err) => {
									debug!("Port mapping error: {}", err);
								},
								Ok(tcp_port) => {
									match gateway.add_any_port(PortMappingProtocol::UDP, SocketAddrV4::new(local_ip, local_udp_port), 0, "Susy Node/UDP") {
										Err(ref err) => {
											debug!("Port mapping error: {}", err);
										},
										Ok(udp_port) => {
