...
use sofkey::{KeyPair, Public, Secret, recover, sign, Generator, Random};
use sofkey::crypto::{ecdh, ecies};
...
/// `SRLPx` protocol handshake. See https://octonion.institute/susy-go/devp2p/blob/master/srlpx.md#encrypted-handshake
...
impl Handshake {
	...
	pub fn readable<Message>(&mut self, io: &IoContext<Message>, host: &HostInfo) -> Result<(), Error> where Message: Send + Clone + Sync + 'static {
		while let Some(data) = self.connection.readable()? {
			match self.state {
				...
				HandshakeState::ReadingAuthSip8 => {
					self.read_auth_sip8(io, host.secret(), &data)?;
				},
				HandshakeState::ReadingAck => {
					self.read_ack(host.secret(), &data)?;
				},
				HandshakeState::ReadingAckSip8 => {
					self.read_ack_sip8(host.secret(), &data)?;
				},
			}
	...
	fn read_auth<Message>(&mut self, io: &IoContext<Message>, secret: &Secret, data: &[u8]) -> Result<(), Error> where Message: Send + Clone + Sync + 'static {
		...
		match ecies::decrypt(secret, &[], data) {
			...
			Err(_) => {
				// Try to interpret as SIP-8 packet
				let total = ((u16::from(data[0]) << 8 | (u16::from(data[1]))) as usize) + 2;
				if total < V4_AUTH_PACKET_SIZE {
					debug!(target: "network", "Wrong SIP8 auth packet size");
					return Err(ErrorKind::BadProtocol.into());
				}
				let rest = total - data.len();
				self.state = HandshakeState::ReadingAuthSip8;
	...
	fn read_auth_sip8<Message>(&mut self, io: &IoContext<Message>, secret: &Secret, data: &[u8]) -> Result<(), Error> where Message: Send + Clone + Sync + 'static {
		trace!(target: "network", "Received SIP8 handshake auth from {:?}", self.connection.remote_addr_str());
		self.auth_cipher.extend_from_slice(data);
		let auth = ecies::decrypt(secret, &self.auth_cipher[0..2], &self.auth_cipher[2..])?;
		let srlp = Rlp::new(&auth);
		let signature: H520 = srlp.val_at(0)?;
		let remote_public: Public = srlp.val_at(1)?;
		let remote_nonce: H256 = srlp.val_at(2)?;
		let remote_version: u64 = srlp.val_at(3)?;
		self.set_auth(secret, &signature, &remote_public, &remote_nonce, remote_version)?;
		self.write_ack_sip8(io)?;
		Ok(())
	}
		...
		match ecies::decrypt(secret, &[], data) {
			...
			Err(_) => {
				...
				if total < V4_ACK_PACKET_SIZE {
					debug!(target: "network", "Wrong SIP8 ack packet size");
					return Err(ErrorKind::BadProtocol.into());
				}
				let rest = total - data.len();
				self.state = HandshakeState::ReadingAckSip8;
				self.connection.expect(rest);
			}
		}
		Ok(())
	...
	fn read_ack_sip8(&mut self, secret: &Secret, data: &[u8]) -> Result<(), Error> {
		trace!(target: "network", "Received SIP8 handshake auth from {:?}", self.connection.remote_addr_str());
		self.ack_cipher.extend_from_slice(data);
		let ack = ecies::decrypt(secret, &self.ack_cipher[0..2], &self.ack_cipher[2..])?;
		let srlp = Rlp::new(&ack);
		self.remote_ephemeral = srlp.val_at(0)?;
		self.remote_nonce = srlp.val_at(1)?;
		self.remote_version = srlp.val_at(2)?;
		self.state = HandshakeState::StartSession;
		Ok(())
	}
	...
	/// Sends SIP8 ack message
	fn write_ack_sip8<Message>(&mut self, io: &IoContext<Message>) -> Result<(), Error> where Message: Send + Clone + Sync + 'static {
		trace!(target: "network", "Sending SIP8 handshake ack to {:?}", self.connection.remote_addr_str());
		let mut srlp = RlpStream::new_list(3);
		srlp.append(self.ecdhe.public());
		srlp.append(&self.nonce);
		srlp.append(&PROTOCOL_VERSION);

		let pad_array = [0u8; 200];
		let pad = &pad_array[0 .. 100 + random::<usize>() % 100];
		srlp.append_raw(pad, 0);

		let encoded = srlp.drain();
...
mod test {
	...
	fn test_handshake_auth_sip8() {
		...
		assert_eq!(h.state, super::HandshakeState::ReadingAuthSip8);
		h.read_auth_sip8(&test_io(), &secret, &auth[super::V4_AUTH_PACKET_SIZE..]).unwrap();
		assert_eq!(h.state, super::HandshakeState::StartSession);
		check_auth(&h, 4);
	}

	#[test]
	fn test_handshake_auth_sip8_2() {
		let mut h = create_handshake(None);
		let secret = "b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291".parse().unwrap();
		let auth =
			"\
	...
	fn test_handshake_ack_sip8() {
		...
		assert_eq!(h.state, super::HandshakeState::ReadingAckSip8);
		h.read_ack_sip8(&secret, &ack[super::V4_ACK_PACKET_SIZE..]).unwrap();
		assert_eq!(h.state, super::HandshakeState::StartSession);
		check_ack(&h, 4);
	}

	#[test]
	fn test_handshake_ack_sip8_2() {
		let remote = "fda1cff674c90c9a197539fe3dfb53086ace64f83ed7c6eabec741f7f381cc803e52ab2cd55d5569bce4347107a310dfd5f88a010cd2ffd1005ca406f1842877".into();
		let mut h = create_handshake(Some(&remote));
		let secret = "49a7b37aa6f6645917e7b807e9d1c00d4fa71f18343b0d4122a4d2df64dd6fee".parse().unwrap();
		let ack =
