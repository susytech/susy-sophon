...
use mio::tcp::*;
use sophon_types::{H128, H256, H512};
use susy_bytes::*;
...
/// Connection implementing `SRLPx` framing
/// https://octonion.institute/susy-go/devp2p/blob/master/srlpx.md#framing
...
impl EncryptedConnection {
	...
	fn read_header(&mut self, header: &[u8]) -> Result<(), Error> {
		...
		let header_srlp = Rlp::new(&hdec[3..6]);
		let protocol_id = header_srlp.val_at::<u16>(0)?;
