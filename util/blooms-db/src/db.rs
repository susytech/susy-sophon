...
struct DatabaseFiles {
	...
	///
	/// Every bloom is an sophon header bloom
	bot: File,
}
...
impl DatabaseFiles {
	...
	pub fn accrue_bloom(&mut self, pos: Positions, bloom: sofbloom::BloomRef) -> io::Result<()> {
		self.top.accrue_bloom::<sofbloom::BloomRef>(pos.top, bloom)?;
		self.mid.accrue_bloom::<sofbloom::BloomRef>(pos.mid, bloom)?;
		self.bot.replace_bloom::<sofbloom::BloomRef>(pos.bot, bloom)?;
		Ok(())
	}
...
impl Database {
	...
	where sofbloom::BloomRef<'a>: From<B>, I: Iterator<Item = B> {
		match self.db_files {
	...
	where sofbloom::BloomRef<'b>: From<B>, 'b: 'a, II: IntoIterator<Item = B, IntoIter = I> + Copy, I: Iterator<Item = B> {
		match self.db_files {
			Some(ref mut db_files) => {
...
fn contains_any<'a, I, B>(bloom: sofbloom::Bloom, mut iterator: I) -> bool
where sofbloom::BloomRef<'a>: From<B>, I: Iterator<Item = B> {
