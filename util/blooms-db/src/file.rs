...
impl File {
	...
	pub fn read_bloom(&self, pos: u64) -> io::Result<sofbloom::Bloom> {
		let mut file_ref = &self.file;
		file_ref.seek(SeekFrom::Start(pos * 256))?;
		let mut bloom = sofbloom::Bloom::default();
		file_ref.read_exact(&mut bloom)?;
		Ok(bloom)
	}

	/// Accrue bloom into bloom at given position.
	pub fn accrue_bloom<'a, B>(&mut self, pos: u64, bloom: B) -> io::Result<()> where sofbloom::BloomRef<'a>: From<B> {
		self.ensure_space_for_write(pos)?;
		let mut old_bloom: sofbloom::Bloom = self.read_bloom(pos)?;
	...
	pub fn replace_bloom<'a, B>(&mut self, pos: u64, bloom: B) -> io::Result<()> where sofbloom::BloomRef<'a>: From<B> {
		self.ensure_space_for_write(pos)?;
		let mut file_ref = &self.file;
		file_ref.seek(SeekFrom::Start(pos * 256))?;
		file_ref.write_all(sofbloom::BloomRef::from(bloom).data())
	}
...
impl<'a> Iterator for FileIterator<'a> {
	type Item = io::Result<sofbloom::Bloom>;
