...
use sofcore_logger::setup_log;
use susy_sophon::{start, ExecutionAction};
use susy_daemonize::AsHandle;
use parking_lot::{Condvar, Mutex};

const PLEASE_RESTART_EXIT_CODE: i32 = 69;
const SUSY_EXECUTABLE_NAME: &str = "susy";
...
#[cfg(windows)]
fn global_cleanup() {
	// We need to cleanup all sockets before spawning another Susy process. This makes sure everything is cleaned up.
...
// Starts susy binary installed via `susy-updater` and returns the code it exits with.
fn run_susy() -> Result<(), Error> {
	global_init();
...
// Run `locally installed version` of susy (i.e, not installed via `susy-updater`)
...
fn main_direct(force_can_restart: bool) -> i32 {
	...
	let mut conf = {
		...
		susy_sophon::Configuration::parse_cli(&args).unwrap_or_else(|e| e.exit())
	};
	...
	// see: `https://octonion.institute/susytech/susy-daemonize/pull/13` for more info
	let handle = if let Some(pid) = conf.args.arg_daemon_pid_file.clone() {
		...
		match susy_daemonize::daemonize(pid) {
			Ok(h) => Some(h),
			Err(e) => {
				error!(
					"{}",
...
fn main() {
	...
	// the user has specified to run its originally installed binary (not via `susy-updater`)
	...
	// the binary is named `susy`
	let same_name = exe_path
		.as_ref()
		.map_or(false, |p| {
			p.file_stem().map_or(false, |n| n == SUSY_EXECUTABLE_NAME)
		});
	...
	if !force_direct && !development && same_name {
		// Try to run the latest installed version of `susy`,
		// Upon failure it falls back to the locally installed version of `susy`
