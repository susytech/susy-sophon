...
usage! {
	...
	}
	{
		...
		["Operating Options"]
			FLAG flag_no_download: (bool) = false, or |c: &Config| c.susy.as_ref()?.no_download.clone(),
			"--no-download",
			"Normally new releases will be downloaded ready for updating. This disables it. Not recommended.",

			FLAG flag_no_consensus: (bool) = false, or |c: &Config| c.susy.as_ref()?.no_consensus.clone(),
			"--no-consensus",
			"Force the binary to run even if there are known issues regarding consensus. Not recommended.",

			FLAG flag_light: (bool) = false, or |c: &Config| c.susy.as_ref()?.light,
			"--light",
			...
			FLAG flag_no_hardcoded_sync: (bool) = false, or |c: &Config| c.susy.as_ref()?.no_hardcoded_sync,
			"--no-hardcoded-sync",
			...
			"--force-direct",
			"Run the originally installed version of Susy, ignoring any updates that have since been installed.",

			ARG arg_mode: (String) = "last", or |c: &Config| c.susy.as_ref()?.mode.clone(),
			"--mode=[MODE]",
			"Set the operating mode. MODE can be one of: last - Uses the last-used mode, active if none; active - Susy continuously syncs the chain; passive - Susy syncs initially, then sleeps and wakes regularly to resync; dark - Susy syncs only when the JSON-RPC is active; offline - Susy doesn't sync.",

			ARG arg_mode_timeout: (u64) = 300u64, or |c: &Config| c.susy.as_ref()?.mode_timeout.clone(),
			"--mode-timeout=[SECS]",
			"Specify the number of seconds before inactivity timeout occurs when mode is dark or passive",

			ARG arg_mode_alarm: (u64) = 3600u64, or |c: &Config| c.susy.as_ref()?.mode_alarm.clone(),
			"--mode-alarm=[SECS]",
			"Specify the number of seconds before auto sleep reawake timeout occurs when mode is passive",

			ARG arg_auto_update: (String) = "critical", or |c: &Config| c.susy.as_ref()?.auto_update.clone(),
			"--auto-update=[SET]",
			...
			ARG arg_auto_update_delay: (u16) = 100u16, or |c: &Config| c.susy.as_ref()?.auto_update_delay.clone(),
			"--auto-update-delay=[NUM]",
			"Specify the maximum number of blocks used for randomly delaying updates.",

			ARG arg_auto_update_check_frequency: (u16) = 20u16, or |c: &Config| c.susy.as_ref()?.auto_update_check_frequency.clone(),
			"--auto-update-check-frequency=[NUM]",
			"Specify the number of blocks between each auto-update check.",

			ARG arg_release_track: (String) = "current", or |c: &Config| c.susy.as_ref()?.release_track.clone(),
			"--release-track=[TRACK]",
			...
			ARG arg_chain: (String) = "foundation", or |c: &Config| c.susy.as_ref()?.chain.clone(),
			"--chain=[CHAIN]",
			"Specify the blockchain type. CHAIN may be either a JSON chain specification file or sophon, classic, poacore, tobalaba, expanse, musicoin, ellaism, mix, callisto, morden, ropsten, kovan, poasokol, testnet, or dev.",

			ARG arg_keys_path: (String) = "$BASE/keys", or |c: &Config| c.susy.as_ref()?.keys_path.clone(),
			"--keys-path=[PATH]",
			"Specify the path for JSON key files to be found",

			ARG arg_identity: (String) = "", or |c: &Config| c.susy.as_ref()?.identity.clone(),
			"--identity=[NAME]",
			"Specify your node's name.",

			ARG arg_base_path: (Option<String>) = None, or |c: &Config| c.susy.as_ref()?.base_path.clone(),
			"-d, --base-path=[PATH]",
			"Specify the base data storage path.",

			ARG arg_db_path: (Option<String>) = None, or |c: &Config| c.susy.as_ref()?.db_path.clone(),
			"--db-path=[PATH]",
		...
		["Convenience Options"]
			...
			"--ports-shift=[SHIFT]",
			"Add SHIFT to all port numbers Susy is listening on. Includes network port and all servers (HTTP JSON-RPC, WebSockets JSON-RPC, IPFS, SecretStore).",
		...
		["Networking Options"]
			...
			"--interface=[IP]",
			"Network interfaces. Valid values are 'all', 'local' or the ip of the interface you want susy to listen to.",
		...
		["API and Console Options – HTTP JSON-RPC"]
			FLAG flag_susy_jsonrpc_allow_missing_blocks: (bool) = false, or |c: &Config| c.rpc.as_ref()?.allow_missing_blocks.clone(),
			"--susy-jsonrpc-allow-missing-blocks",
			...
			FLAG flag_susy_jsonrpc_no_keep_alive: (bool) = false, or |c: &Config| c.rpc.as_ref()?.keep_alive,
			"--susy-jsonrpc-no-keep-alive",
			...
			FLAG flag_susy_jsonrpc_experimental: (bool) = false, or |c: &Config| c.rpc.as_ref()?.experimental_rpcs.clone(),
			"--susy-jsonrpc-experimental",
			"Enable experimental RPCs. Enable to have access to methods from unfinalised SIPs in all namespaces",

			ARG arg_susy_jsonrpc_port: (u16) = 8545u16, or |c: &Config| c.rpc.as_ref()?.port.clone(),
			"--susy-jsonrpc-port=[PORT]",
			"Specify the port portion of the HTTP JSON-RPC API server.",

			ARG arg_susy_jsonrpc_interface: (String) = "local", or |c: &Config| c.rpc.as_ref()?.interface.clone(),
			"--susy-jsonrpc-interface=[IP]",
			...
			ARG arg_susy_jsonrpc_apis: (String) = "susyweb,sof,pubsub,net,susy,private,susy_pubsub,traces,rpc,shh,shh_pubsub", or |c: &Config| c.rpc.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--susy-jsonrpc-apis=[APIS]",
			"Specify the APIs available through the HTTP JSON-RPC interface using a comma-delimited list of API names. Possible names are: all, safe, debug, susyweb, net, sof, pubsub, personal, signer, susy, susy_pubsub, susy_accounts, susy_set, traces, rpc, secretstore, shh, shh_pubsub. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: susyweb, net, sof, pubsub, susy, susy_pubsub, traces, rpc, shh, shh_pubsub",

			ARG arg_susy_jsonrpc_hosts: (String) = "none", or |c: &Config| c.rpc.as_ref()?.hosts.as_ref().map(|vec| vec.join(",")),
			"--susy-jsonrpc-hosts=[HOSTS]",
			"List of allowed Host header values. This option will validate the Host header sent by the browser, it is additional security against some attack vectors. Special options: \"all\", \"none\",.",

			ARG arg_susy_jsonrpc_threads: (usize) = 4usize, or |c: &Config| c.rpc.as_ref()?.processing_threads,
			"--susy-jsonrpc-threads=[THREADS]",
			...
			ARG arg_susy_jsonrpc_cors: (String) = "none", or |c: &Config| c.rpc.as_ref()?.cors.as_ref().map(|vec| vec.join(",")),
			"--susy-jsonrpc-cors=[URL]",
			"Specify CORS header for HTTP JSON-RPC API responses. Special options: \"all\", \"none\".",

			ARG arg_susy_jsonrpc_server_threads: (Option<usize>) = None, or |c: &Config| c.rpc.as_ref()?.server_threads,
			"--susy-jsonrpc-server-threads=[NUM]",
			"Enables multiple threads handling incoming connections for HTTP JSON-RPC server.",

			ARG arg_susy_jsonrpc_max_payload: (Option<usize>) = None, or |c: &Config| c.rpc.as_ref()?.max_payload,
			"--susy-jsonrpc-max-payload=[MB]",
		...
		["API and Console Options – WebSockets"]
			...
			ARG arg_ws_apis: (String) = "susyweb,sof,pubsub,net,susy,susy_pubsub,private,traces,rpc,shh,shh_pubsub", or |c: &Config| c.websockets.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--ws-apis=[APIS]",
			"Specify the JSON-RPC APIs available through the WebSockets interface using a comma-delimited list of API names. Possible names are: all, safe, susyweb, net, sof, pubsub, personal, signer, susy, susy_pubsub, susy_accounts, susy_set, traces, rpc, secretstore, shh, shh_pubsub. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: susyweb, net, sof, pubsub, susy, susy_pubsub, traces, rpc, shh, shh_pubsub",

			ARG arg_ws_origins: (String) = "susy://*,chrome-extension://*,moz-extension://*", or |c: &Config| c.websockets.as_ref()?.origins.as_ref().map(|vec| vec.join(",")),
			"--ws-origins=[URL]",
		...
		["API and Console Options – IPC"]
			...
			ARG arg_ipc_apis: (String) = "susyweb,sof,pubsub,net,susy,susy_pubsub,susy_accounts,private,traces,rpc,shh,shh_pubsub", or |c: &Config| c.ipc.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--ipc-apis=[APIS]",
			"Specify custom API set available via JSON-RPC over IPC using a comma-delimited list of API names. Possible names are: all, safe, susyweb, net, sof, pubsub, personal, signer, susy, susy_pubsub, susy_accounts, susy_set, traces, rpc, secretstore, shh, shh_pubsub. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: susyweb, net, sof, pubsub, susy, susy_pubsub, traces, rpc, shh, shh_pubsub",
		...
		["Sealing/Mining Options"]
			...
			FLAG flag_no_persistent_txqueue: (bool) = false, or |c: &Config| c.susy.as_ref()?.no_persistent_txqueue,
			...
			ARG arg_usd_per_sof: (String) = "auto", or |c: &Config| c.mining.as_ref()?.usd_per_sof.clone(),
			"--usd-per-sof=[SOURCE]",
			"USD value of a single SOF. SOURCE may be either an amount in USD, a web service or 'auto' to use each web service in turn and fallback on the last known good value.",
		...
		["Legacy Options"]
			...
			FLAG flag_graviton: (bool) = false, or |_| None,
			"--graviton",
			"Run in Graviton-compatibility mode. Sets the IPC path to be the same as Graviton's. Overrides the --ipc-path and --ipcpath options. Alters RPCs to reflect Graviton bugs. Includes the personal_ RPC by default.",

			FLAG flag_import_graviton_keys: (bool) = false, or |_| None,
			"--import-graviton-keys",
			"Attempt to import keys from Graviton client.",
			...
			FLAG flag_susy_jsonrpc_off: (bool) = false, or |_| None,
			"--susy-jsonrpc-off",
			...
			"--dapps-apis-all",
			"Dapps server is merged with HTTP JSON-RPC server. Use --susy-jsonrpc-apis.",

			// FLAG Removed in 1.11.

			FLAG flag_public_node: (bool) = false, or |_| None,
			"--public-node",
			"Does nothing; Public node is removed from Susy.",
			...
			ARG arg_sophybase: (Option<String>) = None, or |_| None,
			"--sophybase=[ADDRESS]",
			...
			"--rpcaddr=[IP]",
			"Equivalent to --susy-jsonrpc-interface IP.",

			ARG arg_rpcport: (Option<u16>) = None, or |_| None,
			"--rpcport=[PORT]",
			"Equivalent to --susy-jsonrpc-port PORT.",

			ARG arg_rpcapi: (Option<String>) = None, or |_| None,
			"--rpcapi=[APIS]",
			"Equivalent to --susy-jsonrpc-apis APIS.",

			ARG arg_rpccorsdomain: (Option<String>) = None, or |_| None,
			"--rpccorsdomain=[URL]",
			"Equivalent to --susy-jsonrpc-cors URL.",
...
mod tests {
	...
	fn should_accept_any_argument_order() {
		let args = Args::parse(&["susy", "--no-warp", "account", "list"]).unwrap();
		assert_eq!(args.flag_no_warp, true);

		let args = Args::parse(&["susy", "account", "list", "--no-warp"]).unwrap();
		assert_eq!(args.flag_no_warp, true);

		let args = Args::parse(&["susy", "--chain=dev", "account", "list"]).unwrap();
		assert_eq!(args.arg_chain, "dev");

		let args = Args::parse(&["susy", "account", "list", "--chain=dev"]).unwrap();
		assert_eq!(args.arg_chain, "dev");
	}

	#[test]
	fn should_not_crash_on_warp() {
		let args = Args::parse(&["susy", "--warp"]);
		assert!(args.is_ok());

		let args = Args::parse(&["susy", "account", "list", "--warp"]);
		assert!(args.is_ok());
	}

	#[test]
	fn should_reject_invalid_values() {
		let args = Args::parse(&["susy", "--cache=20"]);
		assert!(args.is_ok());

		let args = Args::parse(&["susy", "--cache=asd"]);
		assert!(args.is_err());
	}

	#[test]
	fn should_parse_args_and_flags() {
		let args = Args::parse(&["susy", "--no-warp"]).unwrap();
		assert_eq!(args.flag_no_warp, true);

		let args = Args::parse(&["susy", "--pruning", "archive"]).unwrap();
		assert_eq!(args.arg_pruning, "archive");

		let args = Args::parse(&["susy", "export", "state", "--no-storage"]).unwrap();
		assert_eq!(args.flag_export_state_no_storage, true);

		let args = Args::parse(&["susy", "export", "state", "--min-balance","123"]).unwrap();
		assert_eq!(args.arg_export_state_min_balance, Some("123".to_string()));
	}

	#[test]
	fn should_exit_gracefully_on_unknown_argument() {
		let result = Args::parse(&["susy", "--please-exit-gracefully"]);
		assert!(
			match result {
	...
	fn should_use_subcommand_arg_default() {
		let args = Args::parse(&["susy", "export", "state", "--at", "123"]).unwrap();
		assert_eq!(args.arg_export_state_at, "123");
		assert_eq!(args.arg_snapshot_at, "latest");

		let args = Args::parse(&["susy", "snapshot", "--at", "123", "file.dump"]).unwrap();
		assert_eq!(args.arg_snapshot_at, "123");
		assert_eq!(args.arg_export_state_at, "latest");

		let args = Args::parse(&["susy", "export", "state"]).unwrap();
		assert_eq!(args.arg_snapshot_at, "latest");
		assert_eq!(args.arg_export_state_at, "latest");

		let args = Args::parse(&["susy", "snapshot", "file.dump"]).unwrap();
		assert_eq!(args.arg_snapshot_at, "latest");
		assert_eq!(args.arg_export_state_at, "latest");
	}
	...
	fn should_parse_multiple_values() {
		let args = Args::parse(&["susy", "account", "import", "~/1", "~/2"]).unwrap();
		assert_eq!(args.arg_account_import_path, Some(vec!["~/1".to_owned(), "~/2".to_owned()]));

		let args = Args::parse(&["susy", "account", "import", "~/1,ext"]).unwrap();
		assert_eq!(args.arg_account_import_path, Some(vec!["~/1,ext".to_owned()]));

		let args = Args::parse(&["susy", "--secretstore-nodes", "abc@127.0.0.1:3333,cde@10.10.10.10:4444"]).unwrap();
		assert_eq!(args.arg_secretstore_nodes, "abc@127.0.0.1:3333,cde@10.10.10.10:4444");

		let args = Args::parse(&["susy", "--password", "~/.safe/1", "--password", "~/.safe/2", "--ui-port", "8123"]).unwrap();
		assert_eq!(args.arg_password, vec!["~/.safe/1".to_owned(), "~/.safe/2".to_owned()]);
		assert_eq!(args.arg_ui_port, Some(8123));

		let args = Args::parse(&["susy", "--password", "~/.safe/1,~/.safe/2", "--ui-port", "8123"]).unwrap();
		assert_eq!(args.arg_password, vec!["~/.safe/1".to_owned(), "~/.safe/2".to_owned()]);
		assert_eq!(args.arg_ui_port, Some(8123));
	}
	...
	fn should_parse_args_and_include_config() {
		...
		config.susy = Some(operating);

		// when
		let args = Args::parse_with_config(&["susy"], config).unwrap();
	...
	fn should_not_use_config_if_cli_is_provided() {
		...
		// when
		let args = Args::parse_with_config(&["susy", "--chain", "xyz"], config).unwrap();
	...
	fn should_parse_full_config() {
		...
		assert_eq!(args, Args {
			...
			arg_chain: "xyz".into(),
			arg_base_path: Some("$HOME/.susy".into()),
			arg_db_path: Some("$HOME/.susy/chains".into()),
			arg_keys_path: "$HOME/.susy/keys".into(),
			arg_identity: "".into(),
			flag_light: false,
			...
			arg_ui_hosts: None,
			arg_ui_path: "$HOME/.susy/signer".into(),
			...
			flag_susy_jsonrpc_no_keep_alive: false,
			flag_susy_jsonrpc_experimental: false,
			arg_susy_jsonrpc_port: 8545u16,
			arg_susy_jsonrpc_interface: "local".into(),
			arg_susy_jsonrpc_cors: "null".into(),
			arg_susy_jsonrpc_apis: "susyweb,sof,net,susy,traces,rpc,secretstore".into(),
			arg_susy_jsonrpc_hosts: "none".into(),
			arg_susy_jsonrpc_server_threads: None,
			arg_susy_jsonrpc_threads: 4,
			arg_susy_jsonrpc_max_payload: None,
			arg_poll_lifetime: 60u32,
			flag_susy_jsonrpc_allow_missing_blocks: false,

			// WS
			flag_no_ws: false,
			arg_ws_port: 8546u16,
			arg_ws_interface: "local".into(),
			arg_ws_apis: "susyweb,sof,net,susy,traces,rpc,secretstore".into(),
			...
			// IPC
			flag_no_ipc: false,
			arg_ipc_path: "$HOME/.susy/jsonrpc.ipc".into(),
			arg_ipc_apis: "susyweb,sof,net,susy,susy_accounts,personal,traces,rpc,secretstore".into(),

			// DAPPS
			arg_dapps_path: Some("$HOME/.susy/dapps".into()),
			flag_no_dapps: false,
			...
			arg_secretstore_path: "$HOME/.susy/secretstore".into(),
			...
			flag_graviton: false,
			flag_testnet: false,
			flag_import_graviton_keys: false,
			...
			arg_log_file: Some("/var/log/susy.log".into()),
			flag_no_color: false,
	...
	fn should_deserialize_toml_file() {
		...
		assert_eq!(config, Config {
			...
			ipc: Some(Ipc {
				disable: None,
				path: None,
				apis: Some(vec!["rpc".into(), "sof".into()]),
			}),
			dapps: Some(Dapps {
			...
			misc: Some(Misc {
				...
				log_file: Some("/var/log/susy.log".into()),
				color: Some(true),
				ports_shift: Some(0),
	...
	fn should_not_accept_min_peers_bigger_than_max_peers() {
		match Args::parse(&["susy", "--max-peers=39", "--min-peers=40"]) {
	...
	fn should_accept_max_peers_equal_or_bigger_than_min_peers() {
		Args::parse(&["susy", "--max-peers=40", "--min-peers=40"]).unwrap();
		Args::parse(&["susy", "--max-peers=100", "--min-peers=40"]).unwrap();
	}
}
