...
use sofkey::{Secret, Public, Password};
...
mod server {
	use std::sync::Arc;
	use sofcore_secretstore;
	use sofkey::KeyPair;
	use ansi_term::Colour::{Red, White};
	use db;
	use super::{Configuration, Dependencies, NodeSecretKey, ContractAddress, Executor};

	fn into_service_contract_address(address: ContractAddress) -> sofcore_secretstore::ContractAddress {
		match address {
			ContractAddress::Registry => sofcore_secretstore::ContractAddress::Registry,
			ContractAddress::Address(address) => sofcore_secretstore::ContractAddress::Address(address),
		}
	}

	/// Key server
	pub struct KeyServer {
		_key_server: Box<sofcore_secretstore::KeyServer>,
	}

	impl KeyServer {
		/// Create new key server
		pub fn new(mut conf: Configuration, deps: Dependencies, executor: Executor) -> Result<Self, String> {
			let self_secret: Arc<sofcore_secretstore::NodeKeyPair> = match conf.self_secret.take() {
				Some(NodeSecretKey::Plain(secret)) => Arc::new(sofcore_secretstore::PlainNodeKeyPair::new(
				...
				Some(NodeSecretKey::KeyStore(account)) => {
					...
					Arc::new(sofcore_secretstore::KeyStoreNodeKeyPair::new(deps.account_provider, account, password.clone())
						.map_err(|e| format!("{}", e))?)
				},
			...
			let mut cconf = sofcore_secretstore::ServiceConfiguration {
				listener_address: if conf.http_enabled { Some(sofcore_secretstore::NodeAddress {
				...
				cluster_config: sofcore_secretstore::ClusterConfiguration {
					listener_address: sofcore_secretstore::NodeAddress {
						address: conf.interface.clone(),
						port: conf.port,
					},
					nodes: conf.nodes.into_iter().map(|(p, (ip, port))| (p, sofcore_secretstore::NodeAddress {
						address: ip,
						port: port,
					})).collect(),
			...
			let key_server = sofcore_secretstore::start(deps.client, deps.sync, deps.miner, self_secret, cconf, db, executor)
