...
mod accounts {
	...
	impl ::sofcore::miner::LocalAccounts for AccountProvider {
	...
	pub fn prepare_account_provider(_spec: &SpecType, _dirs: &Directories, _data_dir: &str, _cfg: AccountsConfig, _passwords: &[Password]) -> Result<AccountProvider, String> {
		warn!("Note: Your instance of Susy Sophon is running without account support. Some CLI options are ignored.");
		Ok(AccountProvider)
	}
	...
	pub fn miner_author(_spec: &SpecType, _dirs: &Directories, _account_provider: &Arc<AccountProvider>, _engine_signer: Address, _passwords: &[Password]) -> Result<Option<::sofcore::miner::Author>, String> {
		Ok(None)
	}

	pub fn private_tx_signer(_account_provider: Arc<AccountProvider>, _passwords: &[Password]) -> Result<Arc<::sofcore_private_tx::Signer>, String> {
		Ok(Arc::new(::sofcore_private_tx::DummySigner))
	}
...
mod accounts {
	...
	pub fn prepare_account_provider(spec: &SpecType, dirs: &Directories, data_dir: &str, cfg: AccountsConfig, passwords: &[Password]) -> Result<AccountProvider, String> {
		...
		let sofstore = SofStore::open_with_iterations(dir, cfg.iterations).map_err(|e| format!("Could not open keys directory: {}", e))?;
		if cfg.refresh_time > 0 {
			sofstore.set_refresh_time(::std::time::Duration::from_secs(cfg.refresh_time));
		}
	...
	impl ::sofcore::miner::LocalAccounts for LocalAccounts {
	...
	pub fn miner_author(spec: &SpecType, dirs: &Directories, account_provider: &Arc<AccountProvider>, engine_signer: Address, passwords: &[Password]) -> Result<Option<::sofcore::miner::Author>, String> {
		use sofcore::engines::EngineSigner;
		...
		for password in passwords {
			let signer = susy_rpc::signer::EngineSigner::new(
				account_provider.clone(),
				engine_signer,
				password.clone(),
			);
			if signer.sign(Default::default()).is_ok() {
				author = Some(::sofcore::miner::Author::Sealer(Box::new(signer)));
			}
		}
		if author.is_none() {
	...
	mod private_tx {
		use super::*;
		use sofkey::{Signature, Message};
		use sofcore_private_tx::{Error};
		...
		impl ::sofcore_private_tx::Signer for AccountSigner {
	...
	pub fn private_tx_signer(accounts: Arc<AccountProvider>, passwords: &[Password]) -> Result<Arc<::sofcore_private_tx::Signer>, String> {
	...
	fn insert_dev_account(account_provider: &AccountProvider) {
		let secret: sofkey::Secret = "4d5db4107d237df6a3d58ee5f70ae63d73d7658d4026f2eefd2f204c81682cb7".into();
		let dev_account = sofkey::KeyPair::from_secret(secret.clone()).expect("Valid secret produces valid key;qed");
	...
	fn build_create_account_hint(spec: &SpecType, keys: &str) -> String {
		format!("You can create an account via RPC, UI or `susy account new --chain {} --keys-path {}`.", spec, keys)
	}
}
