...
fn execute_import_light(cmd: ImportBlockchain) -> Result<(), String> {
	...
	let do_import = |bytes: Vec<u8>| {
		...
		let header: ::types::header::Header = ::srlp::Rlp::new(&bytes).val_at(0)
	...
	match format {
		DataFormat::Binary => {
			loop {
				...
				if n == 0 { break; }
				first_read = 0;
				let s = PayloadInfo::from(&bytes).map_err(|e| format!("Invalid SRLP in the file/stream: {:?}", e))?.total();
				bytes.resize(s, 0);
...
fn execute_import(cmd: ImportBlockchain) -> Result<(), String> {
	...
	let do_import = |bytes| {
		let block = Unverified::from_srlp(bytes).map_err(|_| "Invalid block srlp")?;
		while client.queue_info().is_full() { sleep(Duration::from_secs(1)); }
...
fn start_client(
	...
	if !fat_db && require_fat_db {
		return Err("This command requires Susy to be synced with --fat-db on.".to_owned());
	}
