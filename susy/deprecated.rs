...
pub fn find_deprecated(args: &Args) -> Vec<Deprecated> {
	...
	if args.flag_susy_jsonrpc_off {
		result.push(Deprecated::Replaced("--susy-jsonrpc-off", "--no-jsonrpc"));
	}
	...
	if args.arg_sophybase.is_some() {
		result.push(Deprecated::Replaced("--sophybase", "--author"));
	}
	...
	if args.arg_rpcaddr.is_some() {
		result.push(Deprecated::Replaced("--rpcaddr", "--susy-jsonrpc-interface"));
	}

	if args.arg_rpcport.is_some() {
		result.push(Deprecated::Replaced("--rpcport", "--susy-jsonrpc-port"));
	}

	if args.arg_rpcapi.is_some() {
		result.push(Deprecated::Replaced("--rpcapi", "--susy-jsonrpc-api"));
	}

	if args.arg_rpccorsdomain.is_some() {
		result.push(Deprecated::Replaced("--rpccorsdomain", "--susy-jsonrpc-cors"));
	}
	...
	if args.flag_dapps_apis_all {
		result.push(Deprecated::Replaced("--dapps-apis-all", "--susy-jsonrpc-apis"));
	}
	...
	if args.cmd_dapp {
		result.push(Deprecated::Removed("susy dapp"));
	}
...
mod tests {
	...
	fn test_find_deprecated() {
		...
		assert_eq!(find_deprecated(&{
			...
			Deprecated::Replaced("--susy-jsonrpc-off", "--no-jsonrpc"),
			...
			Deprecated::Replaced("--sophybase", "--author"),
			...
			Deprecated::Replaced("--dapps-apis-all", "--susy-jsonrpc-apis"),
