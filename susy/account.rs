...
pub enum AccountCmd {
	...
	ImportFromGraviton(ImportFromGravitonAccounts)
}
...
/// Parameters for graviton accounts' import
#[derive(Debug, PartialEq)]
pub struct ImportFromGravitonAccounts {
...
mod command {
	...
	use sofstore::{SofStore, SecretStore, SecretVaultRef, import_account, import_accounts, read_graviton_accounts};
	...
	pub fn execute(cmd: AccountCmd) -> Result<String, String> {
		match cmd {
			...
			AccountCmd::ImportFromGraviton(import_graviton_cmd) => import_graviton(import_graviton_cmd)
		}
	}
	...
	fn secret_store(dir: Box<RootDiskDirectory>, iterations: Option<NonZeroU32>) -> Result<SofStore, String> {
		match iterations {
			Some(i) => SofStore::open_with_iterations(dir, i),
			_ => SofStore::open(dir)
	...
	fn import_graviton(i: ImportFromGravitonAccounts) -> Result<String, String> {
		use std::io::ErrorKind;
		use sofstore::Error;

		let dir = Box::new(keys_dir(i.to, i.spec)?);
		let secret_store = Box::new(secret_store(dir, None)?);
		let graviton_accounts = read_graviton_accounts(i.testnet);
		match secret_store.import_graviton_accounts(SecretVaultRef::Root, graviton_accounts, i.testnet) {
			Ok(v) => Ok(format!("Successfully imported {} account(s) from graviton.", v.len())),
			Err(Error::Io(ref io_err)) if io_err.kind() == ErrorKind::NotFound => Err("Failed to find graviton keys folder.".into()),
			Err(err) => Err(format!("Import graviton accounts failed. {}", err))
		}
	}
}
