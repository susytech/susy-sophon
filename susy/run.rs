...
use sofcore::client::{BlockId, Client, Mode, DatabaseCompactionProfile, VMType, BlockChainClient, BlockInfo};
use sofcore::miner::{self, stratum, Miner, MinerService, MinerOptions};
use sofcore::snapshot::{self, SnapshotConfiguration};
use sofcore::spec::{SpecParams, OptimizeFor};
use sofcore::verification::queue::VerifierSettings;
use sofcore_logger::{Config as LogConfig, RotatingLogger};
use sofcore_service::ClientService;
...
use susy_version::version;
use sofcore_private_tx::{ProviderConfig, EncryptorConfig, SecretStoreEncryptor};
use params::{
	...
	where Cr: Fn(String) + 'static + Send
{
	...
	let mut config = light_client::Config {
		...
		chain_column: ::sofcore_db::COL_LIGHT_CHAIN,
		verify_full: true,
	...
	*sync_handle.write() = Arc::downgrade(&light_sync);
	...
	let deps_for_rpc_apis = Arc::new(rpc_apis::LightDependencies {
		...
		fetch: fetch,
		graviton_compatibility: cmd.graviton_compatibility,
		...
		Rr: Fn() + 'static + Send
{
	...
	let store = {
		...
		let store = ::local_store::create(db.key_value().clone(), ::sofcore_db::COL_NODE_INFO, node_info);
...
/// Susy client currently executing in background threads.
///
...
enum RunningClientInner {
	Light {
		rpc: susy_jsonrpc_core::MetaIoHandler<Metadata, informant::Middleware<rpc_apis::LightClientNotifier>>,
		informant: Arc<Informant<LightNodeInformantData>>,
		client: Arc<LightClient>,
		keep_alive: Box<Any>,
	},
	Full {
		rpc: susy_jsonrpc_core::MetaIoHandler<Metadata, informant::Middleware<informant::ClientNotifier>>,
