...
use sofcore::snapshot::{RestorationStatus, SnapshotService as SS};
...
use susy_rpc::is_major_importing_or_waiting;
use susy_rpc::informant::RpcStats;
	...
	fn new_blocks(&self, new_blocks: NewBlocks) {
		...
		if ripe {
			if let Some(block) = new_blocks.imported.last().and_then(|h| client.block(BlockId::Hash(*h))) {
				...
				let size = block.srlp().as_raw().len();
