...
use helpers::susy_ipc_path;
use susy_jsonrpc_core::MetaIoHandler;
use susy_runtime::Executor;
use susy_rpc::informant::{RpcStats, Middleware};
use susy_rpc::{self as rpc, Metadata, DomainsValidation};
use rpc_apis::{self, ApiSet};

pub use susy_rpc::{IpcServer, HttpServer, RequestMiddleware};
pub use susy_rpc::ws::Server as WsServer;

pub const DAPPS_DOMAIN: &'static str = "susyweb.site";
	...
	fn default() -> Self {
		IpcConfiguration {
			...
			socket_addr: if cfg!(windows) {
				...
				susy_ipc_path(&data_dir, "$BASE/jsonrpc.ipc", 0)
			},
...
pub fn new_ws<D: rpc_apis::Dependencies>(
	...
	match start_result {
		...
		Err(rpc::ws::Error(rpc::ws::ErrorKind::Io(ref err), _)) if err.kind() == io::ErrorKind::AddrInUse => Err(
			format!("WebSockets address {} is already in use, make sure that another instance of an Sophon client is not running or change the address using the --ws-port and --ws-interface options.", url)
		),
...
pub fn new_http<D: rpc_apis::Dependencies>(
	...
	match start_result {
		...
		Err(ref err) if err.kind() == io::ErrorKind::AddrInUse => Err(
			format!("{} address {} is already in use, make sure that another instance of an Sophon client is not running or change the address using the --{}-port and --{}-interface options.", id, url, options, options)
		),
