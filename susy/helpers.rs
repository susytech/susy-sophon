...
use sophon_types::{U256, clean_0x, Address};
use journaldb::Algorithm;
use sofcore::client::{Mode, BlockId, VMType, DatabaseCompactionProfile, ClientConfig, VerifierType};
use sofcore::miner::{PendingSet, Penalization};
...
fn to_seconds(s: &str) -> Result<u64, String> {
	let bad = |_| {
		format!("{}: Invalid duration given. See susy --help for more information.", s)
	};
...
/// Returns default graviton ipc path.
pub fn graviton_ipc_path(testnet: bool) -> String {
	// Windows path should not be hardcoded here.
	// Instead it should be a part of path::sophon
	if cfg!(windows) {
		return r"\\.\pipe\graviton.ipc".to_owned();
	}

	if testnet {
		path::sophon::with_testnet("graviton.ipc").to_str().unwrap().to_owned()
	} else {
		path::sophon::with_default("graviton.ipc").to_str().unwrap().to_owned()
	}
}

/// Formats and returns susy ipc path.
pub fn susy_ipc_path(base: &str, path: &str, shift: u16) -> String {
	let mut path = path.to_owned();
	if shift != 0 {
		path = path.replace("jsonrpc.ipc", &format!("susy-jsonrpc-{}.ipc", shift));
	}
...
pub fn default_network_config() -> ::sync::NetworkConfiguration {
	...
	NetworkConfiguration {
		...
		client_version: ::susy_version::version(),
	}
}
...
pub fn execute_upgrades(
	...
	match upgrade(&dirs.path) {
		...
		Err(e) => {
			return Err(format!("Error upgrading susy data: {:?}", e));
		},
		_ => {},
	}
...
mod tests {
	...
	use sofcore::client::{Mode, BlockId};
	use sofcore::miner::PendingSet;
	use sofkey::Password;
	use super::{to_duration, to_mode, to_block_id, to_u256, to_pending_set, to_address, to_addresses, to_price, graviton_ipc_path, to_bootnodes, join_set, password_from_file};
...
"#).unwrap();
	...
	#[test]
	#[cfg(windows)]
	fn test_graviton_ipc_path() {
		assert_eq!(graviton_ipc_path(true), r"\\.\pipe\graviton.ipc".to_owned());
		assert_eq!(graviton_ipc_path(false), r"\\.\pipe\graviton.ipc".to_owned());
	}
	...
	fn test_graviton_ipc_path() {
		use path;
		assert_eq!(graviton_ipc_path(true), path::sophon::with_testnet("graviton.ipc").to_str().unwrap().to_owned());
		assert_eq!(graviton_ipc_path(false), path::sophon::with_default("graviton.ipc").to_str().unwrap().to_owned());
	}
