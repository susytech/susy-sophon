...
use sofcore::client::Mode;
use sofcore::sophon;
use sofcore::spec::{Spec, SpecParams};
...
impl str::FromStr for SpecType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let spec = match s {
			"sophon" | "frontier" | "homestead" | "byzantium" | "foundation" | "mainnet" => SpecType::Foundation,
...
impl SpecType {
	pub fn spec<'a, T: Into<SpecParams<'a>>>(&self, params: T) -> Result<Spec, String> {
		...
		match *self {
			SpecType::Foundation => Ok(sophon::new_foundation(params)),
			SpecType::Classic => Ok(sophon::new_classic(params)),
			SpecType::Poanet => Ok(sophon::new_poanet(params)),
			SpecType::Tobalaba => Ok(sophon::new_tobalaba(params)),
			SpecType::Expanse => Ok(sophon::new_expanse(params)),
			SpecType::Musicoin => Ok(sophon::new_musicoin(params)),
			SpecType::Ellaism => Ok(sophon::new_ellaism(params)),
			SpecType::Mix => Ok(sophon::new_mix(params)),
			SpecType::Callisto => Ok(sophon::new_callisto(params)),
			SpecType::Morden => Ok(sophon::new_morden(params)),
			SpecType::Ropsten => Ok(sophon::new_ropsten(params)),
			SpecType::Kovan => Ok(sophon::new_kovan(params)),
			SpecType::Sokol => Ok(sophon::new_sokol(params)),
...
mod tests {
	...
	fn test_spec_type_parsing() {
		...
		assert_eq!(SpecType::Foundation, "sophon".parse().unwrap());
