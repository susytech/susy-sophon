...
use hash::keccak;
use sophon_types::{U256, H256, Address};
use susy_version::{version_data, version};
use bytes::Bytes;
use ansi_term::Colour;
use sync::{NetworkConfiguration, validate_node_url, self};
use sofkey::{Secret, Public};
use sofcore::client::{VMType};
use sofcore::miner::{stratum, MinerOptions};
use sofcore::snapshot::SnapshotConfiguration;
use sofcore::verification::queue::VerifierSettings;
use miner::pool;
use num_cpus;

use rpc::{IpcConfiguration, HttpConfiguration, WsConfiguration};
use susy_rpc::NetworkSettings;
use cache::CacheConfig;
use helpers::{to_duration, to_mode, to_block_id, to_u256, to_pending_set, to_price, graviton_ipc_path, susy_ipc_path, to_bootnodes, to_addresses, to_address, to_queue_strategy, to_queue_penalization};
...
use sofcore_private_tx::{ProviderConfig, EncryptorConfig};
...
use account::{AccountCmd, NewAccount, ListAccounts, ImportAccounts, ImportFromGravitonAccounts};
...
/// Configuration for the Susy client.
#[derive(Debug, PartialEq)]
...
impl Configuration {
	...
	///
	/// ```
	/// let _cfg = susy_sophon::Configuration::parse_cli(&["--light", "--chain", "kovan"]).unwrap();
	/// ```
	...
	pub(crate) fn into_command(self) -> Result<Execute, String> {
		...
		let graviton_compatibility = self.args.flag_graviton;
		let experimental_rpcs = self.args.flag_susy_jsonrpc_experimental;
		...
		let cmd = if self.args.flag_version {
		...
		} else if self.args.flag_import_graviton_keys {
				let account_cmd = AccountCmd::ImportFromGraviton(
				ImportFromGravitonAccounts {
					spec: spec,
					to: dirs.keys,
			...
			let run_cmd = RunCmd {
				...
				allow_missing_blocks: self.args.flag_susy_jsonrpc_allow_missing_blocks,
				mode: mode,
				tracing: tracing,
				fat_db: fat_db,
				compaction: compaction,
				...
				graviton_compatibility: graviton_compatibility,
				experimental_rpcs,
	...
	fn author(&self) -> Result<Address, String> {
		to_address(self.args.arg_sophybase.clone().or(self.args.arg_author.clone()))
	}
	...
	fn gas_pricer_config(&self) -> Result<GasPricerConfig, String> {
		fn wei_per_gas(usd_per_tx: f32, usd_per_sof: f32) -> U256 {
			let wei_per_usd: f32 = 1.0e18 / usd_per_sof;
		...
		if "auto" == self.args.arg_usd_per_sof.as_str() {
		...
		let usd_per_sof = to_price(&self.args.arg_usd_per_sof)?;
		let wei_per_gas = wei_per_gas(usd_per_tx, usd_per_sof);

		info!(
			"Using a fixed conversion rate of Ξ1 = {} ({} wei/gas)",
			Colour::White.bold().paint(format!("US${:.2}", usd_per_sof)),
	...
	fn net_config(&self) -> Result<NetworkConfiguration, String> {
		...
		ret.client_version = {
			...
			if !self.args.arg_identity.is_empty() {
				// Insert name after the "Susy-Sophon/" at the beginning of version string.
	...
	fn rpc_apis(&self) -> String {
		let mut apis: Vec<&str> = self.args.arg_rpcapi
			.as_ref()
			.unwrap_or(&self.args.arg_susy_jsonrpc_apis)
			.split(",")
			.collect();

		if self.args.flag_graviton {
	...
	fn rpc_cors(&self) -> Option<Vec<String>> {
		let cors = self.args.arg_rpccorsdomain.clone().unwrap_or_else(|| self.args.arg_susy_jsonrpc_cors.to_owned());
		Self::cors(&cors)
	}
	...
	fn rpc_hosts(&self) -> Option<Vec<String>> {
		self.hosts(&self.args.arg_susy_jsonrpc_hosts, &self.rpc_interface())
	}
	...
	fn http_config(&self) -> Result<HttpConfiguration, String> {
		let conf = HttpConfiguration {
			...
			port: self.args.arg_ports_shift + self.args.arg_rpcport.unwrap_or(self.args.arg_susy_jsonrpc_port),
			apis: self.rpc_apis().parse()?,
			hosts: self.rpc_hosts(),
			cors: self.rpc_cors(),
			server_threads: match self.args.arg_susy_jsonrpc_server_threads {
				Some(threads) if threads > 0 => threads,
				_ => 1,
			},
			processing_threads: self.args.arg_susy_jsonrpc_threads,
			max_payload: match self.args.arg_susy_jsonrpc_max_payload {
				Some(max) if max > 0 => max as usize,
				_ => 5usize,
			},
			keep_alive: !self.args.flag_susy_jsonrpc_no_keep_alive,
		};
	...
	fn ipc_path(&self) -> String {
		if self.args.flag_graviton {
			graviton_ipc_path(self.args.flag_testnet)
		} else {
			susy_ipc_path(
	...
	fn rpc_interface(&self) -> String {
		let rpc_interface = self.args.arg_rpcaddr.clone().unwrap_or(self.args.arg_susy_jsonrpc_interface.clone());
	...
	fn rpc_enabled(&self) -> bool {
		!self.args.flag_susy_jsonrpc_off && !self.args.flag_no_jsonrpc
	}
...
mod tests {
	...
	use tempdir::TempDir;
	use sofcore::client::{VMType, BlockId};
	use sofcore::miner::MinerOptions;
	...
	fn test_command_account_list() {
		let args = vec!["susy", "account", "list"];
		let conf = parse(&args);
	...
	fn test_command_account_import() {
		let args = vec!["susy", "account", "import", "my_dir", "another_dir"];
		let conf = parse(&args);
	...
	fn test_command_wallet_import() {
		let args = vec!["susy", "wallet", "import", "my_wallet.json", "--password", "pwd"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_import() {
		let args = vec!["susy", "import", "blockchain.json"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_export() {
		let args = vec!["susy", "export", "blocks", "blockchain.json"];
		let conf = parse(&args);
	...
	fn test_command_state_export() {
		let args = vec!["susy", "export", "state", "state.json"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_export_with_custom_format() {
		let args = vec!["susy", "export", "blocks", "--format", "hex", "blockchain.json"];
		let conf = parse(&args);
		assert_eq!(conf.into_command().unwrap().cmd, Cmd::Blockchain(BlockchainCmd::Export(ExportBlockchain {
			spec: Default::default(),
			cache_config: Default::default(),
	...
	fn test_command_signer_new_token() {
		let args = vec!["susy", "signer", "new-token"];
		let conf = parse(&args);
		...
		assert_eq!(conf.into_command().unwrap().cmd, Cmd::SignerToken(WsConfiguration {
			...
			origins: Some(vec!["susy://*".into(),"chrome-extension://*".into(), "moz-extension://*".into()]),
			hosts: Some(vec![]),
	...
	fn test_ws_max_connections() {
		let args = vec!["susy", "--ws-max-connections", "1"];
		let conf = parse(&args);
	...
	fn should_parse_mining_options() {
		...
		// when
		let conf0 = parse(&["susy"]);
		let conf2 = parse(&["susy", "--tx-queue-strategy", "gas_price"]);
	...
	fn should_fail_on_force_reseal_and_reseal_min_period() {
		let conf = parse(&["susy", "--chain", "dev", "--force-sealing", "--reseal-min-period", "0"]);
	...
	fn should_parse_updater_options() {
		// when
		let conf0 = parse(&["susy", "--release-track=testing"]);
		let conf1 = parse(&["susy", "--auto-update", "all", "--no-consensus", "--auto-update-delay", "300"]);
		let conf2 = parse(&["susy", "--no-download", "--auto-update=all", "--release-track=beta", "--auto-update-delay=300", "--auto-update-check-frequency=100"]);
		let conf3 = parse(&["susy", "--auto-update=xxx"]);
	...
	fn should_parse_network_settings() {
		...
		// when
		let conf = parse(&["susy", "--testnet", "--identity", "testname"]);
	...
	#[test]
	fn should_parse_rpc_settings_with_graviton_compatiblity() {
		// given
		fn assert(conf: Configuration) {
			...
			assert_eq!(conf.rpc_apis(), "susyweb,sof".to_owned());
		}

		// when
		let conf1 = parse(&["susy", "-j",
						 "--susy-jsonrpc-port", "8000",
						 "--susy-jsonrpc-interface", "all",
						 "--susy-jsonrpc-cors", "*",
						 "--susy-jsonrpc-apis", "susyweb,sof"
						 ]);
		let conf2 = parse(&["susy", "--rpc",
							"--rpcport", "8000",
							"--rpcaddr", "all",
							"--rpccorsdomain", "*",
							"--rpcapi", "susyweb,sof"
	...
	fn should_parse_rpc_hosts() {
		...
		// when
		let conf0 = parse(&["susy"]);
		let conf1 = parse(&["susy", "--susy-jsonrpc-hosts", "none"]);
		let conf2 = parse(&["susy", "--susy-jsonrpc-hosts", "all"]);
		let conf3 = parse(&["susy", "--susy-jsonrpc-hosts", "superstring.io,something.io"]);

		// then
		assert_eq!(conf0.rpc_hosts(), Some(Vec::new()));
		assert_eq!(conf1.rpc_hosts(), Some(Vec::new()));
		assert_eq!(conf2.rpc_hosts(), None);
		assert_eq!(conf3.rpc_hosts(), Some(vec!["superstring.io".into(), "something.io".into()]));
	}
	...
	fn should_parse_ipfs_hosts() {
		...
		// when
		let conf0 = parse(&["susy"]);
		let conf1 = parse(&["susy", "--ipfs-api-hosts", "none"]);
		let conf2 = parse(&["susy", "--ipfs-api-hosts", "all"]);
		let conf3 = parse(&["susy", "--ipfs-api-hosts", "superstring.io,something.io"]);

		// then
		assert_eq!(conf0.ipfs_hosts(), Some(Vec::new()));
		assert_eq!(conf1.ipfs_hosts(), Some(Vec::new()));
		assert_eq!(conf2.ipfs_hosts(), None);
		assert_eq!(conf3.ipfs_hosts(), Some(vec!["superstring.io".into(), "something.io".into()]));
	}
	...
	fn should_parse_ipfs_cors() {
		...
		// when
		let conf0 = parse(&["susy"]);
		let conf1 = parse(&["susy", "--ipfs-api-cors", "*"]);
		let conf2 = parse(&["susy", "--ipfs-api-cors", "http://superstring.io,http://something.io"]);

		// then
		assert_eq!(conf0.ipfs_cors(), Some(vec![]));
		assert_eq!(conf1.ipfs_cors(), None);
		assert_eq!(conf2.ipfs_cors(), Some(vec!["http://superstring.io".into(),"http://something.io".into()]));
	}
	...
	fn should_parse_ui_configuration() {
		...
		// when
		let conf0 = parse(&["susy", "--ui-path=signer"]);
		let conf1 = parse(&["susy", "--ui-path=signer", "--ui-no-validation"]);
		let conf2 = parse(&["susy", "--ui-path=signer", "--ui-port", "3123"]);
		let conf3 = parse(&["susy", "--ui-path=signer", "--ui-interface", "test"]);
		let conf4 = parse(&["susy", "--ui-path=signer", "--force-ui"]);

		// then
		assert_eq!(conf0.directories().signer, "signer".to_owned());

		assert!(conf1.ws_config().unwrap().hosts.is_some());
		assert_eq!(conf1.ws_config().unwrap().origins, Some(vec!["susy://*".into(), "chrome-extension://*".into(), "moz-extension://*".into()]));
	...
	fn should_not_bail_on_empty_line_in_reserved_peers() {
		...
		let args = vec!["susy", "--reserved-peers", filename.to_str().unwrap()];
	...
	fn test_mining_preset() {
		let args = vec!["susy", "--config", "mining"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_non_standard_ports_preset() {
		let args = vec!["susy", "--config", "non-standard-ports"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_insecure_preset() {
		let args = vec!["susy", "--config", "insecure"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				...
				// "susyweb,sof,net,personal,susy,susy_set,traces,rpc,susy_accounts");
	...
	fn test_dev_insecure_preset() {
		let args = vec!["susy", "--config", "dev-insecure"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				assert_eq!(c.net_settings.chain, "dev");
	...
	fn test_override_preset() {
		let args = vec!["susy", "--config", "mining", "--min-peers=99"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_identity_arg() {
		let args = vec!["susy", "--identity", "Somebody"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				assert_eq!(c.name, "Somebody");
				assert!(c.net_conf.client_version.starts_with("Susy-Sophon/Somebody/"));
			}
			_ => panic!("Should be Cmd::Run"),
		}
	}
	...
	fn should_apply_ports_shift() {
		...
		// when
		let conf0 = parse(&["susy", "--ports-shift", "1", "--stratum"]);
		let conf1 = parse(&["susy", "--ports-shift", "1", "--susy-jsonrpc-port", "8544"]);
	...
	fn should_expose_all_servers() {
		...
		// when
		let conf0 = parse(&["susy", "--unsafe-expose"]);
	...
	#[test]
	fn allow_ips() {
		let all = parse(&["susy", "--allow-ips", "all"]);
		let private = parse(&["susy", "--allow-ips", "private"]);
		let block_custom = parse(&["susy", "--allow-ips", "-10.0.0.0/8"]);
		let combo = parse(&["susy", "--allow-ips", "public 10.0.0.0/8 -1.0.0.0/8"]);
		let ipv6_custom_public = parse(&["susy", "--allow-ips", "public fc00::/7"]);
		let ipv6_custom_private = parse(&["susy", "--allow-ips", "private -fc00::/7"]);
	...
	fn should_use_correct_cache_path_if_base_is_set() {
		...
		let std = parse(&["susy"]);
		let base = parse(&["susy", "--base-path", "/test"]);
	...
	fn should_respect_only_max_peers_and_default() {
		let args = vec!["susy", "--max-peers=50"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_respect_only_min_peers_and_greater_than_default() {
		let args = vec!["susy", "--min-peers=500"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
