...
use {srlp, multihash, IpfsHandler};
...
use bytes::Bytes;
use sofcore::client::{BlockId, TransactionId};
...
impl IpfsHandler {
	...
	fn route_cid(&self, cid: &str) -> Result<Out> {
		...
		match cid.codec {
			Codec::SophonBlock => self.block(hash),
			Codec::SophonBlockList => self.block_list(hash),
			Codec::SophonTx => self.transaction(hash),
			Codec::SophonStateTrie => self.state_trie(hash),
	...
	fn block_list(&self, hash: H256) -> Result<Out> {
		...
		Ok(Out::OctetStream(srlp::encode_list(&uncles)))
	}
	...
	fn transaction(&self, hash: H256) -> Result<Out> {
		...
		Ok(Out::OctetStream(srlp::encode(&*tx)))
	}
...
mod tests {
	...
	fn cid_route_block_list() {
		...
		// `sof-block-list` with Keccak-256
	...
	fn cid_route_state_trie() {
		...
		// `sof-state-trie` with Keccak-256
