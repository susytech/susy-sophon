...
pub mod tests {
	...
	use crypto::DEFAULT_MAC;
	use sofkey::{self, crypto, Secret, Random, Generator, verify_public};
	...
	fn document_key_generation_and_retrievement_works_over_network_with_single_node() {
		...
		let signature = sofkey::sign(&secret, &document).unwrap();
	...
	fn server_key_generation_and_storing_document_key_works_over_network_with_3_nodes() {
		...
		for threshold in &test_cases {
			...
			let signature = sofkey::sign(&requestor_secret, &server_key_id).unwrap();
