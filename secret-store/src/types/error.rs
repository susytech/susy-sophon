...
impl Error {
	...
	pub fn is_non_fatal(&self) -> bool {
		match *self {
			...
			Error::SofKey(_) | Error::Serde(_) | Error::Hyper(_) | Error::Database(_) | Error::Internal(_) | Error::Io(_) => false,
		}
	}
}
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		match *self {
			...
			Error::SofKey(ref e) => write!(f, "cryptographic error {}", e),
...
impl From<sofkey::Error> for Error {
	fn from(err: sofkey::Error) -> Self {
		Error::SofKey(err.into())
	}
}

impl From<sofkey::crypto::Error> for Error {
	fn from(err: sofkey::crypto::Error) -> Self {
		Error::SofKey(err.to_string())
	}
}
