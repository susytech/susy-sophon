...
mod tests {
	...
	use sofkey::{self, Random, Generator, Public, Secret, public_to_address};
	...
	impl MessageLoop {
		...
		pub fn into_session(&self, at_node: usize) -> SessionImpl {
			let requester = Some(Requester::Signature(sofkey::sign(Random.generate().unwrap().secret(),
		...
		pub fn init_with_version(self, key_version: Option<H256>) -> Result<(Self, Public, H256), Error> {
			...
			let signature = sofkey::sign(requester.secret(), &SessionId::default()).unwrap();
