...
extern crate swasm;
extern crate vm;
extern crate swasm_utils as wasm_utils;
extern crate swasmi;
...
use swasmi::{Error as InterpreterError, Trap};
...
impl WasmInterpreter {
	pub fn run(self: Box<WasmInterpreter>, ext: &mut vm::Ext) -> vm::Result<GasLeft> {
		...
		let loaded_module = swasmi::Module::from_swasm_module(module).map_err(Error::Interpreter)?;

		let instantiation_resolver = env::ImportResolver::with_limit(16, ext.schedule().wasm());

		let module_instance = swasmi::ModuleInstance::new(
			&loaded_module,
			&swasmi::ImportsBuilder::new().with_resolver("env", &instantiation_resolver)
		...
		let (gas_left, result) = {
			...
			if let Err(InterpreterError::Trap(ref trap)) = invoke_result {
				if let swasmi::TrapKind::Host(ref boxed) = *trap.kind() {
