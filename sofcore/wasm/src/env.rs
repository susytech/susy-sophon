...
//! Env module glue for swasmi interpreter
...
/// Internal ids all functions runtime supports. This is just a glue for swasmi interpreter
...
pub mod signatures {
	use swasmi::{self, ValueType};
	use swasmi::ValueType::*;
	...
	impl Into<swasmi::Signature> for StaticSignature {
		fn into(self) -> swasmi::Signature {
			swasmi::Signature::new(self.0, self.1)
		}
	}
}
...
impl swasmi::ModuleImportResolver for ImportResolver {
	fn resolve_func(&self, field_name: &str, _signature: &Signature) -> Result<FuncRef, Error> {
		let func_ref = match field_name {
			...
			_ => {
				return Err(swasmi::Error::Instantiation(
