...
impl<T: Decodable> Decodable for Field<T> {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		match srlp.val_at::<u8>(0)? {
			0 => Ok(Field::Scalar(srlp.val_at::<T>(1)?)),
			1 => Ok({
				let inner_srlp = srlp.at(1)?;
				Field::BackReference(inner_srlp.val_at(0)?, inner_srlp.val_at(1)?)
			}),
...
impl Decodable for HashOrNumber {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		srlp.as_val::<H256>().map(HashOrNumber::Hash)
			.or_else(|_| srlp.as_val().map(HashOrNumber::Number))
	}
}
...
impl Decodable for Request {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		match srlp.val_at::<Kind>(0)? {
			Kind::Headers => Ok(Request::Headers(srlp.val_at(1)?)),
			Kind::HeaderProof => Ok(Request::HeaderProof(srlp.val_at(1)?)),
			Kind::TransactionIndex => Ok(Request::TransactionIndex(srlp.val_at(1)?)),
			Kind::Receipts => Ok(Request::Receipts(srlp.val_at(1)?)),
			Kind::Body => Ok(Request::Body(srlp.val_at(1)?)),
			Kind::Account => Ok(Request::Account(srlp.val_at(1)?)),
			Kind::Storage => Ok(Request::Storage(srlp.val_at(1)?)),
			Kind::Code => Ok(Request::Code(srlp.val_at(1)?)),
			Kind::Execution => Ok(Request::Execution(srlp.val_at(1)?)),
			Kind::Signal => Ok(Request::Signal(srlp.val_at(1)?)),
		}
	}
}
...
impl Encodable for Request {
	fn srlp_append(&self, s: &mut RlpStream) {
		...
		// hack around https://octonion.institute/susytech/susy-sophon/issues/4356
		Encodable::srlp_append(&self.kind(), s);
...
impl Decodable for Response {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		match srlp.val_at::<Kind>(0)? {
			Kind::Headers => Ok(Response::Headers(srlp.val_at(1)?)),
			Kind::HeaderProof => Ok(Response::HeaderProof(srlp.val_at(1)?)),
			Kind::TransactionIndex => Ok(Response::TransactionIndex(srlp.val_at(1)?)),
			Kind::Receipts => Ok(Response::Receipts(srlp.val_at(1)?)),
			Kind::Body => Ok(Response::Body(srlp.val_at(1)?)),
			Kind::Account => Ok(Response::Account(srlp.val_at(1)?)),
			Kind::Storage => Ok(Response::Storage(srlp.val_at(1)?)),
			Kind::Code => Ok(Response::Code(srlp.val_at(1)?)),
			Kind::Execution => Ok(Response::Execution(srlp.val_at(1)?)),
			Kind::Signal => Ok(Response::Signal(srlp.val_at(1)?)),
		}
	}
}
	...
	impl Encodable for Response {
		fn srlp_append(&self, s: &mut RlpStream) {
			...
			for header in &self.headers {
				s.append_raw(header.srlp().as_raw(), 1);
			}
		}
	}
}
...
pub mod block_body {
	...
	impl Decodable for Response {
		fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
			...
			// check body validity.
			let _: Vec<UnverifiedTransaction> = srlp.list_at(0)?;
			let _: Vec<FullHeader> = srlp.list_at(1)?;

			Ok(Response {
				body: encoded::Body::new(srlp.as_raw().to_owned()),
			})
		}
	}
	...
	impl Encodable for Response {
		fn srlp_append(&self, s: &mut RlpStream) {
			s.append_raw(&self.body.srlp().as_raw(), 1);
		}
	}
}
...
mod tests {
	...
	fn check_roundtrip<T>(val: T)
		where T: ::srlp::Encodable + ::srlp::Decodable + PartialEq + ::std::fmt::Debug
	{
		// check as single value.
		let bytes = ::srlp::encode(&val);
		let new_val: T = ::srlp::decode(&bytes).unwrap();
		assert_eq!(val, new_val);

		// check as list containing single value.
		let list = [val];
		let bytes = ::srlp::encode_list(&list);
		let new_list: Vec<T> = ::srlp::decode_list(&bytes);
	...
	fn headers_roundtrip() {
		...
		let res = HeadersResponse {
			headers: vec![
				::common_types::encoded::Header::new(::srlp::encode(&Header::default()))
			]
		};
	...
	fn vec_test() {
		...
		let srlp = Rlp::new(&out);
		assert_eq!(srlp.val_at::<usize>(0).unwrap(), 100usize);
		assert_eq!(srlp.list_at::<Request>(1).unwrap(), reqs);
	}
	...
	fn responses_vec() {
		...
		let raw = ::srlp::encode_list(&reqs);
		assert_eq!(::srlp::decode_list::<Response>(&raw), reqs);
	}
