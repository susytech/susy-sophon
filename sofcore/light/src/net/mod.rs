...
/// This is an implementation of the light sophon network protocol, abstracted
...
impl LightProtocol {
	...
	pub fn make_announcement(&self, io: &IoContext, mut announcement: Announcement) {
		...
		for (peer_id, peer_info) in self.peers.read().iter() {
			...
			if peer_info.sent_head == announcement.head_hash ||
				...
				// https://octonion.institute/susytech/susy-sophon/issues/10419
	...
	pub fn handle_packet(&self, io: &IoContext, peer: PeerId, packet_id: u8, data: &[u8]) {
		...
		let res = match packet_id {
			packet::STATUS => self.status(peer, io, &srlp),
			packet::ANNOUNCE => self.announcement(peer, io, &srlp),

			packet::REQUEST => self.request(peer, io, &srlp),
			packet::RESPONSE => self.response(peer, io, &srlp),

			packet::UPDATE_CREDITS => self.update_credits(peer, io, &srlp),
			packet::ACKNOWLEDGE_UPDATE => self.acknowledge_update(peer, io, &srlp),

			packet::SEND_TRANSACTIONS => self.relay_transactions(peer, io, &srlp),
...
impl LightProtocol {
	...
	fn request(&self, peer_id: PeerId, io: &IoContext, raw: &Rlp) -> Result<(), Error> {
		...
		for request_srlp in raw.at(1)?.iter().take(MAX_REQUESTS) {
			let request: Request = request_srlp.as_val()?;
