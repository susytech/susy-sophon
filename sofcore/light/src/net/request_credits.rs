...
use srlp::{Rlp, RlpStream, Decodable, Encodable, DecoderError};
...
impl Decodable for CostTable {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		...
		for cost_list in srlp.iter().skip(1) {
...
mod tests {
	...
	fn should_serialize_cost_table() {
		...
		let serialized = ::srlp::encode(&costs);

		let new_costs: CostTable = ::srlp::decode(&*serialized).unwrap();
