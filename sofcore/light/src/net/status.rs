...
use srlp::{DecoderError, Encodable, Decodable, RlpStream, Rlp};
...
impl<'a> Parser<'a> {
	...
	// expect a specific next key, and get the value's SRLP.
	...
	// get the next key and value SRLP.
	fn get_next(&mut self) -> Result<Option<(Key, Rlp<'a>)>, DecoderError> {
		while self.pos < self.srlp.item_count()? {
			let pair = self.srlp.at(self.pos)?;
...
pub struct Capabilities {
	...
	/// whether it can relay transactions to the sof network.
	pub tx_relay: bool,
}
...
pub fn parse_handshake(srlp: &Rlp) -> Result<(Status, Capabilities, Option<FlowParams>), DecoderError> {
...
pub fn parse_announcement(srlp: &Rlp) -> Result<Announcement, DecoderError> {
	let mut last_key = None;

	let mut announcement = Announcement {
		head_hash: srlp.val_at(0)?,
		head_num: srlp.val_at(1)?,
		head_td: srlp.val_at(2)?,
		reorg_depth: srlp.val_at(3)?,
		serve_headers: false,
		serve_state_since: None,
		serve_chain_since: None,
		tx_relay: false,
