...
use sofcore::client::{ClientReport, EnvInfo, ClientIoMessage};
use sofcore::engines::{epoch, SofEngine, EpochChange, EpochTransition, Proof};
use sofcore::machine::SophonMachine;
use sofcore::error::{Error, SofcoreResult};
use sofcore::verification::queue::{self, HeaderQueue};
...
pub trait LightChainClient: Send + Sync {
	...
	/// parent queued prior.
	fn queue_header(&self, header: Header) -> SofcoreResult<H256>;
...
impl<T: ChainDataFetcher> Client<T> {
	...
	pub fn import_header(&self, header: Header) -> SofcoreResult<H256> {
	...
	pub fn engine(&self) -> &Arc<SofEngine> {
		&self.engine
	}
	...
	fn check_epoch_signal(&self, verified_header: &Header) -> Result<Option<Proof<SophonMachine>>, T::Error> {
		use sofcore::machine::{AuxiliaryRequest, AuxiliaryData};
	...
	fn write_pending_proof(&self, header: &Header, proof: Proof<SophonMachine>) -> Result<(), T::Error> {
...
impl<T: ChainDataFetcher> ::sofcore::client::ChainInfo for Client<T> {
	fn chain_info(&self) -> BlockChainInfo {
		Client::chain_info(self)
	}
}

impl<T: ChainDataFetcher> ::sofcore::client::EngineClient for Client<T> {
	...
	fn as_full_client(&self) -> Option<&::sofcore::client::BlockChainClient> {
		None
	}
