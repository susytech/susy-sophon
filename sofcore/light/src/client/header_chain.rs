...
use sofcore::engines::epoch::{Transition as EpochTransition, PendingTransition as PendingEpochTransition};
use sofcore::error::{Error, SofcoreResult, ErrorKind as SofcoreErrorKind, BlockError};
use sofcore::spec::{Spec, SpecHardcodedSync};
use sophon_types::{H256, H264, U256};
use heapsize::HeapSizeOf;
use susykv::{DBTransaction, KeyValueDB};
...
/// The best block key. Maps to an SRLP list: [best_era, last_era]
...
fn decode_canonical_transition(t: &[u8]) -> Result<(Header, &[u8]), DecoderError> {
	...
	Ok((srlp.val_at(0)?, srlp.at(1)?.data()?))
}
...
impl HeaderChain {
	...
	pub fn new(
		...
		let genesis = ::srlp::encode(&spec.genesis_header());
		let decoded_header = spec.genesis_header();

		let chain = if let Some(current) = db.get(col, CURRENT_KEY)? {
			let curr : BestAndLatest = ::srlp::decode(&current).expect("decoding db value failed");
			...
			while let Some(entry) = db.get(col, era_key(cur_number).as_bytes())? {
				let entry: Entry = ::srlp::decode(&entry).expect("decoding db value failed");
			...
			if let (&Some(ref hardcoded_sync), HardcodedSync::Allow) = (&spec.hardcoded_sync, allow_hs) {
				...
				for (cht_num, cht_root) in hardcoded_sync.chts.iter().enumerate() {
					batch.put(col, cht_key(cht_num as u64).as_bytes(), &::srlp::encode(cht_root));
				}
	...
	fn insert_inner(
				...
				let parent_td =
					if number == 1 {
						...
						candidates.get(&(number - 1))
							...
							.map_err(SofcoreErrorKind::Block)?
					};
		...
		// insert headers and candidates entries and write era to disk.
		{
			...
			transaction.put(self.col, era_key(number).as_bytes(), &::srlp::encode(&*cur_era))
		}
		...
		if is_new_best {
			...
			for (&height, entry) in candidates.iter_mut().rev().skip_while(|&(height, _)| *height > number) {
				...
				if height != number {
					let srlp_era = ::srlp::encode(&*entry);
					transaction.put(self.col, era_key(height).as_bytes(), &srlp_era);
				}
			}
			...
			if earliest_era + HISTORY + cht::SIZE <= number {
							...
							if let Some(epoch_transition) = maybe_transition {
								...
								if ancient.hash == era_entry.canonical_hash {
									last_canonical_transition = match self.db.get(self.col, &ancient.hash) {
										...
										Ok(Some(header)) => Some((
											epoch_transition,
											::srlp::decode(&header).expect("decoding value from db failed")
										)),
									};
								}
							}
				...
				transaction.put(self.col, cht_key(cht_num).as_bytes(), &::srlp::encode(&cht_root));
		...
		// write the best and latest eras to the database.
		{
			...
			transaction.put(self.col, CURRENT_KEY, &::srlp::encode(&curr))
		}
		Ok(pending)
	}

	/// Generates the specifications for hardcoded sync. This is typically only called manually
	/// from time to time by a Susy developer in order to update the chain specifications.
	///
			...
			let cht = match self.cht_root(cht_num) {
				...
				None if cht_num != 0 => {
					...
					let entry: Entry = {
						...
						::srlp::decode(&bytes).expect("decoding db value failed")
					};
	...
	pub fn cht_root(&self, n: usize) -> Option<H256> {
		match self.db.get(self.col, cht_key(n as u64).as_bytes()) {
			Ok(db_fetch) => db_fetch.map(|bytes| ::srlp::decode(&bytes).expect("decoding value from db failed")),
			Err(e) => {
	...
	pub fn insert_pending_transition(&self, batch: &mut DBTransaction, hash: H256, t: &PendingEpochTransition) {
		...
		batch.put(self.col, &*key, &*::srlp::encode(t));
	}
