...
use sofcore::engines::{SofEngine, StateDependentProof};
use sofcore::machine::SophonMachine;
use sofcore::state::{self, ProvedExecution};
use sophon_types::{H256, U256, Address};
use softrie::{TrieError, TrieDB};
			...
			CheckedRequest::Body(ref check, ref req) => {
				...
				if let Ok(hdr) = check.0.as_ref() {
					if hdr.transactions_root() == KECCAK_NULL_RLP && hdr.uncles_hash() == KECCAK_EMPTY_LIST_RLP {
						...
						stream.append_raw(hdr.srlp().as_raw(), 1);
						stream.begin_list(0);
						stream.begin_list(0);
...
impl From<::srlp::DecoderError> for Error {
	fn from(err: ::srlp::DecoderError) -> Self {
		Error::Decoder(err)
	}
}
...
impl Body {
	...
	pub fn check_response(&self, cache: &Mutex<::cache::Cache>, body: &encoded::Body) -> Result<encoded::Block, Error> {
		...
		let tx_root = ::triehash::ordered_trie_root(body.transactions_srlp().iter().map(|r| r.as_raw()));
		...
		let uncles_hash = keccak(body.uncles_srlp().as_raw());
...
impl BlockReceipts {
	...
	pub fn check_response(&self, cache: &Mutex<::cache::Cache>, receipts: &[Receipt]) -> Result<Vec<Receipt>, Error> {
		...
		let found_root = ::triehash::ordered_trie_root(receipts.iter().map(|r| ::srlp::encode(r)));
...
impl Account {
	...
	pub fn check_response(&self, _: &Mutex<::cache::Cache>, proof: &[Bytes]) -> Result<Option<BasicAccount>, Error> {
		...
		match TrieDB::new(&db, &state_root).and_then(|t| t.get(&keccak(&self.address)))? {
			Some(val) => {
				let srlp = Rlp::new(&val);
				Ok(Some(BasicAccount {
					nonce: srlp.val_at(0)?,
					balance: srlp.val_at(1)?,
					storage_root: srlp.val_at(2)?,
					code_hash: srlp.val_at(3)?,
				}))
			},
			None => {
...
pub struct Signal {
	...
	pub proof_check: Arc<StateDependentProof<SophonMachine>>,
}
...
mod tests {
	...
	use softrie::{SecTrieDB, SecTrieDBMut};
	use trie::Recorder;
	use hash::keccak;

	use sofcore::client::{BlockChainClient, BlockInfo, TestBlockChainClient, EachBlockWith};
	...
	fn check_header_by_hash() {
		...
		let hash = header.hash();
		let raw_header = encoded::Header::new(::srlp::encode(&header));
	...
	fn check_header_with_ancestors() {
		...
		let raw_headers = headers.iter()
			.map(|hdr| encoded::Header::new(::srlp::encode(hdr)))
			.collect::<Vec<_>>();
		...
		let raw_invalid_successor = encoded::Header::new(::srlp::encode(&invalid_successor));
	...
	fn check_body() {
		...
		let req = Body(encoded::Header::new(::srlp::encode(&header)).into());
	...
	fn check_receipts() {
		...
		let receipts_root = ::triehash::ordered_trie_root(
			receipts.iter().map(|x| ::srlp::encode(x))
		);

		header.set_receipts_root(receipts_root);

		let req = BlockReceipts(encoded::Header::new(::srlp::encode(&header)).into());
	...
	fn check_state_proof() {
		...
		let req = Account {
			header: encoded::Header::new(::srlp::encode(&header)).into(),
			address: addr,
		};
