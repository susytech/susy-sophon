...
use softrie::{self, TrieDB, TrieDBMut};
use srlp::{RlpStream, Rlp};

// encode a key.
macro_rules! key {
	($num: expr) => { ::srlp::encode(&$num) }
}
...
impl<DB: HashDB<KeccakHasher, DBValue>> CHT<DB> {
	...
	pub fn prove(&self, num: u64, from_level: u32) -> softrie::Result<Option<Vec<Bytes>>> {
...
pub fn check_proof(proof: &[Bytes], num: u64, root: H256) -> Option<(H256, U256)> {
	...
	let res = match TrieDB::new(&db, &root) {
		...
		Ok(trie) => trie.get_with(&key!(num), |val: &[u8]| {
			...
			srlp.val_at::<H256>(0)
				.and_then(|h| srlp.val_at::<U256>(1).map(|td| (h, td)))
				.ok()
		})
	};
