...
use sofcore_miner::pool::VerifiedTransaction;
use sofcore_miner::service_transaction_checker::ServiceTransactionChecker;
use sophon_types::{H256, Address, U256};
use svm::Schedule;
use hash::keccak;
use io::IoChannel;
use itertools::Itertools;
use journaldb;
use susykv::{DBValue, KeyValueDB, DBTransaction};
...
use engines::{SofEngine, EpochTransition, ForkChoice, EngineError};
use engines::epoch::PendingTransition;
use error::{
	ImportErrorKind, ExecutionError, CallError, BlockError,
	QueueError, QueueErrorKind, Error as SofcoreError, SofcoreResult, ErrorKind as SofcoreErrorKind
};
...
struct Importer {
	...
	/// Sophon engine to be used during import
...
impl Importer {
	...
	fn check_and_lock_block(&self, bytes: &[u8], block: PreverifiedBlock, client: &Client) -> SofcoreResult<(LockedBlock, Option<PendingTransition>)> {
	...
	fn import_old_block(&self, unverified: Unverified, receipts_bytes: &[u8], db: &KeyValueDB, chain: &BlockChain) -> SofcoreResult<()> {
		let receipts = ::srlp::decode_list(receipts_bytes);
		let _import_lock = self.import_lock.lock();
	...
	fn commit_block<B>(&self, block: B, header: &Header, block_data: encoded::Block, pending: Option<PendingTransition>, client: &Client) -> ImportRoute where B: Drain {
		...
		let best = {
			...
			let header = chain.block_header_data(&hash)
				...
				.decode()
				.expect("Stored block header is valid SRLP; qed");
	...
	fn check_epoch_end_signal(
		...
		state_db: &StateDB,
		client: &Client,
	) -> SofcoreResult<Option<PendingTransition>> {
...
impl Client {
	...
	pub fn take_snapshot<W: snapshot_io::SnapshotWriter + Send>(&self, writer: W, at: BlockId, p: &snapshot::Progress) -> Result<(), SofcoreError> {
	...
	/// to avoid excessive SRLP encoding, decoding and hashing.
...
impl snapshot::DatabaseRestore for Client {
	...
	fn restore_db(&self, new_db: &str) -> Result<(), SofcoreError> {
...
impl ImportBlock for Client {
	fn import_block(&self, unverified: Unverified) -> SofcoreResult<H256> {
		if self.chain.read().is_known(&unverified.hash()) {
			bail!(SofcoreErrorKind::Import(ImportErrorKind::AlreadyInChain));
		}

		let status = self.block_status(BlockId::Hash(unverified.parent_hash()));
		if status == BlockStatus::Unknown {
			bail!(SofcoreErrorKind::Block(BlockError::UnknownParent(unverified.parent_hash())));
		}
		...
		match self.importer.block_queue.import(unverified) {
			...
			Err((block, SofcoreError(SofcoreErrorKind::Block(err), _))) => {
				self.importer.bad_blocks.report(block.bytes, format!("{:?}", err));
				bail!(SofcoreErrorKind::Block(err))
			},
			Err((_, e)) => Err(e),
		}
	}
...
impl BlockChainClient for Client {
	...
	fn uncle(&self, id: UncleId) -> Option<encoded::Header> {
		...
		self.block_body(id.block).and_then(|body| body.view().uncle_srlp_at(index))
...
impl IoClient for Client {
	...
	fn queue_ancient_block(&self, unverified: Unverified, receipts_bytes: Bytes) -> SofcoreResult<H256> {
		...
		let hash = unverified.hash();
		{
			...
			if !is_parent_pending && !self.chain.read().is_known(&parent_hash) {
				bail!(SofcoreErrorKind::Block(BlockError::UnknownParent(parent_hash)));
			}
		}
...
impl PrepareOpenBlock for Client {
	fn prepare_open_block(&self, author: Address, gas_range_target: (U256, U256), extra_data: Bytes) -> Result<OpenBlock, SofcoreError> {
		let engine = &*self.engine;
		let chain = self.chain.read();
...
impl ImportSealedBlock for Client {
	fn import_sealed_block(&self, block: SealedBlock) -> SofcoreResult<H256> {
		let start = Instant::now();
		let raw = block.srlp_bytes();
		let header = block.header().clone();
		let hash = header.hash();
		...
		let route = {
			...
			let block_data = block.srlp_bytes();
...
mod tests {
	...
	fn should_return_correct_log_index() {
		...
		let machine = ::sophon::new_frontier_test_machine();
