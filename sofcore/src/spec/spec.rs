...
use bytes::Bytes;
use sophon_types::{H256, Bloom, U256, Address};
use sofjson;
...
use builtin::Builtin;
use engines::{
	SofEngine, NullEngine, InstantSeal, InstantSealParams, BasicAuthority,
	AuthorityRound, DEFAULT_BLOCKHASH_CONTRACT
};
use error::Error;
use executive::Executive;
use factory::Factories;
use machine::SophonMachine;
...
/// Parameters common to sophon-like blockchains.
...
pub struct CommonParams {
	...
	/// SIP150 transition block number.
	pub sip150_transition: BlockNumber,
	/// Number of first block where SIP-160 rules begin.
	pub sip160_transition: BlockNumber,
	/// Number of first block where SIP-161.abc begin.
	pub sip161abc_transition: BlockNumber,
	/// Number of first block where SIP-161.d begins.
	pub sip161d_transition: BlockNumber,
	/// Number of first block where SIP-98 rules begin.
	pub sip98_transition: BlockNumber,
	/// Number of first block where SIP-658 rules begin.
	pub sip658_transition: BlockNumber,
	/// Number of first block where SIP-155 rules begin.
	pub sip155_transition: BlockNumber,
	/// Validate block receipts root.
	pub validate_receipts_transition: BlockNumber,
	/// Validate transaction chain id.
	pub validate_chain_id_transition: BlockNumber,
	/// Number of first block where SIP-140 (Metropolis: REVERT opcode) rules begin.
	pub sip140_transition: BlockNumber,
	/// Number of first block where SIP-210 (Metropolis: BLOCKHASH changes) rules begin.
	pub sip210_transition: BlockNumber,
	/// SIP-210 Blockhash contract address.
	pub sip210_contract_address: Address,
	/// SIP-210 Blockhash contract code.
	pub sip210_contract_code: Bytes,
	/// Gas allocated for SIP-210 blockhash update.
	pub sip210_contract_gas: U256,
	/// Number of first block where SIP-211 (Metropolis: RETURNDATASIZE/RETURNDATACOPY) rules
	/// begin.
	pub sip211_transition: BlockNumber,
	/// Number of first block where SIP-214 rules begin.
	pub sip214_transition: BlockNumber,
	/// Number of first block where SIP-145 rules begin.
	pub sip145_transition: BlockNumber,
	/// Number of first block where SIP-1052 rules begin.
	pub sip1052_transition: BlockNumber,
	/// Number of first block where SIP-1283 rules begin.
	pub sip1283_transition: BlockNumber,
	/// Number of first block where SIP-1283 rules end.
	pub sip1283_disable_transition: BlockNumber,
	/// Number of first block where SIP-1014 rules begin.
	pub sip1014_transition: BlockNumber,
	/// Number of first block where dust cleanup rules (SIP-168 and SIP169) begin.
	...
	/// Maximum size of transaction's SRLP payload
	pub max_transaction_size: usize,
}

impl CommonParams {
	/// Schedule for an SVM in the post-SIP-150-era of the Sophon main net.
	pub fn schedule(&self, block_number: u64) -> ::vm::Schedule {
		if block_number < self.sip150_transition {
			::vm::Schedule::new_homestead()
		} else {
			let max_code_size = self.max_code_size(block_number);
			let mut schedule = ::vm::Schedule::new_post_sip150(
				max_code_size as _,
				block_number >= self.sip160_transition,
				block_number >= self.sip161abc_transition,
				block_number >= self.sip161d_transition
			);
	...
	pub fn update_schedule(&self, block_number: u64, schedule: &mut ::vm::Schedule) {
		schedule.have_create2 = block_number >= self.sip1014_transition;
		schedule.have_revert = block_number >= self.sip140_transition;
		schedule.have_static_call = block_number >= self.sip214_transition;
		schedule.have_return_data = block_number >= self.sip211_transition;
		schedule.have_bitwise_shifting = block_number >= self.sip145_transition;
		schedule.have_extcodehash = block_number >= self.sip1052_transition;
		schedule.sip1283 = block_number >= self.sip1283_transition && !(block_number >= self.sip1283_disable_transition);
		if block_number >= self.sip210_transition {
	...
	pub fn nonzero_bugfix_hard_fork(&self) -> Option<&str> {
		if self.sip155_transition != 0 {
			return Some("sip155Transition");
		}
...
impl From<sofjson::spec::Params> for CommonParams {
	fn from(p: sofjson::spec::Params) -> Self {
		CommonParams {
			...
			subprotocol_name: p.subprotocol_name.unwrap_or_else(|| "sof".to_owned()),
			...
			sip150_transition: p.sip150_transition.map_or(0, Into::into),
			sip160_transition: p.sip160_transition.map_or(0, Into::into),
			sip161abc_transition: p.sip161abc_transition.map_or(0, Into::into),
			sip161d_transition: p.sip161d_transition.map_or(0, Into::into),
			sip98_transition: p.sip98_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip155_transition: p.sip155_transition.map_or(0, Into::into),
			validate_receipts_transition: p.validate_receipts_transition.map_or(0, Into::into),
			validate_chain_id_transition: p.validate_chain_id_transition.map_or(0, Into::into),
			sip140_transition: p.sip140_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip210_transition: p.sip210_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip210_contract_address: p.sip210_contract_address.map_or(0xf0.into(), Into::into),
			sip210_contract_code: p.sip210_contract_code.map_or_else(
				|| {
			...
			sip210_contract_gas: p.sip210_contract_gas.map_or(1000000.into(), Into::into),
			sip211_transition: p.sip211_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip145_transition: p.sip145_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip214_transition: p.sip214_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip658_transition: p.sip658_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip1052_transition: p.sip1052_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip1283_transition: p.sip1283_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip1283_disable_transition: p.sip1283_disable_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
			sip1014_transition: p.sip1014_transition.map_or_else(
				BlockNumber::max_value,
				Into::into,
			),
...
pub struct Spec {
	...
	/// Each seal field, expressed as SRLP, concatenated.
	pub seal_srlp: Bytes,
...
impl Clone for Spec {
	fn clone(&self) -> Spec {
		Spec {
			...
			seal_srlp: self.seal_srlp.clone(),
...
impl SpecHardcodedSync {
	...
	pub fn to_json(self) -> sofjson::spec::HardcodedSync {
		self.into()
	}
}
...
impl From<SpecHardcodedSync> for sofjson::spec::HardcodedSync {
	fn from(sync: SpecHardcodedSync) -> sofjson::spec::HardcodedSync {
		sofjson::spec::HardcodedSync {
			header: sync.header.into_inner().to_hex(),
			total_difficulty: sofjson::uint::Uint(sync.total_difficulty),
			chts: sync.chts.into_iter().map(Into::into).collect(),
		}
	}
}

fn load_machine_from(s: sofjson::spec::Spec) -> SophonMachine {
...
fn load_from(spec_params: SpecParams, s: sofjson::spec::Spec) -> Result<Spec, Error> {
	...
	let GenericSeal(seal_srlp) = g.seal.into();
	let params = CommonParams::from(s.params);
...
impl Spec {
	// create an instance of an Sophon state machine, minus consensus logic.
	fn machine(
		engine_spec: &sofjson::spec::Engine,
		params: CommonParams,
		builtins: BTreeMap<Address, Builtin>,
	) -> SophonMachine {
		if let sofjson::spec::Engine::Sofash(ref sofash) = *engine_spec {
			SophonMachine::with_sofash_extensions(params, builtins, sofash.params.clone().into())
		} else {
			SophonMachine::regular(params, builtins)
		}
	}
	...
	fn engine(
		...
		match engine_spec {
			sofjson::spec::Engine::Null(null) => Arc::new(NullEngine::new(null.params.into(), machine)),
			sofjson::spec::Engine::Sofash(sofash) => Arc::new(::sophon::Sofash::new(spec_params.cache_dir, sofash.params.into(), machine, spec_params.optimization_setting)),
			sofjson::spec::Engine::InstantSeal(Some(instant_seal)) => Arc::new(InstantSeal::new(instant_seal.params.into(), machine)),
			sofjson::spec::Engine::InstantSeal(None) => Arc::new(InstantSeal::new(InstantSealParams::default(), machine)),
			sofjson::spec::Engine::BasicAuthority(basic_authority) => Arc::new(BasicAuthority::new(basic_authority.params.into(), machine)),
			sofjson::spec::Engine::AuthorityRound(authority_round) => AuthorityRound::new(authority_round.params.into(), machine)
	...
	pub fn load_machine<R: Read>(reader: R) -> Result<SophonMachine, String> {
		sofjson::spec::Spec::load(reader)
			.map_err(fmt_err)
	...
	where
		R: Read,
	{
		sofjson::spec::Spec::load(reader).map_err(fmt_err).and_then(
			|x| {
	...
	pub fn genesis_epoch_data(&self) -> Result<Vec<u8>, String> {
		...
		let mut db = journaldb::new(
			Arc::new(susykv_memorydb::create(0)),
	...
	/// Create the SophonMachine corresponding to Spec::new_test.
	#[cfg(any(test, feature = "test-helpers"))]
	pub fn new_test_machine() -> SophonMachine { load_machine_bundled!("null_morden") }
	...
	/// https://github.com/susy-contracts/block-reward/blob/daf7d44383b6cdb11cb6b953b018648e2b027cfb/contracts/ExampleBlockReward.pol
	#[cfg(any(test, feature = "test-helpers"))]
	pub fn new_test_round_block_reward_contract() -> Self {
		load_bundled!("authority_round_block_reward_contract")
	}

	/// TestList.pol used in both specs: https://octonion.institute/susytech/contracts/pull/30/files
...
mod tests {
	...
	// https://octonion.institute/susytech/susy-sophon/issues/1840
	#[test]
	fn test_load_empty() {
