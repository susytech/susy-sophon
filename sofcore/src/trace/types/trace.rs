...
use sophon_types::{U256, Address, Bloom, BloomInput};
use bytes::Bytes;
...
impl Decodable for Reward {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		let res = Reward {
			author: srlp.val_at(0)?,
			value: srlp.val_at(1)?,
			reward_type: srlp.val_at(2)?,
		};
...
impl Decodable for Action {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		let action_type: u8 = srlp.val_at(0)?;
		match action_type {
			0 => srlp.val_at(1).map(Action::Call),
			1 => srlp.val_at(1).map(Action::Create),
			2 => srlp.val_at(1).map(Action::Suicide),
			3 => srlp.val_at(1).map(Action::Reward),
...
impl Decodable for Res {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		let action_type: u8 = srlp.val_at(0)?;
		match action_type {
			0 => srlp.val_at(1).map(Res::Call),
			1 => srlp.val_at(1).map(Res::Create),
			2 => srlp.val_at(1).map(Res::FailedCall),
			3 => srlp.val_at(1).map(Res::FailedCreate),
			4 => Ok(Res::None),
