...
impl PodState {
	...
	/// Get the root hash of the trie of the SRLP of this.
	pub fn root(&self) -> H256 {
		sec_trie_root(self.0.iter().map(|(k, v)| (k, v.srlp())))
	}
...
impl From<sofjson::blockchain::State> for PodState {
	fn from(s: sofjson::blockchain::State) -> PodState {
		let state = s.into_iter().map(|(addr, acc)| (addr.into(), PodAccount::from(acc))).collect();
		PodState(state)
	}
}

impl From<sofjson::spec::State> for PodState {
	fn from(s: sofjson::spec::State) -> PodState {
