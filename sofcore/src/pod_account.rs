...
impl PodAccount {
	...
	/// Returns the SRLP for this account.
	pub fn srlp(&self) -> Bytes {
		let mut stream = RlpStream::new_list(4);
		stream.append(&self.nonce);
		stream.append(&self.balance);
		stream.append(&sec_trie_root(self.storage.iter().map(|(k, v)| (k, srlp::encode(&U256::from(&**v))))));
	...
	pub fn insert_additional(&self, db: &mut HashDB<KeccakHasher, DBValue>, factory: &TrieFactory<KeccakHasher, RlpCodec>) {
		...
		for (k, v) in &self.storage {
			if let Err(e) = t.insert(k, &srlp::encode(&U256::from(&**v))) {
...
impl From<sofjson::blockchain::Account> for PodAccount {
	fn from(a: sofjson::blockchain::Account) -> Self {
		PodAccount {
...
impl From<sofjson::spec::Account> for PodAccount {
	fn from(a: sofjson::spec::Account) -> Self {
		PodAccount {
