...
use sofkey::KeyPair;
use svm::Factory as SvmFactory;
use hash::keccak;
use io::IoChannel;
use susykv::KeyValueDB;
use susykv_rocksdb::{self, Database, DatabaseConfig};
use parking_lot::RwLock;
use srlp::{self, RlpStream};
use tempdir::TempDir;
...
pub fn create_test_block_with_data(header: &Header, transactions: &[SignedTransaction], uncles: &[Header]) -> Bytes {
	let mut srlp = RlpStream::new_list(3);
	srlp.append(header);
	srlp.begin_list(transactions.len());
	for t in transactions {
		srlp.append_raw(&srlp::encode(t), 1);
	}
	srlp.append_list(&uncles);
	srlp.out()
}
	...
	F: Fn() -> Spec
{
	...
	for _ in 0..block_number {
		...
		if let Err(e) = client.import_block(Unverified::from_srlp(b.srlp_bytes()).unwrap()) {
			panic!("error importing block which is valid by definition: {:?}", e);
		}

		last_header = view!(BlockView, &b.srlp_bytes()).header();
...
pub fn push_blocks_to_client(client: &Arc<Client>, timestamp_salt: u64, starting_number: usize, block_number: usize) {
	...
	for _ in 0..block_number {
		...
		if let Err(e) = client.import_block(Unverified::from_srlp(create_test_block(&header)).unwrap()) {
			panic!("error importing block which is valid by definition: {:?}", e);
		}
	}
}
...
pub fn get_test_client_with_blocks(blocks: Vec<Bytes>) -> Arc<Client> {
	...
	for block in blocks {
		if let Err(e) = client.import_block(Unverified::from_srlp(block).unwrap()) {
...
pub fn new_db() -> Arc<BlockChainDB> {
	...
	let db = TestBlockChainDB {
		...
		key_value: Arc::new(::susykv_memorydb::create(::db::NUM_COLUMNS.unwrap()))
	};
...
pub fn restoration_db_handler(config: susykv_rocksdb::DatabaseConfig) -> Box<BlockChainDBHandler> {
	struct RestorationDBHandler {
		config: susykv_rocksdb::DatabaseConfig,
	}
	...
	impl BlockChainDBHandler for RestorationDBHandler {
		fn open(&self, db_path: &Path) -> io::Result<Arc<BlockChainDB>> {
			let key_value = Arc::new(susykv_rocksdb::Database::open(&self.config, &db_path.to_string_lossy())?);
...
pub fn get_temp_state_with_factory(factory: SvmFactory) -> State<::state_db::StateDB> {
	let journal_db = get_temp_state_db();
