...
mod test {
	...
	fn transaction_filter() {
		...
		let mut basic_tx_with_sophy_and_to_key7 = Transaction::default();
		basic_tx_with_sophy_and_to_key7.action = Action::Call(Address::from("d41c057fd1c78805aac12b0a94a405c0461a6fbb"));
		basic_tx_with_sophy_and_to_key7.value = U256::from(123123);
		let mut call_tx_with_sophy = Transaction::default();
		call_tx_with_sophy.action = Action::Call(Address::from("0000000000000000000000000000000000000005"));
		call_tx_with_sophy.value = U256::from(123123);
		...
		let mut basic_tx_with_sophy_and_to_key6 = Transaction::default();
		basic_tx_with_sophy_and_to_key6.action = Action::Call(Address::from("e57bfe9f44b819898f47bf37e5af72a0783e1141"));
		basic_tx_with_sophy_and_to_key6.value = U256::from(123123);
		...
		assert!(!filter.transaction_allowed(&genesis, block_number, &basic_tx_with_sophy_and_to_key7.clone().sign(key5.secret(), None), &*client));
		assert!(!filter.transaction_allowed(&genesis, block_number, &call_tx_with_sophy.clone().sign(key5.secret(), None), &*client));
		assert!(filter.transaction_allowed(&genesis, block_number, &basic_tx.clone().sign(key6.secret(), None), &*client));
		assert!(filter.transaction_allowed(&genesis, block_number, &basic_tx_with_sophy_and_to_key7.clone().sign(key6.secret(), None), &*client));
		assert!(filter.transaction_allowed(&genesis, block_number, &basic_tx_to_key6.clone().sign(key7.secret(), None), &*client));
		assert!(!filter.transaction_allowed(&genesis, block_number, &basic_tx_with_sophy_and_to_key6.clone().sign(key7.secret(), None), &*client));
	}
