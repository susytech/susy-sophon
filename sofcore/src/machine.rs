...
//! Sophon-like state machine definition.
...
/// Susy tries to round block.gas_limit to multiple of this constant
pub const SUSY_GAS_LIMIT_DETERMINANT: U256 = U256([37, 0, 0, 0]);

/// Sofash-specific extensions.
#[derive(Debug, Clone)]
pub struct SofashExtensions {
...
impl From<::sofjson::spec::SofashParams> for SofashExtensions {
...
/// An sophon-like state machine.
pub struct SophonMachine {
	params: CommonParams,
	builtins: Arc<BTreeMap<Address, Builtin>>,
	tx_filter: Option<Arc<TransactionFilter>>,
	sofash_extensions: Option<SofashExtensions>,
	schedule_rules: Option<Box<ScheduleCreationRules>>,
}

impl SophonMachine {
	/// Regular sophon machine.
	pub fn regular(params: CommonParams, builtins: BTreeMap<Address, Builtin>) -> SophonMachine {
		let tx_filter = TransactionFilter::from_params(&params).map(Arc::new);
		SophonMachine {
			params: params,
			builtins: Arc::new(builtins),
			tx_filter: tx_filter,
			sofash_extensions: None,
			schedule_rules: None,
		}
	}

	/// Sophon machine with sofash extensions.
	// TODO: either unify or specify to mainnet specifically and include other specific-chain HFs?
	pub fn with_sofash_extensions(params: CommonParams, builtins: BTreeMap<Address, Builtin>, extensions: SofashExtensions) -> SophonMachine {
		let mut machine = SophonMachine::regular(params, builtins);
		machine.sofash_extensions = Some(extensions);
		machine
	}
	...
	/// Get a reference to the sofash-specific extensions.
	pub fn sofash_extensions(&self) -> Option<&SofashExtensions> {
		self.sofash_extensions.as_ref()
	}
}
...
impl SophonMachine {
	...
	fn push_last_hash(&self, block: &mut ExecutedBlock) -> Result<(), Error> {
		...
		if block.header().number() == params.sip210_transition {
			let state = block.state_mut();
			state.init_code(&params.sip210_contract_address, params.sip210_contract_code.clone())?;
		}
		if block.header().number() >= params.sip210_transition {
			let parent_hash = block.header().parent_hash().clone();
			let _ = self.execute_as_system(
				block,
				params.sip210_contract_address,
				params.sip210_contract_gas,
	...
	pub fn on_new_block(&self, block: &mut ExecutedBlock) -> Result<(), Error> {
		...
		if let Some(ref sofash_params) = self.sofash_extensions {
			if block.header().number() == sofash_params.dao_hardfork_transition {
				let state = block.state_mut();
				for child in &sofash_params.dao_hardfork_accounts {
					let beneficiary = &sofash_params.dao_hardfork_beneficiary;
					state.balance(child)
	...
	pub fn populate_from_parent(&self, header: &mut Header, parent: &Header, gas_floor_target: U256, gas_ceil_target: U256) {
		...
		if let Some(ref sofash_params) = self.sofash_extensions {
			...
			if header.number() >= sofash_params.dao_hardfork_transition &&
				header.number() <= sofash_params.dao_hardfork_transition + 9 {
	...
	/// Get the SVM schedule for the given block number.
	pub fn schedule(&self, block_number: BlockNumber) -> Schedule {
		let mut schedule = match self.sofash_extensions {
	...
	pub fn signing_chain_id(&self, env_info: &EnvInfo) -> Option<u64> {
		...
		if env_info.number >= params.sip155_transition {
	...
	pub fn verify_transaction_basic(&self, t: &UnverifiedTransaction, header: &Header) -> Result<(), transaction::Error> {
		let check_low_s = match self.sofash_extensions {
		...
		let chain_id = if header.number() < self.params().validate_chain_id_transition {
			t.chain_id()
		} else if header.number() >= self.params().sip155_transition {
	...
	/// Performs pre-validation of SRLP decoded transaction before other processing
	pub fn decode_transaction(&self, transaction: &[u8]) -> Result<UnverifiedTransaction, transaction::Error> {
		let srlp = Rlp::new(&transaction);
		if srlp.as_raw().len() > self.params().max_transaction_size {
			debug!("Rejected oversized transaction of {} bytes", srlp.as_raw().len());
			return Err(transaction::Error::TooBig)
		}
		srlp.as_val().map_err(|e| transaction::Error::InvalidRlp(e.to_string()))
	}
}

/// Auxiliary data fetcher for an Sophon machine. In Sophon-like machines
...
impl ::susy_machine::Machine for SophonMachine {
	type Header = Header;
...
impl<'a> ::susy_machine::LocalizedMachine<'a> for SophonMachine {
	type StateContext = Call<'a>;
	type AuxiliaryData = AuxiliaryData<'a>;
}

/// A state machine that uses block rewards.
pub trait WithRewards: ::susy_machine::Machine {
...
impl WithRewards for SophonMachine {
	fn note_rewards(
		&self,
		live: &mut Self::LiveBlock,
		rewards: &[(Address, RewardType, U256)],
...
// 2) it will be a nearest (with tendency to increase) multiple of SUSY_GAS_LIMIT_DETERMINANT
fn round_block_gas_limit(gas_limit: U256, lower_limit: U256, upper_limit: U256) -> U256 {
	let increased_gas_limit = gas_limit + (SUSY_GAS_LIMIT_DETERMINANT - gas_limit % SUSY_GAS_LIMIT_DETERMINANT);
	if increased_gas_limit > upper_limit {
		let decreased_gas_limit = increased_gas_limit - SUSY_GAS_LIMIT_DETERMINANT;
...
mod tests {
	...
	fn get_default_sofash_extensions() -> SofashExtensions {
		SofashExtensions {
	...
	fn should_disallow_unsigned_transactions() {
		let srlp = "ea80843b9aca0083015f90948921ebb5f79e9e3920abe571004d0b1d5119c154865af3107a400080038080".into();
		let transaction: UnverifiedTransaction = ::srlp::decode(&::rustc_hex::FromHex::from_hex(srlp).unwrap()).unwrap();
		let spec = ::sophon::new_ropsten_test();
		let sofparams = get_default_sofash_extensions();

		let machine = SophonMachine::with_sofash_extensions(
			spec.params().clone(),
			Default::default(),
			sofparams,
		);
	...
	#[test]
	fn sofash_gas_limit_is_multiple_of_determinant() {
		use sophon_types::U256;

		let spec = ::sophon::new_homestead_test();
		let sofparams = get_default_sofash_extensions();
		...
		assert_eq!(SUSY_GAS_LIMIT_DETERMINANT, U256::from(37));
