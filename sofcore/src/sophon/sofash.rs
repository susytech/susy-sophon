...
use sophon_types::{H256, H64, U256};
use sofjson;
...
use sofash::{self, quick_get_difficulty, slow_hash_block_number, SofashManager, OptimizeFor};
use machine::SophonMachine;

/// Number of blocks in an sofash snapshot.
// make dependent on difficulty incrment divisor?
const SNAPSHOT_BLOCKS: u64 = 5000;
/// Maximum number of blocks allowed in an sofash snapshot.
const MAX_SNAPSHOT_BLOCKS: u64 = 30000;

/// Sofash specific seal
#[derive(Debug, PartialEq)]
pub struct Seal {
	/// Sofash seal mix_hash
	pub mix_hash: H256,
	/// Sofash seal nonce
	pub nonce: H64,
}
...
impl Seal {
	/// Tries to parse srlp as sofash seal.
...
pub struct SofashParams {
	...
	/// Number of first block where SIP-100 rules begin.
	pub sip100b_transition: u64,
...
impl From<sofjson::spec::SofashParams> for SofashParams {
	fn from(p: sofjson::spec::SofashParams) -> Self {
		SofashParams {
			...
			sip100b_transition: p.sip100b_transition.map_or(u64::max_value(), Into::into),
			...
			block_reward: p.block_reward.map_or_else(
				...
				|reward| {
					match reward {
						sofjson::spec::BlockReward::Single(reward) => {
							let mut ret = BTreeMap::new();
							ret.insert(0, reward.into());
							ret
						},
						sofjson::spec::BlockReward::Multi(multi) => {
							multi.into_iter()
...
/// Engine using Sofash proof-of-work consensus algorithm, suitable for Sophon
/// mainnet chains in the Olympic, Frontier and Homestead eras.
pub struct Sofash {
	sofash_params: SofashParams,
	pow: SofashManager,
	machine: SophonMachine,
}

impl Sofash {
	/// Create a new instance of Sofash engine
	pub fn new<T: Into<Option<OptimizeFor>>>(
		...
		let progpow_transition = sofash_params.progpow_transition;

		Arc::new(Sofash {
			sofash_params,
			machine,
			pow: SofashManager::new(cache_dir.as_ref(), optimize_for.into(), progpow_transition),
		})
	}
}
...
// TODO [rphmeier]
//
// for now, this is different than Sofash's own epochs, and signal
// "consensus epochs".
// in this sense, `Sofash` is epochless: the same `EpochVerifier` can be used
// for any block in the chain.
// in the future, we might move the Sofash epoch
// caching onto this mechanism as well.
impl engines::EpochVerifier<SophonMachine> for Arc<Sofash> {
...
impl Engine<SophonMachine> for Arc<Sofash> {
	fn name(&self) -> &str { "Sofash" }
	fn machine(&self) -> &SophonMachine { &self.machine }
	...
	fn on_close_block(&self, block: &mut ExecutedBlock) -> Result<(), Error> {
		use std::ops::Shr;
		use susy_machine::LiveBlock;

		let author = *LiveBlock::header(&*block).author();
		let number = LiveBlock::header(&*block).number();

		let rewards = match self.sofash_params.block_reward_contract {
			Some(ref c) if number >= self.sofash_params.block_reward_contract_transition => {
			...
			_ => {
				...
				let (_, reward) = self.sofash_params.block_reward.iter()
					.rev()
				...
				let eras_rounds = self.sofash_params.ecip1017_era_rounds;
	...
	fn verify_block_basic(&self, header: &Header) -> Result<(), Error> {
		...
		let min_difficulty = self.sofash_params.minimum_difficulty;
		...
		let difficulty = sofash::boundary_to_difficulty(&H256(quick_get_difficulty(
			&header.bare_hash().0,
			seal.nonce.low_u64(),
			&seal.mix_hash.0,
			header.number() >= self.sofash_params.progpow_transition
		)));
	...
	fn verify_block_unordered(&self, header: &Header) -> Result<(), Error> {
		...
		let difficulty = sofash::boundary_to_difficulty(&H256(result.value));
	...
	fn epoch_verifier<'a>(&self, _header: &Header, _proof: &'a [u8]) -> engines::ConstructedVerifier<'a, SophonMachine> {
...
impl Sofash {
	fn calculate_difficulty(&self, header: &Header, parent: &Header) -> U256 {
		...
		let difficulty_hardfork = header.number() >= self.sofash_params.difficulty_hardfork_transition;
		let difficulty_bound_divisor = if difficulty_hardfork {
			self.sofash_params.difficulty_hardfork_bound_divisor
		} else {
			self.sofash_params.difficulty_bound_divisor
		};

		let expip2_hardfork = header.number() >= self.sofash_params.expip2_transition;
		let duration_limit = if expip2_hardfork {
			self.sofash_params.expip2_duration_limit
		} else {
			self.sofash_params.duration_limit
		};

		let frontier_limit = self.sofash_params.homestead_transition;
		...
		let mut target = if header.number() < frontier_limit {
			...
			trace!(target: "sofash", "Calculating difficulty parent.difficulty={}, header.timestamp={}, parent.timestamp={}", parent.difficulty(), header.timestamp(), parent.timestamp());
			//block_diff = parent_diff + parent_diff // 2048 * max(1 - (block_timestamp - parent_timestamp) // 10, -99)
			let (increment_divisor, threshold) = if header.number() < self.sofash_params.sip100b_transition {
				(self.sofash_params.difficulty_increment_divisor, 1)
			} else if parent_has_uncles {
				(self.sofash_params.metropolis_difficulty_increment_divisor, 2)
			} else {
				(self.sofash_params.metropolis_difficulty_increment_divisor, 1)
			};
		...
		if header.number() < self.sofash_params.bomb_defuse_transition {
			if header.number() < self.sofash_params.ecip1010_pause_transition {
				let mut number = header.number();
				let original_number = number;
				for (block, delay) in &self.sofash_params.difficulty_bomb_delays {
			...
			} else if header.number() < self.sofash_params.ecip1010_continue_transition {
				let fixed_difficulty = ((self.sofash_params.ecip1010_pause_transition / EXP_DIFF_PERIOD) - 2) as usize;
				target = cmp::max(min_difficulty, target + (U256::from(1) << fixed_difficulty));
			} else {
				let period = ((parent.number() + 1) / EXP_DIFF_PERIOD) as usize;
				let delay = ((self.sofash_params.ecip1010_continue_transition - self.sofash_params.ecip1010_pause_transition) / EXP_DIFF_PERIOD) as usize;
...
mod tests {
	...
	use std::sync::Arc;
	use std::collections::BTreeMap;
	use sophon_types::{H64, H256, U256, Address};
	use block::*;
	...
	use super::{Sofash, SofashParams, ecip1017_eras_block_reward};
	use srlp;
	use tempdir::TempDir;
	...
	fn get_default_sofash_params() -> SofashParams {
		SofashParams {
			...
			sip100b_transition: u64::max_value(),
	...
	fn can_do_difficulty_verification_fail() {
		...
		header.set_seal(vec![srlp::encode(&H256::zero()), srlp::encode(&H64::zero())]);
	...
	fn can_do_proof_of_work_unordered_verification_fail() {
		let engine = test_spec().engine;
		let mut header: Header = Header::default();
		header.set_seal(vec![srlp::encode(&H256::from("b251bd2e0283d0658f2cadfdc8ca619b5de94eca5742725e2e757dd13ed7503d")), srlp::encode(&H64::zero())]);
		header.set_difficulty(U256::from_str("ffffffffffffffffffffffffffffffffffffffffffffaaaaaaaaaaaaaaaaaaaa").unwrap());
	...
	fn difficulty_frontier() {
		...
		let sofparams = get_default_sofash_params();
		let tempdir = TempDir::new("").unwrap();
		let sofash = Sofash::new(tempdir.path(), sofparams, machine, None);
		...
		let difficulty = sofash.calculate_difficulty(&header, &parent_header);
	...
	fn difficulty_classic_bomb_delay() {
		let machine = new_homestead_test_machine();
		let sofparams = SofashParams {
			ecip1010_pause_transition: 3000000,
			..get_default_sofash_params()
		};
		let tempdir = TempDir::new("").unwrap();
		let sofash = Sofash::new(tempdir.path(), sofparams, machine, None);
		...
		assert_eq!(
			...
			sofash.calculate_difficulty(&header, &parent_header)
		);
	...
	fn test_extra_info() {
		...
		let mut header = Header::default();
		header.set_seal(vec![srlp::encode(&H256::from("b251bd2e0283d0658f2cadfdc8ca619b5de94eca5742725e2e757dd13ed7503d")), srlp::encode(&H64::zero())]);
		let info = sofash.extra_info(&header);
