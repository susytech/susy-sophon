...
use std::sync::Arc;
use hash::keccak;
use sophon_types::{H256, U256, U512, Address};
use bytes::{Bytes, BytesRef};
use state::{Backend as StateBackend, State, Substate, CleanupMode};
use executed::ExecutionError;
use machine::SophonMachine as Machine;
use svm::{CallType, Finalize, FinalizationResult};
use vm::{
...
/// Roughly estimate what stack size each level of svm depth will use. (Debug build)
const STACK_SIZE_PER_DEPTH: usize = 128 * 1024;

#[cfg(not(debug_assertions))]
/// Roughly estimate what stack size each level of svm depth will use.
...
impl<'a> CallCreateExecutive<'a> {
	...
	fn check_sip684<B: 'a + StateBackend>(params: &ActionParams, state: &State<B>) -> vm::Result<()> {
		...
		match self.kind {
			...
			CallCreateExecutiveKind::ExecCreate(params, mut unconfirmed_substate) => {
...

				{
					...
					let mut pre_inner = || {
						Self::check_sip684(&params, state)?;
						Self::check_static_flag(&params, static_flag, is_create)?;
						state.checkpoint();
...
mod tests {
	...
	fn make_frontier_machine(max_depth: usize) -> SophonMachine {
		let mut machine = ::sophon::new_frontier_test_machine();
		machine.set_schedule_creation_rules(Box::new(move |s, _| s.max_depth = max_depth));
		machine
	}

	fn make_byzantium_machine(max_depth: usize) -> SophonMachine {
		let mut machine = ::sophon::new_byzantium_test_machine();
		machine.set_schedule_creation_rules(Box::new(move |s, _| s.max_depth = max_depth));
		machine
	}
	...
	svm_test!{test_sender_balance: test_sender_balance_int}
	...
	svm_test!{test_create_contract_out_of_depth: test_create_contract_out_of_depth_int}
	...
	fn test_trace_reverted_create() {
		...
		state.add_balance(&sender, &U256::from(100), CleanupMode::NoEmpty).unwrap();
		let info = EnvInfo::default();
		let machine = ::sophon::new_byzantium_test_machine();
		let schedule = machine.schedule(info.number);
		let mut substate = Substate::new();
		let mut tracer = ExecutiveTracer::default();
		let mut vm_tracer = ExecutiveVMTracer::toplevel();
	...
	svm_test!{test_create_contract_value_too_high: test_create_contract_value_too_high_int}
	...
	svm_test!{test_create_contract_without_max_depth: test_create_contract_without_max_depth_int}
	...
	svm_test_ignore!{test_aba_calls: test_aba_calls_int}
	...
	// test is incorrect, mk
	// TODO: fix (preferred) or remove
	svm_test_ignore!{test_recursive_bomb1: test_recursive_bomb1_int}
	...
	// test is incorrect, mk
	// TODO: fix (preferred) or remove
	svm_test_ignore!{test_transact_simple: test_transact_simple_int}
	...
	svm_test!{test_transact_invalid_nonce: test_transact_invalid_nonce_int}
	...
	svm_test!{test_transact_gas_limit_reached: test_transact_gas_limit_reached_int}
	...
	svm_test!{test_not_enough_cash: test_not_enough_cash_int}
	...
	svm_test!{test_keccak: test_keccak_int}
	...
	svm_test!{test_revert: test_revert_int}
	...
	svm_test!{test_sip1283: test_sip1283_int}
	fn test_sip1283(factory: Factory) {
		...
		let info = EnvInfo::default();
		let machine = ::sophon::new_constantinople_test_machine();
		let schedule = machine.schedule(info.number);
	...
	fn wasm_activated_test() {
		...
		let machine = ::sophon::new_kovan_wasm_test_machine();
