...
//! General error types for use in sofcore.

use std::{fmt, error};
use std::time::SystemTime;

use sophon_types::{H256, U256, Address, Bloom};
use sofkey::Error as SofkeyError;
use softrie::TrieError;
use srlp;
...
pub enum BlockError {
	...
	/// Seal element of type H256 (max_hash for Sofash, but could be something else for
...
error_chain! {
	types {
		Error, ErrorKind, ErrorResultExt, SofcoreResult;
	}
	...
	foreign_links {
		...
		Execution(ExecutionError) #[doc = "Error concerning SVM code execution."];
		...
		Sofkey(SofkeyError) #[doc = "Sofkey error."];
		Decoder(srlp::DecoderError) #[doc = "SRLP decoding errors"];
	}
