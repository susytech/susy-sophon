...
fn should_return_registrar() {
	...
	let spec = sophon::new_morden(&tempdir.path().to_owned());
...
fn imports_good_block() {
	...
	if client.import_block(Unverified::from_srlp(good_block).unwrap()).is_err() {
...
fn returns_block_body() {
	...
	assert_eq!(body.at(0).unwrap().as_raw()[..], block.srlp().at(1).as_raw()[..]);
	assert_eq!(body.at(1).unwrap().as_raw()[..], block.srlp().at(2).as_raw()[..]);
}
