...
fn can_trace_block_and_uncle_reward() {
	...
	if let Err(e) = client.import_block(Unverified::from_srlp(root_block.srlp_bytes()).unwrap()) {
		panic!("error importing block which is valid by definition: {:?}", e);
	}

	last_header = view!(BlockView, &root_block.srlp_bytes()).header();
	...
	if let Err(e) = client.import_block(Unverified::from_srlp(parent_block.srlp_bytes()).unwrap()) {
		panic!("error importing block which is valid by definition: {:?}", e);
	}

	last_header = view!(BlockView,&parent_block.srlp_bytes()).header();
	...
	let res = client.import_block(Unverified::from_srlp(block.srlp_bytes()).unwrap());
	if res.is_err() {
