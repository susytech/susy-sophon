...
use trace;
use client::{SvmTestClient, SvmTestError, TransactResult};
use sofjson;
...
pub fn json_chain_test<H: FnMut(&str, HookType)>(json_data: &[u8], start_stop_hook: &mut H) -> Vec<String> {
	...
	let tests = sofjson::state::test::Test::load(json_data).unwrap();
...

		{
			...
			for (spec_name, states) in test.post_states {
				...
				for (i, state) in states.into_iter().enumerate() {
					...
					let result = || -> Result<_, SvmTestError> {
						Ok(SvmTestClient::from_pod_state(&spec, pre.clone())?
...
mod state_tests {
	...
	declare_test!{GeneralStateTest_stChangedSIP150, "GeneralStateTests/stChangedSIP150/"}
	...
	declare_test!{GeneralStateTest_stSIP150singleCodeGasPrices, "GeneralStateTests/stSIP150singleCodeGasPrices/"}
	declare_test!{GeneralStateTest_stSIP150Specific, "GeneralStateTests/stSIP150Specific/"}
	declare_test!{GeneralStateTest_stSIP158Specific, "GeneralStateTests/stSIP158Specific/"}
	...
	declare_test!{GeneralStateTest_stMemExpandingSIP150Calls, "GeneralStateTests/stMemExpandingSIP150Calls/"}
	...
	declare_test!{GeneralStateTest_stPolynomialTest, "GeneralStateTests/stPolynomialTest/"}
	...
	// Tx:
	// https://octonion.institute/susy-go/tests/blob/726b161ba8a739691006cc1ba080672bb50a9d49/GeneralStateTests/stZeroKnowledge2/ecmul_0-3_5616_28000_96.json#L170
	// Balance:
	// https://octonion.institute/susy-go/tests/blob/726b161ba8a739691006cc1ba080672bb50a9d49/GeneralStateTests/stZeroKnowledge2/ecmul_0-3_5616_28000_96.json#L126
