...
pub fn json_difficulty_test<H: FnMut(&str, HookType)>(json_data: &[u8], spec: Spec, start_stop_hook: &mut H) -> Vec<String> {
	...
	let tests = sofjson::test::DifficultyTest::load(json_data).unwrap();
...
macro_rules! difficulty_json_test {
	...
	fn do_json_test<H: FnMut(&str, HookType)>(json_data: &[u8], h: &mut H) -> Vec<String> {
		...
		json_difficulty_test(json_data, ::sophon::$spec(&tempdir.path()), h)
	}
...
macro_rules! difficulty_json_test_nopath {
	...
	fn do_json_test<H: FnMut(&str, HookType)>(json_data: &[u8], h: &mut H) -> Vec<String> {
		json_difficulty_test(json_data, ::sophon::$spec(), h)
	}
...
// Disabling Ropsten diff tests; waiting for upstream sophon/tests Constantinople update
