...
impl From<sofjson::vm::Call> for CallCreate {
	fn from(c: sofjson::vm::Call) -> Self {
		let dst: Option<sofjson::hash::Address> = c.destination.into();
		CallCreate {
			data: c.data.into(),
...
fn do_json_test_for<H: FnMut(&str, HookType)>(vm_type: &VMType, json_data: &[u8], start_stop_hook: &mut H) -> Vec<String> {
	let tests = sofjson::vm::Test::load(json_data).unwrap();
	...
	for (name, vm) in tests.into_iter() {
		...
		let (res, callcreates) = {
			...
			let mut svm = vm_factory.create(params, &schedule, 0);
			let res = svm.exec(&mut ex).ok().expect("TestExt never trap; resume error never happens; qed");
		...
		let log_hash = {
			let mut srlp = RlpStream::new_list(substate.logs.len());
