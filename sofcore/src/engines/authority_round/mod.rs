...
use sofjson;
use machine::{AuxiliaryData, Call, SophonMachine};
use hash::keccak;
...
use srlp::{encode, Decodable, DecoderError, Encodable, RlpStream, Rlp};
use sophon_types::{H256, H520, Address, U128, U256};
...
impl From<sofjson::spec::AuthorityRoundParams> for AuthorityRoundParams {
	fn from(p: sofjson::spec::AuthorityRoundParams) -> Self {
...
impl EpochManager {
	...
	fn zoom_to(&mut self, client: &EngineClient, machine: &SophonMachine, validators: &ValidatorSet, header: &Header) -> bool {
...
impl EmptyStep {
	...
	fn verify(&self, validators: &ValidatorSet) -> Result<bool, Error> {
		let message = keccak(empty_step_srlp(self.step, &self.parent_hash));
		let correct_proposer = step_proposer(validators, &self.parent_hash, self.step);

		sofkey::verify_address(&correct_proposer, &self.signature.into(), &message)
			.map_err(|e| e.into())
	}

	fn author(&self) -> Result<Address, Error> {
		let message = keccak(empty_step_srlp(self.step, &self.parent_hash));
		let public = sofkey::recover(&self.signature.into(), &message)?;
		Ok(sofkey::public_to_address(&public))
	}
...
impl Encodable for EmptyStep {
	fn srlp_append(&self, s: &mut RlpStream) {
		let empty_step_srlp = empty_step_srlp(self.step, &self.parent_hash);
		s.begin_list(2)
			.append(&self.signature)
			.append_raw(&empty_step_srlp, 1);
	}
}

impl Decodable for EmptyStep {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		let signature = srlp.val_at(0)?;
		let empty_step_srlp = srlp.at(1)?;

		let step = empty_step_srlp.val_at(0)?;
		let parent_hash = empty_step_srlp.val_at(1)?;

		Ok(EmptyStep { signature, step, parent_hash })
	}
}

pub fn empty_step_full_srlp(signature: &H520, empty_step_srlp: &[u8]) -> Vec<u8> {
	let mut s = RlpStream::new_list(2);
	s.append(signature).append_raw(empty_step_srlp, 1);
	s.out()
}

pub fn empty_step_srlp(step: u64, parent_hash: &H256) -> Vec<u8> {
	let mut s = RlpStream::new_list(2);
...
impl super::EpochVerifier<SophonMachine> for EpochVerifier {
...
fn header_seal_hash(header: &Header, empty_steps_srlp: Option<&[u8]>) -> H256 {
	match empty_steps_srlp {
		Some(empty_steps_srlp) => {
			let mut message = header.bare_hash().to_vec();
			message.extend_from_slice(empty_steps_srlp);
			keccak(message)
		},
		None => {
			header.bare_hash()
...
fn header_step(header: &Header, empty_steps_transition: u64) -> Result<u64, ::srlp::DecoderError> {
...
fn header_signature(header: &Header, empty_steps_transition: u64) -> Result<Signature, ::srlp::DecoderError> {
	let expected_seal_fields = header_expected_seal_fields(header, empty_steps_transition);
...
// (i.e. header.number() >= self.empty_steps_transition).
fn header_empty_steps(header: &Header) -> Result<Vec<EmptyStep>, ::srlp::DecoderError> {
...
fn verify_external(header: &Header, validators: &ValidatorSet, empty_steps_transition: u64) -> Result<(), Error> {
	...
	let is_invalid_proposer = *header.author() != correct_proposer || {
		let empty_steps_srlp = if header.number() >= empty_steps_transition {
			Some(header_empty_steps_raw(header))
		} else {
			None
		};

		let header_seal_hash = header_seal_hash(header, empty_steps_srlp);
		!sofkey::verify_address(&correct_proposer, &proposer_signature, &header_seal_hash)?
	};
...
fn combine_proofs(signal_number: BlockNumber, set_proof: &[u8], finality_proof: &[u8]) -> Vec<u8> {
	let mut stream = ::srlp::RlpStream::new_list(3);
...
impl AuthorityRound {
	...
	pub fn new(our_params: AuthorityRoundParams, machine: SophonMachine) -> Result<Arc<Self>, Error> {
	...
	fn generate_empty_step(&self, parent_hash: &H256) {
		...
		let empty_step_srlp = empty_step_srlp(step, parent_hash);

		if let Ok(signature) = self.sign(keccak(&empty_step_srlp)).map(Into::into) {
			let message_srlp = empty_step_full_srlp(&signature, &empty_step_srlp);
			...
			self.broadcast_message(message_srlp);
...
impl Engine<SophonMachine> for AuthorityRound {
	...
	fn handle_message(&self, srlp: &[u8]) -> Result<(), EngineError> {
		fn fmt_err<T: ::std::fmt::Debug>(x: T) -> EngineError {
			EngineError::MalformedMessage(format!("{:?}", x))
		}

		let srlp = Rlp::new(srlp);
		let empty_step: EmptyStep = srlp.as_val().map_err(fmt_err)?;;
	...
	fn generate_seal(&self, block: &ExecutedBlock, parent: &Header) -> Seal {
		...
		if is_step_proposer(&*validators, header.parent_hash(), step, header.author()) {
			...
			let empty_steps_srlp = if header.number() >= self.empty_steps_transition {
				let empty_steps: Vec<_> = empty_steps.iter().map(|e| e.sealed()).collect();
				Some(::srlp::encode_list(&empty_steps))
			} else {
				None
			};

			if let Ok(signature) = self.sign(header_seal_hash(header, empty_steps_srlp.as_ref().map(|e| &**e))) {
				...
				if self.step.can_propose.compare_and_swap(true, false, AtomicOrdering::SeqCst) {
					...
					if let Some(empty_steps_srlp) = empty_steps_srlp {
						fields.push(empty_steps_srlp);
					}
	...
	fn signals_epoch_end(&self, header: &Header, aux: AuxiliaryData)
		-> super::EpochChange<SophonMachine>
	{
	...
	fn is_epoch_end(
		...
		for finalized_hash in finalized {
			if let Some(pending) = transition_store(*finalized_hash) {
				...
				let finality_proof = ::srlp::encode_list(&finality_proof);
	...
	fn epoch_verifier<'a>(&self, _header: &Header, proof: &'a [u8]) -> ConstructedVerifier<'a, SophonMachine> {
	...
	fn sign(&self, hash: H256) -> Result<Signature, Error> {
		Ok(self.signer.read()
			.as_ref()
			.ok_or(sofkey::Error::InvalidAddress)?
			.sign(hash)?
		)
	}
...
mod tests {
	...
	use sophon_types::{Address, H520, H256, U256};
	use sofkey::Signature;
	...
	use engines::{Seal, Engine, EngineError, SofEngine};
		...
		F: FnOnce(&mut AuthorityRoundParams),
	{
		...
		let machine = ::machine::SophonMachine::regular(c_params, Default::default());
	...
	fn empty_step(engine: &SofEngine, step: u64, parent_hash: &H256) -> EmptyStep {
		let empty_step_srlp = super::empty_step_srlp(step, parent_hash);
		let signature = engine.sign(keccak(&empty_step_srlp)).unwrap().into();
		let parent_hash = parent_hash.clone();
		EmptyStep { step, signature, parent_hash }
	}

	fn sealed_empty_step(engine: &SofEngine, step: u64, parent_hash: &H256) -> SealedEmptyStep {
		let empty_step_srlp = super::empty_step_srlp(step, parent_hash);
		let signature = engine.sign(keccak(&empty_step_srlp)).unwrap().into();
		SealedEmptyStep { signature, step }
	}

	fn set_empty_steps_seal(header: &mut Header, step: u64, block_signature: &sofkey::Signature, empty_steps: &[SealedEmptyStep]) {
		header.set_seal(vec![
			encode(&(step as usize)),
			encode(&(&**block_signature as &[u8])),
			::srlp::encode_list(&empty_steps),
		]);
	}
	...
	fn broadcast_empty_step_message() {
		...
		// spec starts with step 2
		let empty_step_srlp = encode(&empty_step(engine, 2, &genesis_header.hash()));

		// we've received the message
		assert!(notify.messages.read().contains(&empty_step_srlp));
	...
	fn seal_with_empty_steps() {
		...
		if let Seal::Regular(seal) = engine.generate_seal(b2.block(), &genesis_header) {
			...
			let empty_steps = ::srlp::encode_list(&vec![empty_step2]);
	...
	fn seal_empty_block_with_empty_steps() {
		...
		if let Seal::Regular(seal) = engine.generate_seal(b3.block(), &genesis_header) {
			...
			let empty_steps = ::srlp::encode_list(&vec![empty_step2, empty_step3]);
	...
	fn extra_info_from_seal() {
		...
		header.set_seal(vec![
			encode(&2usize),
			encode(&H520::default()),
			::srlp::encode_list(&vec![sealed_empty_step]),
		]);
