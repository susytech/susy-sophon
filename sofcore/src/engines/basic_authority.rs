...
pub struct BasicAuthorityParams {
	/// Valid signatories.
	pub validators: sofjson::spec::ValidatorSet,
}

impl From<sofjson::spec::BasicAuthorityParams> for BasicAuthorityParams {
	fn from(p: sofjson::spec::BasicAuthorityParams) -> Self {
...
fn verify_external(header: &Header, validators: &ValidatorSet) -> Result<(), Error> {
	...
	let signer = sofkey::public_to_address(&sofkey::recover(&sig.into(), &header.bare_hash())?);
...
impl BasicAuthority {
	...
	pub fn new(our_params: BasicAuthorityParams, machine: SophonMachine) -> Self {
		BasicAuthority {
			machine: machine,
...
impl Engine<SophonMachine> for BasicAuthority {
	...
	fn generate_seal(&self, block: &ExecutedBlock, _parent: &Header) -> Seal {
		...
		if self.validators.contains(header.parent_hash(), author) {
			...
			if let Ok(signature) = self.sign(header.bare_hash()) {
				return Seal::Regular(vec![::srlp::encode(&(&H520::from(signature) as &[u8]))]);
			} else {
	...
	fn epoch_verifier<'a>(&self, header: &Header, proof: &'a [u8]) -> ConstructedVerifier<'a, SophonMachine> {
		let first = header.number() == 0;
	...
	fn sign(&self, hash: H256) -> Result<Signature, Error> {
		Ok(self.signer.read()
			.as_ref()
			.ok_or_else(|| sofkey::Error::InvalidAddress)?
			.sign(hash)?
		)
	}
...
mod tests {
	...
	fn can_do_signature_verification_fail() {
		...
		header.set_seal(vec![::srlp::encode(&H520::default())]);
