...
use susy_machine::{Machine, LocalizedMachine as Localized, TotalScoredHeader};
use sophon_types::{H256, U256, Address};
use unexpected::{Mismatch, OutOfBounds};
use bytes::Bytes;
use types::ancestry_action::AncestryAction;

/// Default SIP-210 contract code.
/// As defined in https://octonion.institute/susy-go/SIPs/pull/210
...
/// A system-calling closure. Enacts calls on a block's state with code either from an on-chain contract, or hard-coded SVM or WASM (if enabled on-chain) codes.
...
pub fn default_system_or_code_call<'a>(machine: &'a ::machine::SophonMachine, block: &'a mut ::block::ExecutedBlock) -> impl FnMut(SystemOrCodeCallKind, Vec<u8>) -> Result<Vec<u8>, String> + 'a {
	move |to, data| {
		let result = match to {
...
/// Common type alias for an engine coupled with an Sophon-like state machine.
// TODO: make this a _trait_ alias when those exist.
// fortunately the effect is largely the same since engines are mostly used
// via trait objects.
pub trait SofEngine: Engine<::machine::SophonMachine> {
...
impl<T> SofEngine for T where T: Engine<::machine::SophonMachine> { }
