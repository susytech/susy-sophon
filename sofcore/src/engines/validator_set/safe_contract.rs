...
use machine::{AuxiliaryData, Call, SophonMachine, AuxiliaryRequest};
...
// TODO: sofabi should be able to generate this.
...
impl ::engines::StateDependentProof<SophonMachine> for StateProof {
	fn generate_proof(&self, caller: &Call) -> Result<Vec<u8>, String> {
		prove_initial(self.contract_address, &self.header, caller)
	}

	fn check_proof(&self, machine: &SophonMachine, proof: &[u8]) -> Result<(), String> {
...
fn check_first_proof(machine: &SophonMachine, contract_address: Address, old_header: Header, state_items: &[DBValue])
...
fn decode_first_proof(srlp: &Rlp) -> Result<(Header, Vec<DBValue>), ::error::Error> {
	let header = srlp.val_at(0)?;
	let state_items = srlp.at(1)?.iter().map(|x| {
...
fn decode_proof(srlp: &Rlp) -> Result<(Header, Vec<Receipt>), ::error::Error> {
	Ok((srlp.val_at(0)?, srlp.list_at(1)?))
}
...
impl ValidatorSet for ValidatorSafeContract {
	...
	fn signals_epoch_end(&self, first: bool, header: &Header, aux: AuxiliaryData)
		-> ::engines::EpochChange<SophonMachine>
	{
	...
	fn epoch_set(&self, first: bool, machine: &SophonMachine, _number: ::types::BlockNumber, proof: &[u8])
		-> Result<(SimpleList, Option<H256>), ::error::Error>
	{
		let srlp = Rlp::new(proof);

		if first {
			trace!(target: "engine", "Recovering initial epoch set");

			let (old_header, state_items) = decode_first_proof(&srlp)?;
			let number = old_header.number();
...
mod tests {
	...
	fn knows_validators() {
		...
		for i in 1..4 {
			sync_client.import_block(Unverified::from_srlp(client.block(BlockId::Number(i)).unwrap().into_inner()).unwrap()).unwrap();
		}
