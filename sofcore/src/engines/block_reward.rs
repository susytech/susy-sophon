...
use sofabi;
use sofabi::ParamType;
use sophon_types::{H160, Address, U256};

use std::sync::Arc;
use hash::keccak;
use error::Error;
use machine::WithRewards;
use susy_machine::Machine;
use trace;
...
impl BlockRewardContract {
	...
	pub fn reward(
		...
		// since this is a non-constant call we can't use sofabi's function output
		...
		let tokens = sofabi::decode(types, &output)
			.map_err(|err| err.to_string())
			.map_err(::engines::EngineError::FailedSystemCall)?;

		assert!(tokens.len() == 2);

		let addresses = tokens[0].clone().to_array().expect("type checked by sofabi::decode; qed");
		let rewards = tokens[1].clone().to_array().expect("type checked by sofabi::decode; qed");
		...
		let addresses = addresses.into_iter().map(|t| t.to_address().expect("type checked by sofabi::decode; qed"));
		let rewards = rewards.into_iter().map(|t| t.to_uint().expect("type checked by sofabi::decode; qed"));
