...
use hash::keccak;
use srlp::{RlpStream, Encodable, encode_list};
...
impl ::susy_machine::LiveBlock for ExecutedBlock {
	type Header = Header;
...
impl ::susy_machine::Transactions for ExecutedBlock {
...
impl<'x> OpenBlock<'x> {
	...
	///
	/// If valid, it will be executed, and archived togsophy with the receipt.
	...
	pub fn close_and_lock(self) -> Result<LockedBlock, Error> {
		...
		s.block.header.set_transactions_root(ordered_trie_root(s.block.transactions.iter().map(|e| e.srlp_bytes())));
		let uncle_bytes = encode_list(&s.block.uncles);
		s.block.header.set_uncles_hash(keccak(&uncle_bytes));
		s.block.header.set_state_root(s.block.state.root().clone());
		s.block.header.set_receipts_root(ordered_trie_root(s.block.receipts.iter().map(|r| r.srlp_bytes())));
...
impl ClosedBlock {
	...
	pub fn reopen(self, engine: &SofEngine) -> OpenBlock {
...
impl LockedBlock {
	...
	pub fn strip_receipts_outcomes(&mut self) {
		...
		self.block.header.set_receipts_root(
			ordered_trie_root(self.block.receipts.iter().map(|r| r.srlp_bytes()))
		);
	...
	pub fn seal(self, engine: &SofEngine, seal: Vec<Bytes>) -> Result<SealedBlock, BlockError> {
...
impl SealedBlock {
	/// Get the SRLP-encoding of the block.
	pub fn srlp_bytes(&self) -> Bytes {
		let mut block_srlp = RlpStream::new_list(3);
		block_srlp.append(&self.block.header);
		block_srlp.append_list(&self.block.transactions);
		block_srlp.append_list(&self.block.uncles);
		block_srlp.out()
	}
}
...
mod tests {
	...
	fn enact_bytes(
		...
		let block = Unverified::from_srlp(block_bytes)?;
	...
	fn enact_and_seal(
		...
		let header = Unverified::from_srlp(block_bytes.clone())?.header;
	...
	fn enact_block() {
		...
		assert_eq!(e.srlp_bytes(), orig_bytes);
