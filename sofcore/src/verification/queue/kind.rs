...
pub trait Kind: 'static + Sized + Send + Sync {
	...
	fn create(input: Self::Input, engine: &SofEngine, check_seal: bool) -> Result<Self::Unverified, (Self::Input, Error)>;

	/// Attempt to verify the `Unverified` item using the given engine.
	fn verify(unverified: Self::Unverified, engine: &SofEngine, check_seal: bool) -> Result<Self::Verified, Error>;
}
...
pub mod blocks {
	...
	impl Kind for Blocks {
		...
		fn verify(un: Self::Unverified, engine: &SofEngine, check_seal: bool) -> Result<Self::Verified, Error> {
			let hash = un.hash();
	...
	impl Unverified {
		...
		pub fn from_srlp(bytes: Bytes) -> Result<Self, ::srlp::DecoderError> {
			use srlp::Rlp;
			let (header, transactions, uncles) = {
				let srlp = Rlp::new(&bytes);
				let header = srlp.val_at(0)?;
				let transactions = srlp.list_at(1)?;
				let uncles = srlp.list_at(2)?;
