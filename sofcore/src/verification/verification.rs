...
pub fn verify_block_basic(block: &Unverified, engine: &SofEngine, check_seal: bool) -> Result<(), Error> {
...
/// Phase 2 verification. Perform costly checks such as transaction signatures and block nonce for sofash.
/// Still operates on a individual block
/// Returns a `PreverifiedBlock` structure populated with transactions
pub fn verify_block_unordered(block: Unverified, engine: &SofEngine, check_seal: bool) -> Result<PreverifiedBlock, Error> {
...
pub fn verify_block_family<C: BlockInfo + CallContract>(header: &Header, parent: &Header, engine: &SofEngine, do_full: Option<FullFamilyParams<C>>) -> Result<(), Error> {
...
fn verify_uncles(block: &PreverifiedBlock, bc: &BlockProvider, engine: &SofEngine) -> Result<(), Error> {
	let header = &block.header;
...
pub fn verify_header_params(header: &Header, engine: &SofEngine, is_full: bool, check_seal: bool) -> Result<(), Error> {
	if check_seal {
	...
	if let Some(ref ext) = engine.machine().sofash_extensions() {
...
fn verify_parent(header: &Header, parent: &Header, engine: &SofEngine) -> Result<(), Error> {
...
fn verify_block_integrity(block: &Unverified) -> Result<(), Error> {
	let block_srlp = Rlp::new(&block.bytes);
	let tx = block_srlp.at(1)?;
	...
	let expected_uncles = keccak(block_srlp.at(2)?.as_raw());
...
mod tests {
	...
	use sophon_types::{H256, BloomRef, U256};
	...
	impl TestBlockChain {
		...
		pub fn insert(&mut self, bytes: Bytes) {
			let header = Unverified::from_srlp(bytes.clone()).unwrap().header;
	...
	impl BlockProvider for TestBlockChain {
		...
		fn block_header_data(&self, hash: &H256) -> Option<encoded::Header> {
			self.block(hash)
				.map(|b| b.header_view().srlp().as_raw().to_vec())
		...
		fn block_details(&self, hash: &H256) -> Option<BlockDetails> {
			self.blocks.get(hash).map(|bytes| {
				let header = Unverified::from_srlp(bytes.to_vec()).unwrap().header;
				BlockDetails {
	...
	fn basic_test(bytes: &[u8], engine: &SofEngine) -> Result<(), Error> {
		let unverified = Unverified::from_srlp(bytes.to_vec())?;
		verify_block_basic(&unverified, engine, true)
	}

	fn family_test<BC>(bytes: &[u8], engine: &SofEngine, bc: &BC) -> Result<(), Error> where BC: BlockProvider {
		let block = Unverified::from_srlp(bytes.to_vec()).unwrap();
		let header = block.header;
	...
	fn unordered_test(bytes: &[u8], engine: &SofEngine) -> Result<(), Error> {
		let un = Unverified::from_srlp(bytes.to_vec())?;
	...
	fn test_verify_block_basic_with_invalid_transactions() {
		...
		let block = {
			let mut srlp = srlp::RlpStream::new_list(3);
			let mut header = Header::default();
			// that's an invalid transaction list srlp
			...
			srlp.append(&header);
			srlp.append_list::<Vec<u8>, _>(&invalid_transactions);
	...
	fn test_verify_block() {
		...
		let sip86_transactions = [ tr3.clone() ];
		...
		uncles_srlp.append_list(&good_uncles);
		let good_uncles_hash = keccak(uncles_srlp.as_raw());
		let good_transactions_root = ordered_trie_root(good_transactions.iter().map(|t| ::srlp::encode::<UnverifiedTransaction>(t)));
		let sip86_transactions_root = ordered_trie_root(sip86_transactions.iter().map(|t| ::srlp::encode::<UnverifiedTransaction>(t)));
		...
		bad_header.set_transactions_root(sip86_transactions_root.clone());
		bad_header.set_uncles_hash(good_uncles_hash.clone());
		match basic_test(&create_test_block_with_data(&bad_header, &sip86_transactions, &good_uncles), engine) {
			Err(Error(ErrorKind::Transaction(ref e), _)) if e == &::sofkey::Error::InvalidSignature.into() => (),
	...
	fn dust_protection() {
		...
		let machine = SophonMachine::regular(params, BTreeMap::new());
