...
use softrie::{TrieFactory, TrieDB, SecTrieDB, Result as TrieResult};
use pod_account::*;
use srlp::{RlpStream, encode};
...
impl Account {
	...
	/// Create a new account from SRLP.
	pub fn from_srlp(srlp: &[u8]) -> Result<Account, Error> {
		::srlp::decode::<BasicAccount>(srlp)
			.map(|ba| ba.into())
			.map_err(|e| e.into())
	}
	...
	fn get_and_cache_storage(storage_root: &H256, storage_cache: &mut LruCache<H256, H256>, db: &HashDB<KeccakHasher, DBValue>, key: &H256) -> TrieResult<H256> {
		...
		let panicky_decoder = |bytes:&[u8]| ::srlp::decode(&bytes).expect("decoding db value failed");
	...
	fn cached_moved_original_storage_at(&self, key: &H256) -> Option<H256> {
		// If storage root is empty SRLP, then early return zero value. Practically, this makes it so that if
...
#[cfg(test)]
mod tests {
	use srlp_compress::{compress, decompress, snapshot_swapper};
	...
	#[test]
	fn account_compress() {
		let raw = Account::new_basic(2.into(), 4.into()).srlp();
	...
	fn storage_at() {
		...
		let a = Account::from_srlp(&srlp).expect("decoding db value failed");
	...
	fn note_code() {
		...
		let mut a = Account::from_srlp(&srlp).expect("decoding db value failed");
	...
	fn srlpio() {
		...
		let b = Account::from_srlp(&a.srlp()).unwrap();
	...
	fn new_account() {
		let a = Account::new(69u8.into(), 0u8.into(), HashMap::new(), Bytes::new());
		assert_eq!(a.srlp().to_hex(), "f8448045a056e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421a0c5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470");
