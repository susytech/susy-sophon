...
use softrie::{TrieDB, Result as TrieResult};
...
///
/// `State` can work togsophy with `StateDB` to share account cache.
///
...
impl<B: Backend> State<B> {
	...
	pub fn new_contract(&mut self, contract: &Address, balance: U256, nonce_offset: U256) -> TrieResult<()> {
		...
		if overflow {
			return Err(Box::new(TrieError::DecoderError(H256::from(contract),
				srlp::DecoderError::Custom("Nonce overflow".into()))));
		}
	...
	pub fn exists(&self, a: &Address) -> TrieResult<bool> {
		...
		// check if account exists in the database directly before SIP-161 is in effect.
		...
		FStorageAt: Fn(&Account, &HashDB<KeccakHasher, DBValue>, &H256) -> TrieResult<H256>
	{
		...
		let from_srlp = |b: &[u8]| Account::from_srlp(b).expect("decoding db value failed");
		let maybe_acc = db.get_with(address, from_srlp)?;
		...
		V: trace::VMTracer,
	{
		...
		let sip658 = env_info.number >= params.sip658_transition;
		let no_intermediate_commits =
			sip658 ||
			(env_info.number >= params.sip98_transition && env_info.number >= params.validate_receipts_transition);
			...
			for (address, ref mut a) in accounts.iter_mut().filter(|&(_, ref a)| a.is_dirty()) {
				...
				match a.account {
					Some(ref mut account) => {
						trie.insert(address, &account.srlp())?;
					},
					None => {
						trie.remove(address)?;
					},
	...
	fn account_to_pod_account(&self, account: &Account, address: &Address) -> Result<PodAccount, Error> {
		...
		for o_kv in trie.iter()? {
			if let Ok((key, val)) = o_kv {
				pod_storage.insert(key[..].into(), srlp::decode::<U256>(&val[..]).expect("Decoded from trie which was encoded from the same type; qed").into());
			}
		}
	...
	fn ensure_cached<F, U>(&self, a: &Address, require: RequireCache, check_null: bool, f: F) -> TrieResult<U>
		...
		match result {
			...
			None => {
				...
				let from_srlp = |b: &[u8]| Account::from_srlp(b).expect("decoding db value failed");
				let mut maybe_acc = db.get_with(a, from_srlp)?;
			...
			match self.db.get_cached_account(a) {
				...
				None => {
					let maybe_acc = if !self.db.is_known_null(a) {
						...
						let db = self.factories.trie.readonly(db, &self.root)?;
						let from_srlp = |b:&[u8]| { Account::from_srlp(b).expect("decoding db value failed") };
						AccountEntry::new_clean(db.get_with(a, from_srlp)?)
					} else {
...
impl<B: Backend> State<B> {
	...
	pub fn prove_account(&self, account_key: H256) -> TrieResult<(Vec<Bytes>, BasicAccount)> {
		...
		let maybe_account: Option<BasicAccount> = {
			let panicky_decoder = |bytes: &[u8]| {
				::srlp::decode(bytes).expect(&format!("prove_account, could not query trie for account key={}", &account_key))
			};
	...
	pub fn prove_storage(&self, account_key: H256, storage_key: H256) -> TrieResult<(Vec<Bytes>, H256)> {
		...
		let trie = TrieDB::new(db, &self.root)?;
		let from_srlp = |b: &[u8]| Account::from_srlp(b).expect("decoding db value failed");
		let acc = match trie.get_with(&account_key, from_srlp)? {
			Some(acc) => acc,
...
mod tests {
	...
	fn should_not_trace_subcall_transaction_to_builtin() {
		...
		let expected_trace = vec![FlatTrace {
			...
			result: trace::Res::Call(trace::CallResult {
				gas_used: U256::from(3_721), // in post-sip150
				output: vec![]
			}),
			subtraces: 0,
		}];
	...
	fn should_trace_callcode_properly() {
		...
		let expected_trace = vec![FlatTrace {
			...
			result: trace::Res::Call(trace::CallResult {
				gas_used: 724.into(), // in post-sip150
				output: vec![]
			}),
		}, FlatTrace {
	...
	fn should_trace_delegatecall_properly() {
		...
		let expected_trace = vec![FlatTrace {
			...
			result: trace::Res::Call(trace::CallResult {
				gas_used: U256::from(736), // in post-sip150
				output: vec![]
			}),
		}, FlatTrace {
			trace_address: vec![0].into_iter().collect(),
