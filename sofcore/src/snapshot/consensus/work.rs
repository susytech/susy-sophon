...
struct PowWorker<'a> {
	chain: &'a BlockChain,
	// block, receipt srlp pairs.
	srlps: VecDeque<Bytes>,
	current_hash: H256,
...
impl<'a> PowWorker<'a> {
	...
	fn chunk_all(&mut self, snapshot_blocks: u64) -> Result<(), Error> {
		...
		for _ in 0..snapshot_blocks {
			...
			let abridged_srlp = AbridgedBlock::from_block_view(&block.view()).into_inner();

			let pair = {
				let mut pair_stream = RlpStream::new_list(2);
				pair_stream.append_raw(&abridged_srlp, 1).append(&receipts);
				pair_stream.out()
			};
			...
			if new_loaded_size > self.preferred_size && !self.srlps.is_empty() {
	...
	fn write_chunk(&mut self, last: H256) -> Result<(), Error> {
		trace!(target: "snapshot", "prepared block chunk with {} blocks", self.srlps.len());
		...
		let num_entries = self.srlps.len();
		let mut srlp_stream = RlpStream::new_list(3 + num_entries);
		srlp_stream.append(&parent_number).append(&parent_hash).append(&parent_total_difficulty);

		for pair in self.srlps.drain(..) {
			srlp_stream.append_raw(&pair, 1);
		}

		let raw_data = srlp_stream.out();
...
///
/// After all chunks have been submitted, we "glue" the chunks togsophy.
...
impl Rebuilder for PowRebuilder {
	...
	fn feed(&mut self, chunk: &[u8], engine: &SofEngine, abort_flag: &AtomicBool) -> Result<(), ::error::Error> {
		use snapshot::verify_old_block;
		use sophon_types::U256;
		use triehash::ordered_trie_root;

		let srlp = Rlp::new(chunk);
		let item_count = srlp.item_count()?;
		...
		let mut cur_number = srlp.val_at::<u64>(0)? + 1;
		let mut parent_hash = srlp.val_at::<H256>(1)?;
		let parent_total_difficulty = srlp.val_at::<U256>(2)?;

		for idx in 3..item_count {
			if !abort_flag.load(Ordering::SeqCst) { return Err(Error::RestorationAborted.into()) }

			let pair = srlp.at(idx)?;
			let abridged_srlp = pair.at(0)?.as_raw().to_owned();
			let abridged_block = AbridgedBlock::from_raw(abridged_srlp);
			...
			let block_bytes = encoded::Block::new(block.srlp_bytes());
	...
	/// Glue togsophy any disconnected chunks and check that the chain is complete.
	fn finalize(&mut self, _: &SofEngine) -> Result<(), ::error::Error> {
		let mut batch = self.db.transaction();
