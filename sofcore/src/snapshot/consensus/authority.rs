...
use engines::{SofEngine, EpochVerifier, EpochTransition};
...
impl SnapshotComponents for PoaSnapshot {
	fn chunk_all(
			...
			.take_while(|&(_, ref t)| t.block_number <= number)
		{
			...
			pending_size = if new_loaded_size > preferred_size && !srlps.is_empty() {
				write_chunk(false, &mut srlps, sink)?;
				entry.len()
			} else {
				new_loaded_size
			};
		...
		write_chunk(true, &mut srlps, sink)?;
...
// writes a chunk composed of the inner SRLPs here.
...
struct ChunkRebuilder {
	...
	last_epochs: Vec<(Header, Box<EpochVerifier<SophonMachine>>)>,
}
...
impl ChunkRebuilder {
	fn verify_transition(
		&mut self,
		last_verifier: &mut Option<Box<EpochVerifier<SophonMachine>>>,
		transition_srlp: Rlp,
		engine: &SofEngine,
		...
		// decode.
		let header: Header = transition_srlp.val_at(0)?;
		let epoch_data: Bytes = transition_srlp.val_at(1)?;
...
impl Rebuilder for ChunkRebuilder {
	fn feed(
		...
		let is_last_chunk: bool = srlp.val_at(0)?;
		let num_items = srlp.item_count()?;
		...
		for transition_srlp in srlp.iter().skip(1).take(num_transitions).with_position() {
			if !abort_flag.load(Ordering::SeqCst) { return Err(Error::RestorationAborted.into()) }

			let (is_first, is_last) = match transition_srlp {
			...
			let transition_srlp = transition_srlp.into_inner();
		...
		if is_last_chunk {
			...
			let last_srlp = srlp.at(num_items - 1)?;
			let block = Block {
				header: last_srlp.val_at(0)?,
				transactions: last_srlp.list_at(1)?,
				uncles: last_srlp.list_at(2)?,
			};
			let block_data = block.srlp_bytes();
			let receipts: Vec<Receipt> = last_srlp.list_at(3)?;
			...
			let parent_td: U256 = last_srlp.val_at(4)?;
	...
	fn finalize(&mut self, _engine: &SofEngine) -> Result<(), ::error::Error> {
		if !self.had_genesis {
