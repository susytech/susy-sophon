...
use susykv_rocksdb::{Database, DatabaseConfig};
...
fn get_code_from_prev_chunk() {
	...
	let thin_srlp = acc_stream.out();
	let acc: BasicAccount = ::srlp::decode(&thin_srlp).expect("error decoding basic account");

	let mut make_chunk = |acc, hash| {
		let mut db = journaldb::new_memory_db();
		AccountDBMut::from_hash(&mut db, hash).insert(&code[..]);

		let fat_srlp = account::to_fat_srlps(&hash, &acc, &AccountDB::from_hash(&db, hash), &mut used_code, usize::max_value(), usize::max_value()).unwrap();
		let mut stream = RlpStream::new_list(1);
		stream.append_raw(&fat_srlp[0], 1);
		stream.out()
	};
