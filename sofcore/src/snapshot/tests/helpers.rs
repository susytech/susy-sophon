...
use softrie::{SecTrieDBMut, TrieDB, TrieDBMut};
...
impl StateProducer {
	...
	pub fn tick<R: Rng>(&mut self, rng: &mut R, db: &mut HashDB<KeccakHasher, DBValue>) {
		...
		for &mut (ref mut address_hash, ref mut account_data) in &mut accounts_to_modify {
			let mut account: BasicAccount = ::srlp::decode(&*account_data).expect("error decoding basic account");
			let acct_db = AccountDBMut::from_hash(db, *address_hash);
			fill_storage(acct_db, &mut account.storage_root, &mut self.storage_seed);
			*account_data = DBValue::from_vec(::srlp::encode(&account));
		}
		...
		for _ in 0..new_accs {
			...
			let acc = ::state::Account::new_basic(balance.into(), nonce.into()).srlp();
