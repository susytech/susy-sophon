...
impl Restoration {
	...
	// feeds a block chunk
	fn feed_blocks(&mut self, hash: H256, chunk: &[u8], engine: &SofEngine, flag: &AtomicBool) -> Result<(), Error> {
	...
	fn finalize(mut self, engine: &SofEngine) -> Result<(), Error> {
		use trie::TrieError;
...
mod tests {
	...
	fn cannot_finish_with_invalid_chunks() {
		use sophon_types::H256;
		use susykv_rocksdb::DatabaseConfig;
