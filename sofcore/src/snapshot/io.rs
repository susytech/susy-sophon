...
impl SnapshotWriter for PackedWriter {
	...
	fn finish(mut self, manifest: ManifestData) -> io::Result<()> {
		...
		let manifest_srlp = stream.out();

		self.file.write_all(&manifest_srlp)?;
		let off = self.cur_len;
		trace!(target: "snapshot_io", "writing manifest of len {} to offset {}", manifest_srlp.len(), off);
...
impl PackedReader {
	...
	pub fn new(path: &Path) -> Result<Option<Self>, ::snapshot::error::Error> {
		...
		let srlp = Rlp::new(&manifest_buf);

		let (start, version) = if srlp.item_count()? == 5 {
			(0, 1)
		} else {
			(1, srlp.val_at(0)?)
		};
		...
		let state: Vec<ChunkInfo> = srlp.list_at(0 + start)?;
		let blocks: Vec<ChunkInfo> = srlp.list_at(1 + start)?;

		let manifest = ManifestData {
			version: version,
			state_hashes: state.iter().map(|c| c.0).collect(),
			block_hashes: blocks.iter().map(|c| c.0).collect(),
			state_root: srlp.val_at(2 + start)?,
			block_number: srlp.val_at(3 + start)?,
			block_hash: srlp.val_at(4 + start)?,
		};
...
impl LooseReader {
	...
	pub fn new(mut dir: PathBuf) -> Result<Self, ::error::Error> {
		...
		let manifest = ManifestData::from_srlp(&manifest_buf[..])?;
