...
//! https://wiki.superstring.io/Warp-Sync-Snapshot-Format
...
impl<'a> StateChunker<'a> {
	...
	fn write_chunk(&mut self) -> Result<(), Error> {
		...
		for srlp in self.srlps.drain(..) {
			stream.append_raw(&srlp, 1);
		}
...
pub fn chunk_state<'a>(db: &HashDB<KeccakHasher, DBValue>, root: &H256, writer: &Mutex<SnapshotWriter + 'a>, progress: &'a Progress, part: Option<usize>) -> Result<Vec<H256>, Error> {
	...
	for item in account_iter {
		...
		let account = ::srlp::decode(&*account_data)?;
		let account_db = AccountDB::from_hash(db, account_key_hash);

		let fat_srlps = account::to_fat_srlps(&account_key_hash, &account, &account_db, &mut used_code, PREFERRED_CHUNK_SIZE - chunker.chunk_size(), PREFERRED_CHUNK_SIZE)?;
		for (i, fat_srlp) in fat_srlps.into_iter().enumerate() {
			if i > 0 {
				chunker.write_chunk()?;
			}
			chunker.push(fat_srlp)?;
		}
	}
...
impl StateRebuilder {
	...
	pub fn feed(&mut self, chunk: &[u8], flag: &AtomicBool) -> Result<(), ::error::Error> {
		...
		let empty_srlp = StateAccount::new_basic(U256::zero(), U256::zero()).srlp();
		let mut pairs = Vec::with_capacity(srlp.item_count()?);

		// initialize the pairs vector with empty values so we have slots to write into.
		pairs.resize(srlp.item_count()?, (H256::new(), Vec::new()));
		...
		// batch trie writes
		{
			...
			for (hash, thin_srlp) in pairs {
				if !flag.load(Ordering::SeqCst) { return Err(Error::RestorationAborted.into()) }

				if &thin_srlp[..] != &empty_srlp[..] {
					self.bloom.set(&*hash);
				}
				account_trie.insert(&hash, &thin_srlp)?;
			}
		}
...
fn rebuild_accounts(
	...
	for (account_srlp, out) in account_fat_srlps.into_iter().zip(out_chunk.iter_mut()) {
		if !abort_flag.load(Ordering::SeqCst) { return Err(Error::RestorationAborted.into()) }

		let hash: H256 = account_srlp.val_at(0)?;
		let fat_srlp = account_srlp.at(1)?;
		...
		let thin_srlp = {
			...
			let (acc, maybe_code) = {
				...
				account::from_fat_srlp(&mut acct_db, fat_srlp, storage_root)?
			};
	...
	if let Some(&(ref hash, ref srlp)) = out_chunk.iter().last() {
		known_storage_roots.insert(*hash, ::srlp::decode::<BasicAccount>(srlp)?.storage_root);
	}
	if let Some(&(ref hash, ref srlp)) = out_chunk.iter().next() {
		known_storage_roots.insert(*hash, ::srlp::decode::<BasicAccount>(srlp)?.storage_root);
	}
	Ok(status)
}
...
pub fn verify_old_block(rng: &mut OsRng, header: &Header, engine: &SofEngine, chain: &BlockChain, always: bool) -> Result<(), ::error::Error> {
