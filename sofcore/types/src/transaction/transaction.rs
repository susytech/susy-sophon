...
use sophon_types::{H256, H160, Address, U256};
use sofjson;
use sofkey::{self, Signature, Secret, Public, recover, public_to_address};
use hash::keccak;
use heapsize::HeapSizeOf;
use srlp::{self, RlpStream, Rlp, DecoderError, Encodable};
...
/// Fake address for unsigned transactions as defined by SIP-86.
...
impl srlp::Decodable for Action {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		if srlp.is_empty() {
			Ok(Action::Create)
		} else {
			Ok(Action::Call(srlp.as_val()?))
		}
	}
}

impl srlp::Encodable for Action {
...
impl Transaction {
	/// Append object with a without signature into SRLP stream
	pub fn srlp_append_unsigned_transaction(&self, s: &mut RlpStream, chain_id: Option<u64>) {
...
impl From<sofjson::state::Transaction> for SignedTransaction {
	fn from(t: sofjson::state::Transaction) -> Self {
		let to: Option<sofjson::hash::Address> = t.to.into();
...
impl From<sofjson::transaction::Transaction> for UnverifiedTransaction {
	fn from(t: sofjson::transaction::Transaction) -> Self {
		let to: Option<sofjson::hash::Address> = t.to.into();
...
impl Transaction {
	...
	pub fn hash(&self, chain_id: Option<u64>) -> H256 {
		...
		self.srlp_append_unsigned_transaction(&mut stream, chain_id);
		keccak(stream.as_raw())
	}

	/// Signs the transaction as coming from `sender`.
	pub fn sign(self, secret: &Secret, chain_id: Option<u64>) -> SignedTransaction {
		let sig = ::sofkey::sign(secret, &self.hash(chain_id))
	...
	/// Add SIP-86 compatible empty signature.
...
impl srlp::Decodable for UnverifiedTransaction {
...
impl srlp::Encodable for UnverifiedTransaction {
	fn srlp_append(&self, s: &mut RlpStream) { self.srlp_append_sealed_transaction(s) }
}

impl UnverifiedTransaction {
	/// Used to compute hash of created transactions
	fn compute_hash(mut self) -> UnverifiedTransaction {
		let hash = keccak(&*self.srlp_bytes());
		self.hash = hash;
		self
	}
	...
	/// Append object with a signature into SRLP stream
	fn srlp_append_sealed_transaction(&self, s: &mut RlpStream) {
		s.begin_list(9);
		s.append(&self.nonce);
		s.append(&self.gas_price);
		s.append(&self.gas);
	...
	/// The `v` value that appears in the SRLP.
	...
	pub fn check_low_s(&self) -> Result<(), sofkey::Error> {
		if !self.signature().is_low_s() {
			Err(sofkey::Error::InvalidSignature.into())
		} else {
			Ok(())
		}
	}

	/// Get the hash of this transaction (keccak of the SRLP).
	pub fn hash(&self) -> H256 {
		self.hash
	}

	/// Recovers the public key of the sender.
	pub fn recover_public(&self) -> Result<Public, sofkey::Error> {
	...
	pub fn verify_basic(&self, check_low_s: bool, chain_id: Option<u64>, allow_empty_signature: bool) -> Result<(), error::Error> {
		...
		// Disallow unsigned transactions in case SIP-86 is disabled.
		if !allow_empty_signature && self.is_unsigned() {
			return Err(sofkey::Error::InvalidSignature.into());
		}
		// SIP-86: Transactions of this form MUST have gasprice = 0, nonce = 0, value = 0, and do NOT increment the nonce of account 0.
...
impl srlp::Encodable for SignedTransaction {
	fn srlp_append(&self, s: &mut RlpStream) { self.transaction.srlp_append_sealed_transaction(s) }
}
...
impl SignedTransaction {
	...
	pub fn new(transaction: UnverifiedTransaction) -> Result<Self, sofkey::Error> {
...
mod tests {
	...
	fn sender_test() {
		...
		let t: UnverifiedTransaction = srlp::decode(&bytes).expect("decoding UnverifiedTransaction failed");
		assert_eq!(t.data, b"");
	...
	#[test]
	fn signing_sip155_zero_chainid() {
		...
		let sig = ::sofkey::sign(&key.secret(), &hash).unwrap();
	...
	fn should_agree_with_vitalik() {
		...
		let test_vector = |tx_data: &str, address: &'static str| {
			let signed = srlp::decode(&FromHex::from_hex(tx_data).unwrap()).expect("decoding tx data failed");
