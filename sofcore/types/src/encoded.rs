...
//! Lazily-decoded owning views of SRLP-encoded blockchain objects.
...
impl Header {
	...
	/// Expects the data to be an SRLP-encoded header -- any other case will likely lead to
	...
	pub fn decode(&self) -> Result<FullHeader, srlp::DecoderError> {
		srlp::decode(&self.0)
	}
	...
	/// Get the srlp of the header.
	#[inline]
	pub fn srlp(&self) -> Rlp { Rlp::new(&self.0) }
...
impl Body {
	/// Create a new owning block body view. The raw bytes passed in must be an srlp-encoded block
	/// body.
	...
	/// Get the SRLP of this block body.
	#[inline]
	pub fn srlp(&self) -> Rlp {
		Rlp::new(&self.0)
	}
...
// forwarders to borrowed view.
impl Body {
	/// Get raw srlp of transactions
	pub fn transactions_srlp(&self) -> Rlp { self.view().transactions_srlp().srlp }
	...
	/// Get raw srlp of uncle headers
	pub fn uncles_srlp(&self) -> Rlp { self.view().uncles_srlp().srlp }
...
impl Block {
	/// Create a new owning block view. The raw bytes passed in must be an srlp-encoded block.
	...
	pub fn new_from_header_and_body(header: &views::HeaderView, body: &views::BodyView) -> Self {
		...
		stream.append_raw(header.srlp().as_raw(), 1);
		stream.append_raw(body.transactions_srlp().as_raw(), 1);
		stream.append_raw(body.uncles_srlp().as_raw(), 1);
	...
	pub fn decode(&self) -> Result<FullBlock, srlp::DecoderError> { srlp::decode(&self.0) }

	/// Decode the header.
	pub fn decode_header(&self) -> FullHeader { self.view().srlp().val_at(0) }

	/// Clone the encoded header.
	pub fn header(&self) -> Header { Header(self.view().srlp().at(0).as_raw().to_vec()) }

	/// Get the srlp of this block.
	#[inline]
	pub fn srlp(&self) -> Rlp {
		Rlp::new(&self.0)
	}
