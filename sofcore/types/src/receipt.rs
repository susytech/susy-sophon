...
use sophon_types::{H160, H256, U256, Address, Bloom};
use heapsize::HeapSizeOf;
use srlp::{Rlp, RlpStream, Encodable, Decodable, DecoderError};
...
pub enum TransactionOutcome {
	/// Status and state root are unknown under SIP-98 rules.
	Unknown,
	/// State root is known. Pre SIP-98 and SIP-658 rules.
	StateRoot(H256),
	/// Status code is known. SIP-658 rules.
	StatusCode(u8),
}
	...
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		if srlp.item_count()? == 3 {
			Ok(Receipt {
				...
				gas_used: srlp.val_at(0)?,
				log_bloom: srlp.val_at(1)?,
				logs: srlp.list_at(2)?,
			})
		} else {
			Ok(Receipt {
				gas_used: srlp.val_at(1)?,
				log_bloom: srlp.val_at(2)?,
				logs: srlp.list_at(3)?,
				outcome: {
					let first = srlp.at(0)?;
...
mod tests {
	...
	fn test_no_state_root() {
		...
		assert_eq!(&::srlp::encode(&r)[..], &expected[..]);
	}
	...
	fn test_basic() {
		...
		let encoded = ::srlp::encode(&r);
		assert_eq!(&encoded[..], &expected[..]);
		let decoded: Receipt = ::srlp::decode(&encoded).expect("decoding receipt failed");
		assert_eq!(decoded, r);
	}
