...
use sophon_types::{H256, U256, Address, Bloom};
use bytes::Bytes;
use srlp::{Rlp, RlpStream, Encodable, DecoderError, Decodable};
use BlockNumber;
...
/// A block header.
///
/// Reflects the specific SRLP fields of a block in the chain with additional room for the seal
...
pub struct Header {
	...
	/// Block difficulty.
	difficulty: U256,
	/// Vector of post-SRLP-encoded fields.
	seal: Vec<Bytes>,
...
impl Header {
	...
	/// Get & memoize the hash of this header (keccak of the SRLP with seal).
	...
	/// Get the hash of this header (keccak of the SRLP with seal).
	pub fn hash(&self) -> H256 {
		self.hash.unwrap_or_else(|| keccak(self.srlp(Seal::With)))
	}

	/// Get the hash of the header excluding the seal
	pub fn bare_hash(&self) -> H256 {
		keccak(self.srlp(Seal::Without))
	}

	/// Encode the header, getting a type-safe wrapper around the SRLP.
	pub fn encoded(&self) -> ::encoded::Header {
		::encoded::Header::new(self.srlp(Seal::With))
	}

	/// Get the SRLP representation of this Header.
	fn srlp(&self, with_seal: Seal) -> Bytes {
		let mut s = RlpStream::new();
		self.stream_srlp(&mut s, with_seal);
		s.out()
	}

	/// Place this header into an SRLP stream `s`, optionally `with_seal`.
	fn stream_srlp(&self, s: &mut RlpStream, with_seal: Seal) {
...
impl ::susy_machine::Header for Header {
...
impl ::susy_machine::ScoredHeader for Header {
	type Value = U256;
...
impl ::susy_machine::Header for ExtendedHeader {
...
impl ::susy_machine::ScoredHeader for ExtendedHeader {
	type Value = U256;
...
impl ::susy_machine::TotalScoredHeader for ExtendedHeader {
	type Value = U256;

	fn total_score(&self) -> U256 { self.parent_total_difficulty + *self.header.difficulty() }
}

impl ::susy_machine::FinalizableHeader for ExtendedHeader {
...
mod tests {
	...
	fn test_header_seal_fields() {
		// that's srlp of block header created with sofash engine.
		let header_srlp = "".from_hex().unwrap();
		...
		let header: Header = srlp::decode(&header_srlp).expect("error decoding header");
	...
	fn decode_and_encode_header() {
		...
		let header: Header = srlp::decode(&header_srlp).expect("error decoding header");
		let encoded_header = srlp::encode(&header);

		assert_eq!(header_srlp, encoded_header);
	}
	...
	fn reject_header_with_large_timestamp() {
		...
		let header_srlp = "".from_hex().unwrap();

		// This should fail decoding timestamp
		let header: Result<Header, _> = srlp::decode(&header_srlp);
		assert_eq!(header.unwrap_err(), srlp::DecoderError::RlpIsTooBig);
	}
}
