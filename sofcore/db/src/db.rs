...
pub trait Writable {
	...
	fn write<T, R>(&mut self, col: Option<u32>, key: &Key<T, Target = R>, value: &T) where T: srlp::Encodable, R: Deref<Target = [u8]>;

	/// Deletes key from the databse.
	fn delete<T, R>(&mut self, col: Option<u32>, key: &Key<T, Target = R>) where T: srlp::Encodable, R: Deref<Target = [u8]>;
...
impl Writable for DBTransaction {
	fn write<T, R>(&mut self, col: Option<u32>, key: &Key<T, Target = R>, value: &T) where T: srlp::Encodable, R: Deref<Target = [u8]> {
		self.put(col, &key.key(), &srlp::encode(value));
	}
...
impl<KVDB: KeyValueDB + ?Sized> Readable for KVDB {
	fn read<T, R>(&self, col: Option<u32>, key: &Key<T, Target = R>) -> Option<T>
		where T: srlp::Decodable, R: Deref<Target = [u8]> {
		self.get(col, &key.key())
			.expect(&format!("db get failed, key: {:?}", &key.key() as &[u8]))
			.map(|v| srlp::decode(&v).expect("decode db value failed") )
