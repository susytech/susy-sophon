...
use susykv::PREFIX_LEN as DB_PREFIX_LEN;
use srlp;
use srlp_derive::{RlpEncodableWrapper, RlpDecodableWrapper, RlpEncodable, RlpDecodable};
...
impl srlp::Encodable for BlockDetails {
	fn srlp_append(&self, stream: &mut srlp::RlpStream) {
...
impl srlp::Decodable for BlockDetails {
	fn decode(srlp: &srlp::Rlp) -> Result<Self, srlp::DecoderError> {
		let use_short_version = match srlp.item_count()? {
			4 => true,
			5 => false,
			_ => return Err(srlp::DecoderError::RlpIncorrectListLen),
		};

		Ok(BlockDetails {
			number: srlp.val_at(0)?,
			total_difficulty: srlp.val_at(1)?,
			parent: srlp.val_at(2)?,
			children: srlp.list_at(3)?,
