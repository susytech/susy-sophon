...
use tests::snapshot::*;
use sofcore::client::{TestBlockChainClient, BlockChainClient, Client as SofcoreClient,
...
use chain::{ChainSync, SyncSupplier, SOF_PROTOCOL_VERSION_63, PAR_PROTOCOL_VERSION_3};
...
impl FlushingBlockChainClient for SofcoreClient {
	fn flush(&self) {
		self.flush_queue();
	}
}
...
impl<'p, C> SyncIo for TestIo<'p, C> where C: FlushingBlockChainClient, C: 'p {
	...
	fn sof_protocol_version(&self, _peer: PeerId) -> u8 {
		SOF_PROTOCOL_VERSION_63.0
	}

	fn protocol_version(&self, protocol: &ProtocolId, peer_id: PeerId) -> u8 {
		if protocol == &WARP_SYNC_PROTOCOL_ID { PAR_PROTOCOL_VERSION_3.0 } else { self.sof_protocol_version(peer_id) }
	}
...
pub struct SofPeer<C> where C: FlushingBlockChainClient {
	pub chain: Arc<C>,
	pub miner: Arc<Miner>,
...
impl<C> SofPeer<C> where C: FlushingBlockChainClient {
...
impl<C: FlushingBlockChainClient> Peer for SofPeer<C> {
...
impl TestNet<SofPeer<TestBlockChainClient>> {
	...
	pub fn new_with_config(n: usize, config: SyncConfig) -> Self {
		...
		for _ in 0..n {
			...
			net.peers.push(Arc::new(SofPeer {
	...
	pub fn peer_mut(&mut self, i: usize) -> &mut SofPeer<TestBlockChainClient> {
		Arc::get_mut(&mut self.peers[i]).expect("Arc never exposed externally")
	}
}

impl TestNet<SofPeer<SofcoreClient>> {
	pub fn with_spec<F>(
		n: usize,
		config: SyncConfig,
		spec_factory: F,
...
impl<C: FlushingBlockChainClient> TestNet<SofPeer<C>> {
...
pub struct TestIoHandler {
	pub client: Arc<SofcoreClient>,
	pub private_tx_queued: Mutex<usize>,
}

impl TestIoHandler {
	pub fn new(client: Arc<SofcoreClient>) -> Self {
		TestIoHandler {
			client,
...
impl ChainNotify for SofPeer<SofcoreClient> {
