...
use sofcore::client::{ClientIoMessage, BlockChainClient};
use sofcore::executive::{contract_address};
use sofcore::engines;
use sofcore::miner::{self, MinerService};
use sofcore::spec::Spec;
use sofcore::test_helpers::{push_block_with_transactions};
use sofcore_private_tx::{Provider, ProviderConfig, NoopEncryptor, Importer, SignedPrivateTransaction, StoringKeyProvider};
use sofkey::KeyPair;
...
fn send_private_transaction() {
	...
	let signer = Arc::new(sofcore_private_tx::KeyPairSigner(vec![s0.clone(), s1.clone()]));
