...
use sofcore::client::{BlockStatus, BlockId};
use sofcore::error::{ImportErrorKind, QueueErrorKind, BlockError, Error as SofcoreError, ErrorKind as SofcoreErrorKind};
use sync_io::SyncIo;
...
const MAX_BODIES_TO_REQUEST_SMALL: usize = 32; // Size request for susy clients prior to 2.4.0
...
impl From<srlp::DecoderError> for BlockDownloaderImportError {
	fn from(_: srlp::DecoderError) -> BlockDownloaderImportError {
...
impl BlockDownloader {
	...
	pub fn import_headers(&mut self, io: &mut SyncIo, r: &Rlp, expected_hash: H256) -> Result<DownloadAction, BlockDownloaderImportError> {
		...
		for i in 0..item_count {
			let info = SyncHeader::from_srlp(r.at(i)?.as_raw().to_vec())?;
	...
	pub fn import_bodies(&mut self, r: &Rlp, expected_hashes: &[H256]) -> Result<(), BlockDownloaderImportError> {
		...
		if item_count == 0 {
			...
			for i in 0..item_count {
				let body = SyncBody::from_srlp(r.at(i)?.as_raw())?;
				bodies.push(body);
			}
		...
		if item_count == 0 {
			...
			for i in 0..item_count {
				let receipt = r.at(i).map_err(|e| {
					trace_sync!(self, "Error decoding block receipts SRLP: {:?}", e);
					BlockDownloaderImportError::Invalid
				})?;
	...
	pub fn collect_blocks(&mut self, io: &mut SyncIo, allow_out_of_order: bool) -> DownloadAction {
		...
		for block_and_receipts in blocks {
			...
			match result {
				Err(SofcoreError(SofcoreErrorKind::Import(ImportErrorKind::AlreadyInChain), _)) => {
					trace_sync!(self, "Block already in chain {:?}", h);
					self.block_imported(&h, number, &parent);
				},
				Err(SofcoreError(SofcoreErrorKind::Import(ImportErrorKind::AlreadyQueued), _)) => {
				...
				Err(SofcoreError(SofcoreErrorKind::Block(BlockError::UnknownParent(_)), _)) if allow_out_of_order => {
					break;
				},
				Err(SofcoreError(SofcoreErrorKind::Block(BlockError::UnknownParent(_)), _)) => {
					trace_sync!(self, "Unknown new block parent, restarting sync");
					break;
				},
				Err(SofcoreError(SofcoreErrorKind::Block(BlockError::TemporarilyInvalid(_)), _)) => {
					debug_sync!(self, "Block temporarily invalid: {:?}, restarting sync", h);
					break;
				},
				Err(SofcoreError(SofcoreErrorKind::Queue(QueueErrorKind::Full(limit)), _)) => {
...
mod tests {
	use super::*;
	use sofcore::client::TestBlockChainClient;
	use sofcore::spec::Spec;
	use sofkey::{Generator,Random};
	use hash::keccak;
	use parking_lot::RwLock;
	use srlp::{encode_list,RlpStream};
	use tests::helpers::TestIo;
	use tests::snapshot::TestSnapshotService;
	use types::transaction::{Transaction,SignedTransaction};
	use triehash_sophon::ordered_trie_root;
	...
	fn import_headers(headers: &[BlockHeader], downloader: &mut BlockDownloader, io: &mut SyncIo) -> Result<DownloadAction, BlockDownloaderImportError> {
		...
		let srlp = Rlp::new(&bytes);
		let expected_hash = headers.first().unwrap().hash();
		downloader.import_headers(io, &srlp, expected_hash)
	}
	...
	fn import_headers_in_chain_head_state() {
		...
		let srlp_data = encode_list(&valid_headers);
		let valid_srlp = Rlp::new(&srlp_data);

		match downloader.import_headers(&mut io, &valid_srlp, genesis_hash) {
		...
		let srlp_data = encode_list(&invalid_start_block_headers);
		let invalid_start_block_srlp = Rlp::new(&srlp_data);

		match downloader.import_headers(&mut io, &invalid_start_block_srlp, genesis_hash) {
		...
		let srlp_data = encode_list(&invalid_skip_headers);
		let invalid_skip_srlp = Rlp::new(&srlp_data);

		match downloader.import_headers(&mut io, &invalid_skip_srlp, genesis_hash) {
			Err(BlockDownloaderImportError::Invalid) => (),
			_ => panic!("expected BlockDownloaderImportError"),
		};
		...
		let srlp_data = encode_list(&too_many_headers);

		let too_many_srlp = Rlp::new(&srlp_data);
		match downloader.import_headers(&mut io, &too_many_srlp, genesis_hash) {
			Err(BlockDownloaderImportError::Invalid) => (),
			_ => panic!("expected BlockDownloaderImportError"),
		};
	}
	...
	fn import_headers_in_blocks_state() {
		...
		let srlp_data = encode_list(&headers);
		let headers_srlp = Rlp::new(&srlp_data);

		match downloader.import_headers(&mut io, &headers_srlp, headers[0].hash()) {
			Ok(DownloadAction::None) => (),
	...
	fn import_bodies() {
		...
		for i in 0..4 {
			...
			let mut uncles = if i > 0 {
				...
				::srlp::EMPTY_LIST_RLP.to_vec()
			};

			let mut txs = encode_list(&[dummy_signed_tx()]);
			let tx_root = ordered_trie_root(Rlp::new(&txs).iter().map(|r| r.as_raw()));

			let mut srlp = RlpStream::new_list(2);
			srlp.append_raw(&txs, 1);
			srlp.append_raw(&uncles, 1);
			bodies.push(srlp.out());
		...
		let srlp_data = encode_list(&headers[0..3]);
		let headers_srlp = Rlp::new(&srlp_data);
		assert!(downloader.import_headers(&mut io, &headers_srlp, headers[0].hash()).is_ok());

		// Import first body successfully.
		let mut srlp_data = RlpStream::new_list(1);
		srlp_data.append_raw(&bodies[0], 1);
		let bodies_srlp = Rlp::new(srlp_data.as_raw());
		assert!(downloader.import_bodies(&bodies_srlp, &[headers[0].hash(), headers[1].hash()]).is_ok());

		// Import second body successfully.
		let mut srlp_data = RlpStream::new_list(1);
		srlp_data.append_raw(&bodies[1], 1);
		let bodies_srlp = Rlp::new(srlp_data.as_raw());
		assert!(downloader.import_bodies(&bodies_srlp, &[headers[0].hash(), headers[1].hash()]).is_ok());

		// Import unexpected third body.
		let mut srlp_data = RlpStream::new_list(1);
		srlp_data.append_raw(&bodies[2], 1);
		let bodies_srlp = Rlp::new(srlp_data.as_raw());
		match downloader.import_bodies(&bodies_srlp, &[headers[0].hash(), headers[1].hash()]) {
			Err(BlockDownloaderImportError::Invalid) => (),
			_ => panic!("expected BlockDownloaderImportError"),
		};
	}
	...
	fn import_receipts() {
		...
		for i in 0..4 {
			...
			//
			// The SRLP-encoded integers are clearly not receipts, but the BlockDownloader treats
			// all receipts as byte blobs, so it does not matter.
			let mut receipts_srlp = if i < 2 {
				encode_list(&[0u32])
			} else {
				encode_list(&[i as u32])
			};
			let receipts_root = ordered_trie_root(Rlp::new(&receipts_srlp).iter().map(|r| r.as_raw()));
			receipts.push(receipts_srlp);
		...
		let mut srlp_data = RlpStream::new_list(2);
		srlp_data.append_raw(&receipts[1], 1);
		srlp_data.append_raw(&receipts[2], 1);
		let receipts_srlp = Rlp::new(srlp_data.as_raw());
		assert!(downloader.import_receipts(&receipts_srlp, &[headers[1].hash(), headers[2].hash()]).is_ok());

		// Import unexpected fourth receipt.
		let mut srlp_data = RlpStream::new_list(1);
		srlp_data.append_raw(&receipts[3], 1);
		let bodies_srlp = Rlp::new(srlp_data.as_raw());
		match downloader.import_bodies(&bodies_srlp, &[headers[1].hash(), headers[2].hash()]) {
			Err(BlockDownloaderImportError::Invalid) => (),
			_ => panic!("expected BlockDownloaderImportError"),
		};
	}
