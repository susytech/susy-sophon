...
use sofcore::error::{Error as SofcoreError, ErrorKind as SofcoreErrorKind, ImportErrorKind, BlockError};
use sofcore::snapshot::{ManifestData, RestorationStatus};
use sofcore::verification::queue::kind::blocks::Unverified;
...
use super::{
	...
	SyncRequester,
	SyncState,
	SOF_PROTOCOL_VERSION_62,
	SOF_PROTOCOL_VERSION_63,
	MAX_NEW_BLOCK_AGE,
	MAX_NEW_HASHES,
...
impl SyncHandler {
	...
	pub fn on_packet(sync: &mut ChainSync, io: &mut SyncIo, peer: PeerId, packet_id: u8, data: &[u8]) {
		...
		if let Some(packet_id) = SyncPacket::from_u8(packet_id) {
			let result = match packet_id {
				StatusPacket => SyncHandler::on_peer_status(sync, io, peer, &srlp),
				BlockHeadersPacket => SyncHandler::on_peer_block_headers(sync, io, peer, &srlp),
				BlockBodiesPacket => SyncHandler::on_peer_block_bodies(sync, io, peer, &srlp),
				ReceiptsPacket => SyncHandler::on_peer_block_receipts(sync, io, peer, &srlp),
				NewBlockPacket => SyncHandler::on_peer_new_block(sync, io, peer, &srlp),
				NewBlockHashesPacket => SyncHandler::on_peer_new_hashes(sync, io, peer, &srlp),
				SnapshotManifestPacket => SyncHandler::on_snapshot_manifest(sync, io, peer, &srlp),
				SnapshotDataPacket => SyncHandler::on_snapshot_data(sync, io, peer, &srlp),
				PrivateTransactionPacket => SyncHandler::on_private_transaction(sync, io, peer, &srlp),
				SignedPrivateTransactionPacket => SyncHandler::on_signed_private_transaction(sync, io, peer, &srlp),
				_ => {
	...
	pub fn on_peer_new_block(sync: &mut ChainSync, io: &mut SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		let block = Unverified::from_srlp(r.at(0)?.as_raw().to_vec())?;
		...
		match io.chain().import_block(block) {
			...
			Err(SofcoreError(SofcoreErrorKind::Block(BlockError::UnknownParent(p)), _)) => {
				unknown = true;
	...
	fn on_snapshot_manifest(sync: &mut ChainSync, io: &mut SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		let manifest_srlp = r.at(0)?;
		let manifest = ManifestData::from_srlp(manifest_srlp.as_raw())?;
		...
		sync.snapshot.reset_to(&manifest, &keccak(manifest_srlp.as_raw()));
	...
	fn on_peer_status(sync: &mut ChainSync, io: &mut SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		if false
			...
			|| (!warp_protocol && (peer.protocol_version < SOF_PROTOCOL_VERSION_62.0 || peer.protocol_version > SOF_PROTOCOL_VERSION_63.0))
		{
			trace!(target: "sync", "Peer {} unsupported sof protocol ({})", peer_id, peer.protocol_version);
			return Err(DownloaderImportError::Invalid);
		}
...
#[cfg(test)]
mod tests {
	use sofcore::client::{ChainInfo, EachBlockWith, TestBlockChainClient};
	...
	fn handles_peer_new_hashes() {
		...
		let hashes_srlp = Rlp::new(&hashes_data);

		let result = SyncHandler::on_peer_new_hashes(&mut sync, &mut io, 0, &hashes_srlp);
	...
	fn handles_peer_new_hashes_empty() {
		...
		let hashes_srlp = Rlp::new(&empty_hashes_data);
