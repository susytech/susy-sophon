...
//! This implementation uses sophon protocol v63
//!
...
use bytes::Bytes;
use srlp::{RlpStream, DecoderError};
use network::{self, PeerId, PacketId};
use network::client_version::ClientVersion;
use sofcore::client::{BlockChainClient, BlockStatus, BlockId, BlockChainInfo, BlockQueueInfo};
...
use api::{SofProtocolInfo as PeerInfoDigest, WARP_SYNC_PROTOCOL_ID, PriorityTask};
...
/// 63 version of Sophon protocol.
pub const SOF_PROTOCOL_VERSION_63: (u8, u8) = (63, 0x11);
/// 62 version of Sophon protocol.
pub const SOF_PROTOCOL_VERSION_62: (u8, u8) = (62, 0x11);
/// 1 version of Susy protocol and the packet count.
pub const PAR_PROTOCOL_VERSION_1: (u8, u8) = (1, 0x15);
/// 2 version of Susy protocol (consensus messages added).
pub const PAR_PROTOCOL_VERSION_2: (u8, u8) = (2, 0x16);
/// 3 version of Susy protocol (private transactions messages added).
...
pub const MAX_RECSIPTS_HEADERS_TO_SEND: usize = 256;
...
const RECSIPTS_TIMEOUT: Duration = Duration::from_secs(10);
		...
		let mut work = || {
			...
			match task {
				...
				PriorityTask::PropagateBlock { started, block, hash, difficulty } => {
					...
					let srlp = ChainSync::create_block_srlp(&block, total_difficulty);
					for peers in sync.get_peers(&chain_info, PeerState::SameBlock).chunks(10) {
						check_deadline(deadline)?;
						for peer in peers {
							SyncPropagator::send_packet(io, *peer, NewBlockPacket, srlp.clone());
...
// Static methods
impl ChainSync {
	/// creates srlp to send for the tree defined by 'from' and 'to' hashes
	fn create_new_hashes_srlp(chain: &BlockChainClient, from: &H256, to: &H256) -> Option<Bytes> {
			...
			Some(route) => {
				...
				match route.blocks.len() {
					...
					_ => {
						...
						let mut srlp_stream = RlpStream::new_list(blocks.len());
						for block_hash in  blocks {
							let mut hash_srlp = RlpStream::new_list(2);
							...
							hash_srlp.append(&block_hash);
							hash_srlp.append(&number);
							srlp_stream.append_raw(hash_srlp.as_raw(), 1);
						}
						Some(srlp_stream.out())
					}
				}
			},
			None => None
	...
	/// creates srlp from block bytes and total difficulty
	fn create_block_srlp(bytes: &Bytes, total_difficulty: U256) -> Bytes {
		let mut srlp_stream = RlpStream::new_list(2);
		srlp_stream.append_raw(bytes, 1);
		srlp_stream.append(&total_difficulty);
		srlp_stream.out()
	}

	/// creates latest block srlp for the given client
	fn create_latest_block_srlp(chain: &BlockChainClient) -> Bytes {
		Self::create_block_srlp(
	...
	/// creates given hash block srlp for the given client
	fn create_new_block_srlp(chain: &BlockChainClient, hash: &H256) -> Bytes {
		Self::create_block_srlp(
...
impl ChainSync {
	...
	pub fn status(&self) -> SyncStatus {
		...
		SyncStatus {
			...
			protocol_version: SOF_PROTOCOL_VERSION_63.0,
	...
	fn send_status(&mut self, io: &mut SyncIo, peer: PeerId) -> Result<(), network::Error> {
		...
		let protocol = if warp_protocol { warp_protocol_version } else { SOF_PROTOCOL_VERSION_63.0 };
		...
		if warp_protocol {
			...
			let manifest_hash = manifest.map_or(H256::new(), |m| keccak(m.into_srlp()));
	...
	pub fn maintain_peers(&mut self, io: &mut SyncIo) {
		...
		for (peer_id, peer) in &self.peers {
			...
			let timeout = match peer.asking {
				...
				PeerAsking::BlockReceipts => elapsed > RECSIPTS_TIMEOUT,
...
pub mod tests {
	...
	use sofcore::client::{BlockChainClient, EachBlockWith, TestBlockChainClient, ChainInfo, BlockInfo};
	use sofcore::miner::{MinerService, PendingOrdering};
	...
	pub fn get_dummy_block(order: u32, parent_hash: H256) -> Bytes {
		...
		let mut srlp = RlpStream::new_list(3);
		srlp.append(&header);
		srlp.append_raw(&::srlp::EMPTY_LIST_RLP, 1);
		srlp.append_raw(&::srlp::EMPTY_LIST_RLP, 1);
		srlp.out()
	}
	...
	pub fn get_dummy_blocks(order: u32, parent_hash: H256) -> Bytes {
		...
		srlp.append_raw(&get_dummy_block(order, parent_hash), 1);
		let difficulty: U256 = (100 * order).into();
		srlp.append(&difficulty);
		srlp.out()
	}

	pub fn get_dummy_hashes() -> Bytes {
		let mut srlp = RlpStream::new_list(5);
		for _ in 0..5 {
			let mut hash_d_srlp = RlpStream::new_list(2);
			let hash: H256 = H256::from(0u64);
			let diff: U256 = U256::from(1u64);
			hash_d_srlp.append(&hash);
			hash_d_srlp.append(&diff);

			srlp.append_raw(&hash_d_srlp.out(), 1);
		}
	...
	fn calculates_tree_for_lagging_peer() {
		...
		let srlp = ChainSync::create_new_hashes_srlp(&client, &end, &start);
		assert!(srlp.is_none());

		let srlp = ChainSync::create_new_hashes_srlp(&client, &start, &end).unwrap();
		// size of three srlp encoded hash-difficulty
		assert_eq!(107, srlp.len());
	}
	// idea is that what we produce when propagading latest hashes should be accepted in
	// on_peer_new_hashes in our code as well
	#[test]
	fn hashes_srlp_mutually_acceptable() {
		let mut client = TestBlockChainClient::new();
		client.add_blocks(100, EachBlockWith::Uncle);
	...
	#[test]
	fn block_srlp_mutually_acceptable() {
		let mut client = TestBlockChainClient::new();
		client.add_blocks(100, EachBlockWith::Uncle);
		let queue = RwLock::new(VecDeque::new());
		let mut sync = dummy_sync_with_peer(client.block_hash_delta_minus(5), &client);
	...
	fn should_add_transactions_to_queue() {
		fn sender(tx: &UnverifiedTransaction) -> Address {
			sofkey::public_to_address(&tx.recover_public().unwrap())
		}
