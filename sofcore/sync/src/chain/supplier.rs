...
impl SyncSupplier {
	...
	pub fn dispatch_packet(sync: &RwLock<ChainSync>, io: &mut SyncIo, peer: PeerId, packet_id: u8, data: &[u8]) {
		let srlp = Rlp::new(data);

		if let Some(id) = SyncPacket::from_u8(packet_id) {
			let result = match id {
				GetBlockBodiesPacket => SyncSupplier::return_srlp(
					io, &srlp, peer,
					SyncSupplier::return_block_bodies,
					|e| format!("Error sending block bodies: {:?}", e)),

				GetBlockHeadersPacket => SyncSupplier::return_srlp(
					io, &srlp, peer,
					SyncSupplier::return_block_headers,
					|e| format!("Error sending block headers: {:?}", e)),

				GetReceiptsPacket => SyncSupplier::return_srlp(
					io, &srlp, peer,
					SyncSupplier::return_receipts,
					|e| format!("Error sending receipts: {:?}", e)),

				GetNodeDataPacket => SyncSupplier::return_srlp(
					io, &srlp, peer,
					SyncSupplier::return_node_data,
					|e| format!("Error sending nodes: {:?}", e)),

				GetSnapshotManifestPacket => SyncSupplier::return_srlp(
					io, &srlp, peer,
					SyncSupplier::return_snapshot_manifest,
					|e| format!("Error sending snapshot manifest: {:?}", e)),

				GetSnapshotDataPacket => SyncSupplier::return_srlp(
					io, &srlp, peer,
				...
				_ => {
					...
					match id {
						ConsensusDataPacket => {
							SyncHandler::on_consensus_packet(io, peer, &srlp)
						},
						TransactionsPacket => {
							let res = {
								let sync_ro = sync.read();
								SyncHandler::on_peer_transactions(&*sync_ro, io, peer, &srlp)
							};
							if res.is_err() {
	...
	fn return_block_headers(io: &SyncIo, r: &Rlp, peer_id: PeerId) -> RlpResponseResult {
			...
			match io.chain().block_header(BlockId::Hash(hash)) {
				Some(hdr) => {
					...
					if max_headers == 1 || io.chain().block_hash(BlockId::Number(number)) != Some(hash) {
						...
						srlp.append_raw(&hdr.into_inner(), 1);
						return Ok(Some((BlockHeadersPacket.id(), srlp)));
					}
					number
				}
		...
		let mut srlp = RlpStream::new_list(count as usize);
		srlp.append_raw(&data, count as usize);
		trace!(target: "sync", "{} -> GetBlockHeaders: returned {} entries", peer_id, count);
		Ok(Some((BlockHeadersPacket.id(), srlp)))
	}
	...
	fn return_block_bodies(io: &SyncIo, r: &Rlp, peer_id: PeerId) -> RlpResponseResult {
		...
		let mut srlp = RlpStream::new_list(added);
		srlp.append_raw(&data, added);
		trace!(target: "sync", "{} -> GetBlockBodies: returned {} entries", peer_id, added);
		Ok(Some((BlockBodiesPacket.id(), srlp)))
	}
	...
	fn return_node_data(io: &SyncIo, r: &Rlp, peer_id: PeerId) -> RlpResponseResult {
		...
		Ok(Some((NodeDataPacket.id(), srlp)))
	}

	fn return_receipts(io: &SyncIo, srlp: &Rlp, peer_id: PeerId) -> RlpResponseResult {
		let payload_soft_limit = io.payload_soft_limit();
		let mut count = srlp.item_count().unwrap_or(0);
		trace!(target: "sync", "{} -> GetReceipts: {} entries", peer_id, count);
		if count == 0 {
			debug!(target: "sync", "Empty GetReceipts request, ignoring.");
			return Ok(None);
		}
		count = cmp::min(count, MAX_RECSIPTS_HEADERS_TO_SEND);
		let mut added_headers = 0usize;
		let mut data = Bytes::new();
		let mut total_bytes = 0;
		for i in 0..count {
			if let Some(receipts) = io.chain().block_receipts(&srlp.val_at::<H256>(i)?) {
				let mut receipts_bytes = ::srlp::encode(&receipts);
		...
		let mut srlp_result = RlpStream::new_list(added_headers);
		srlp_result.append_raw(&data, added_headers);
		Ok(Some((ReceiptsPacket.id(), srlp_result)))
	}
	...
	fn return_snapshot_manifest(io: &SyncIo, r: &Rlp, peer_id: PeerId) -> RlpResponseResult {
		...
		let srlp = match io.snapshot_service().manifest() {
			Some(manifest) => {
				trace!(target: "warp", "{} <- SnapshotManifest", peer_id);
				let mut srlp = RlpStream::new_list(1);
				srlp.append_raw(&manifest.into_srlp(), 1);
				srlp
			},
			None => {
		...
		Ok(Some((SnapshotManifestPacket.id(), srlp)))
	}
	...
	fn return_snapshot_data(io: &SyncIo, r: &Rlp, peer_id: PeerId) -> RlpResponseResult {
		...
		let srlp = match io.snapshot_service().chunk(hash) {
			Some(data) => {
				let mut srlp = RlpStream::new_list(1);
		...
		Ok(Some((SnapshotDataPacket.id(), srlp)))
	}

	fn return_srlp<FRlp, FError>(io: &mut SyncIo, srlp: &Rlp, peer: PeerId, srlp_func: FRlp, error_func: FError) -> Result<(), PacketDecodeError>
		where FRlp : Fn(&SyncIo, &Rlp, PeerId) -> RlpResponseResult,
			FError : FnOnce(network::Error) -> String
	{
		let response = srlp_func(io, srlp, peer);
		match response {
			Err(e) => Err(e),
			Ok(Some((packet_id, srlp_stream))) => {
				io.respond(packet_id, srlp_stream.out()).unwrap_or_else(
...
mod test {
	...
	use sofcore::client::{BlockChainClient, EachBlockWith, TestBlockChainClient};
	...
	fn return_block_headers() {
		...
		fn to_header_vec(srlp: ::chain::RlpResponseResult) -> Vec<SyncHeader> {
			Rlp::new(&srlp.unwrap().unwrap().1.out()).iter().map(|r| SyncHeader::from_srlp(r.as_raw().to_vec()).unwrap()).collect()
		}
		...
		let headers: Vec<_> = blocks.iter().map(|b| SyncHeader::from_srlp(Rlp::new(b).at(0).unwrap().as_raw().to_vec()).unwrap()).collect();
	...
	fn respect_packet_limit() {
		...
		let mut small_srlp_request = RlpStream::new_list(small_num_blocks);
		let mut large_srlp_request = RlpStream::new_list(large_num_blocks);

		for i in 0..small_num_blocks {
			let hash: H256 = client.block_hash(BlockId::Number(i as u64)).unwrap();
			small_srlp_request.append(&hash);
			large_srlp_request.append(&hash);
		}
		...
		let small_result = SyncSupplier::return_block_bodies(&io, &Rlp::new(&small_srlp_request.out()), 0);
		let small_result = small_result.unwrap().unwrap().1;
		assert_eq!(Rlp::new(&small_result.out()).item_count().unwrap(), small_num_blocks);

		let large_result = SyncSupplier::return_block_bodies(&io, &Rlp::new(&large_srlp_request.out()), 0);
	...
	fn return_nodes() {
		...
		// it returns srlp ONLY for hashes started with "f"
		let result = SyncSupplier::return_node_data(&io, &Rlp::new(&node_request.clone()), 0);

		assert!(result.is_ok());
		let srlp_result = result.unwrap();
		assert!(srlp_result.is_some());

		// the length of one srlp-encoded hashe
		let srlp = srlp_result.unwrap().1.out();
		let srlp = Rlp::new(&srlp);
		assert_eq!(Ok(1), srlp.item_count());
	...
	fn return_receipts() {
		...
		// the length of two srlp-encoded receipts
		assert_eq!(603, srlp_result.unwrap().1.out().len());
