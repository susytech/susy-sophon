...
impl SyncPropagator {
	...
	pub fn propagate_blocks(sync: &mut ChainSync, chain_info: &BlockChainInfo, io: &mut SyncIo, blocks: &[H256], peers: &[PeerId]) -> usize {
		...
		let sent = peers.len();
		let mut send_packet = |io: &mut SyncIo, srlp: Bytes| {
			for peer_id in peers {
				SyncPropagator::send_packet(io, *peer_id, NewBlockPacket, srlp.clone());
		...
		if blocks.is_empty() {
			let srlp =  ChainSync::create_latest_block_srlp(io.chain());
			send_packet(io, srlp);
		} else {
			for h in blocks {
				let srlp =  ChainSync::create_new_block_srlp(io.chain(), h);
				send_packet(io, srlp);
			}
		}
	...
	pub fn propagate_new_hashes(sync: &mut ChainSync, chain_info: &BlockChainInfo, io: &mut SyncIo, peers: &[PeerId]) -> usize {
		...
		let srlp = match ChainSync::create_new_hashes_srlp(io.chain(), &last_parent, &best_block_hash) {
			Some(srlp) => srlp,
			None => return 0
		};
		...
		for peer_id in peers {
			...
			SyncPropagator::send_packet(io, *peer_id, NewBlockHashesPacket, srlp.clone());
		}
		sent
	}
	...
	fn propagate_transactions_to_peers<F: FnMut() -> bool>(
		...
		let send_packet = |io: &mut SyncIo, peer_id: PeerId, sent: usize, srlp: Bytes| {
			let size = srlp.len();
			SyncPropagator::send_packet(io, peer_id, TransactionsPacket, srlp);
		...
		for peer_id in peers {
			...
			if peer_info.last_sent_transactions.is_empty() {
				...
				send_packet(io, peer_id, all_transactions_hashes.len(), all_transactions_srlp.clone());
			...
			let (packet, to_send) = {
				...
				for tx in &transactions {
					...
					if to_send.contains(&hash) {
						...
						tx.srlp_append(&mut transaction);
	...
	pub fn propagate_proposed_blocks(sync: &mut ChainSync, io: &mut SyncIo, proposed: &[Bytes]) {
		...
		for block in proposed {
			let srlp = ChainSync::create_block_srlp(
				block,
...
#[cfg(test)]
mod tests {
	use sofcore::client::{BlockInfo, ChainInfo, EachBlockWith, TestBlockChainClient};
	...
	fn should_propagate_service_transaction_to_selected_peers_only() {
		...
		// when peer#1 is Graviton
		insert_dummy_peer(&mut sync, 1, block_hash);
		io.peers_info.insert(1, "Graviton".to_owned());
		// and peer#2 is Susy, accepting service transactions
		insert_dummy_peer(&mut sync, 2, block_hash);
		io.peers_info.insert(2, "Susy-Sophon/v2.6.0/linux/rustc".to_owned());
		// and peer#3 is Susy, accepting service transactions
		insert_dummy_peer(&mut sync, 3, block_hash);
		io.peers_info.insert(3, "Susy-Sophon/ABCDEFGH/v2.7.3/linux/rustc".to_owned());
	...
	fn should_propagate_service_transaction_is_sent_as_separate_message() {
		...
		// when peer#1 is Susy, accepting service transactions
		insert_dummy_peer(&mut sync, 1, block_hash);
		io.peers_info.insert(1, "Susy-Sophon/v2.6.0/linux/rustc".to_owned());
		...
		let sent_transactions: Vec<UnverifiedTransaction> = io.packets.iter()
			.filter_map(|p| {
				...
				let srlp = Rlp::new(&*p.data);
				let item_count = srlp.item_count().unwrap_or(0);
				if item_count != 1 {
					return None;
				}

				srlp.at(0).ok().and_then(|r| r.as_val().ok())
			})
			.collect();
