...
use bytes::Bytes;
use srlp::{Rlp, RlpStream, DecoderError};
use network;
...
impl SyncHeader {
	pub fn from_srlp(bytes: Bytes) -> Result<Self, DecoderError> {
		let result = SyncHeader {
			header: ::srlp::decode(&bytes)?,
			bytes,
		};
...
impl SyncBody {
	pub fn from_srlp(bytes: &[u8]) -> Result<Self, DecoderError> {
		let srlp = Rlp::new(bytes);
		let transactions_srlp = srlp.at(0)?;
		let uncles_srlp = srlp.at(1)?;

		let result = SyncBody {
			transactions_bytes: transactions_srlp.as_raw().to_vec(),
			transactions: transactions_srlp.as_list()?,
			uncles_bytes: uncles_srlp.as_raw().to_vec(),
			uncles: uncles_srlp.as_list()?,
		};
	...
	fn empty_body() -> Self {
		SyncBody {
			transactions_bytes: ::srlp::EMPTY_LIST_RLP.to_vec(),
			transactions: Vec::with_capacity(0),
			uncles_bytes: ::srlp::EMPTY_LIST_RLP.to_vec(),
...
mod test {
	...
	use sofcore::client::{TestBlockChainClient, EachBlockWith, BlockId, BlockChainClient};
	...
	fn insert_headers() {
		...
		assert_eq!(
			...
			blocks[0..6].iter().map(|b| Unverified::from_srlp(b.to_vec()).unwrap()).collect::<Vec<_>>()
		);
		assert!(!bc.contains(&hashes[0]));
		...
		assert_eq!(
			bc.drain().into_iter().map(|b| b.block).collect::<Vec<_>>(),
			blocks[6..16].iter().map(|b| Unverified::from_srlp(b.to_vec()).unwrap()).collect::<Vec<_>>()
		);
