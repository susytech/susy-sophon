...
use sophon_types::{H256, H512, U256};
use io::{TimerToken};
use sofkey::Secret;
use sofcore::client::{BlockChainClient, ChainNotify, NewBlocks, ChainMessageType};
use sofcore::snapshot::SnapshotService;
use types::BlockNumber;
use sync_io::NetSyncIo;
use chain::{ChainSyncApi, SyncStatus as SofSyncStatus};
use std::net::{SocketAddr, AddrParseError};
use std::str::FromStr;
use parking_lot::{RwLock, Mutex};
use chain::{SOF_PROTOCOL_VERSION_63, SOF_PROTOCOL_VERSION_62,
...
/// Sophon sync protocol
pub const SOF_PROTOCOL: ProtocolId = *b"sof";
/// Sophon light protocol
...
pub struct PeerInfo {
	...
	/// Sof protocol info.
	pub sof_info: Option<SofProtocolInfo>,
	/// Light protocol info.
	pub pip_info: Option<PipProtocolInfo>,
}

/// Sophon protocol info.
#[derive(Debug)]
pub struct SofProtocolInfo {
	/// Protocol version
	pub version: u32,
...
/// SofSync initialization parameters.
pub struct Params {
	/// Configuration.
...
/// Sophon network protocol handler
pub struct SofSync {
	/// Network service
	network: NetworkService,
	/// Main (sof/par) protocol handler
	sof_handler: Arc<SyncProtocolHandler>,
...
impl SofSync {
	...
	pub fn new(params: Params, connection_filter: Option<Arc<ConnectionFilter>>) -> Result<Arc<SofSync>, Error> {
		...
		let sync = Arc::new(SofSync {
			network: service,
			sof_handler: Arc::new(SyncProtocolHandler {
				sync,
				chain: params.chain,
...
impl SyncProvider for SofSync {
	/// Get sync status
	fn status(&self) -> SofSyncStatus {
		self.sof_handler.sync.status()
	}
	...
	fn peers(&self) -> Vec<PeerInfo> {
		self.network.with_context_eval(self.subprotocol_name, |ctx| {
			...
			let peer_info = self.sof_handler.sync.peer_info(&peer_ids);
	...
	fn transactions_stats(&self) -> BTreeMap<H256, TransactionStats> {
		self.sof_handler.sync.transactions_stats()
	}
}
...
impl ChainNotify for SofSync {
	...
	fn new_blocks(&self, new_blocks: NewBlocks)
	{
		...
		self.network.with_context(self.subprotocol_name, |context| {
			let mut sync_io = NetSyncIo::new(context, &*self.sof_handler.chain, &*self.sof_handler.snapshot_service,
				&self.sof_handler.overlay);
			self.sof_handler.sync.write().chain_new_blocks(
				&mut sync_io,
				&new_blocks.imported,
				&new_blocks.invalid,
		...
		self.network.with_context(self.light_subprotocol_name, |context| {
			...
			let chain_info = self.sof_handler.chain.chain_info();
	...
	fn start(&self) {
			...
			Err((err, listen_address)) => {
				match err.into() {
					ErrorKind::Io(ref e) if e.kind() == io::ErrorKind::AddrInUse => {
						warn!("Network port {:?} is already in use, make sure that another instance of an Sophon client is not running or change the port using the --port option.", listen_address.expect("Listen address is not set."))
					},
		...
		self.network.register_protocol(self.sof_handler.clone(), self.subprotocol_name, &[SOF_PROTOCOL_VERSION_62, SOF_PROTOCOL_VERSION_63])
			.unwrap_or_else(|e| warn!("Error registering sophon protocol: {:?}", e));
		// register the warp sync subprotocol
		self.network.register_protocol(self.sof_handler.clone(), WARP_SYNC_PROTOCOL_ID, &[PAR_PROTOCOL_VERSION_1, PAR_PROTOCOL_VERSION_2, PAR_PROTOCOL_VERSION_3])
	...
	fn stop(&self) {
		self.sof_handler.snapshot_service.abort_restore();
		self.network.stop();
	}

	fn broadcast(&self, message_type: ChainMessageType) {
		self.network.with_context(WARP_SYNC_PROTOCOL_ID, |context| {
			let mut sync_io = NetSyncIo::new(context, &*self.sof_handler.chain, &*self.sof_handler.snapshot_service, &self.sof_handler.overlay);
			match message_type {
				ChainMessageType::Consensus(message) => self.sof_handler.sync.write().propagate_consensus_packet(&mut sync_io, message),
				ChainMessageType::PrivateTransaction(transaction_hash, message) =>
					self.sof_handler.sync.write().propagate_private_transaction(&mut sync_io, transaction_hash, PrivateTransactionPacket, message),
				ChainMessageType::SignedPrivateTransaction(transaction_hash, message) =>
					self.sof_handler.sync.write().propagate_private_transaction(&mut sync_io, transaction_hash, SignedPrivateTransactionPacket, message),
			}
		});
	}

	fn transactions_received(&self, txs: &[UnverifiedTransaction], peer_id: PeerId) {
		let mut sync = self.sof_handler.sync.write();
...
impl LightHandler for TxRelay {
	fn on_transactions(&self, ctx: &EventContext, relay: &[::types::transaction::UnverifiedTransaction]) {
		...
		self.0.queue_transactions(relay.iter().map(|tx| ::srlp::encode(tx)).collect(), ctx.peer())
	}
}
...
impl ManageNetwork for SofSync {
	fn accept_unreserved_peers(&self) {
	...
	fn stop_network(&self) {
		self.network.with_context(self.subprotocol_name, |context| {
			let mut sync_io = NetSyncIo::new(context, &*self.sof_handler.chain, &*self.sof_handler.snapshot_service, &self.sof_handler.overlay);
			self.sof_handler.sync.write().abort(&mut sync_io);
		});
