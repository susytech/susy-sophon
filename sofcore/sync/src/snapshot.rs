...
use sofcore::snapshot::{ManifestData, SnapshotService};
...
impl Snapshot {
	...
	/// Reset collection for a manifest SRLP
...
mod test {
	...
	use bytes::Bytes;
	use super::*;
	use sofcore::snapshot::ManifestData;
	...
	fn test_manifest() -> (ManifestData, H256, Vec<Bytes>, Vec<Bytes>) {
		...
		let mhash = keccak(manifest.clone().into_srlp());
	...
	fn validate_chunks() {
		...
		assert_eq!(snapshot.snapshot_hash(), Some(keccak(manifest.into_srlp())));
	}
