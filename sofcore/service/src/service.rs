...
use sofcore::client::{Client, ClientConfig, ChainNotify, ClientIoMessage};
use sofcore::miner::Miner;
use sofcore::snapshot::service::{Service as SnapshotService, ServiceParams as SnapServiceParams};
use sofcore::snapshot::{SnapshotService as _SnapshotService, RestorationStatus};
use sofcore::spec::Spec;

use sofcore_private_tx::{self, Importer, Signer};
use Error;

pub struct PrivateTxService {
	provider: Arc<sofcore_private_tx::Provider>,
}

impl PrivateTxService {
	fn new(provider: Arc<sofcore_private_tx::Provider>) -> Self {
		PrivateTxService {
			provider,
		}
	}
	...
	pub fn provider(&self) -> Arc<sofcore_private_tx::Provider> {
		self.provider.clone()
	}
}
...
impl PrivateTxHandler for PrivateTxService {
	fn import_private_transaction(&self, srlp: &[u8]) -> Result<H256, String> {
		match self.provider.import_private_transaction(srlp) {
	...
	fn import_signed_private_transaction(&self, srlp: &[u8]) -> Result<H256, String> {
		match self.provider.import_signed_private_transaction(srlp) {
			Ok(import_result) => Ok(import_result),
			Err(err) => {
...
impl ClientService {
	...
	pub fn start(
		...
		miner: Arc<Miner>,
		signer: Arc<Signer>,
		encryptor: Box<sofcore_private_tx::Encryptor>,
		private_tx_conf: sofcore_private_tx::ProviderConfig,
		private_encryptor_conf: sofcore_private_tx::EncryptorConfig,
		) -> Result<ClientService, Error>
	{
		...
		let private_keys = Arc::new(sofcore_private_tx::SecretStoreKeys::new(
			client.clone(),
			private_encryptor_conf.key_server_account,
		));
		let provider = Arc::new(sofcore_private_tx::Provider::new(
			client.clone(),
			miner,
			signer,
			encryptor,
...
mod tests {
	...
	use sofcore::client::ClientConfig;
	use sofcore::miner::Miner;
	use sofcore::spec::Spec;
	use sofcore::test_helpers;
	use susykv_rocksdb::{DatabaseConfig, CompactionProfile};
	use super::*;

	use sofcore_private_tx;
	...
	fn it_can_be_started() {
		...
		let service = ClientService::start(
			...
			Arc::new(sofcore_private_tx::DummySigner),
			Box::new(sofcore_private_tx::NoopEncryptor),
			Default::default(),
			Default::default(),
		);
