...
impl From<Box<softrie::TrieError>> for Error {
	fn from(err: Box<softrie::TrieError>) -> Self {
		Error::Internal(format!("Internal error: {}", err))
	}
}

impl From<softrie::TrieError> for Error {
	fn from(err: softrie::TrieError) -> Self {
		Error::Internal(format!("Internal error: {}", err))
	}
}
