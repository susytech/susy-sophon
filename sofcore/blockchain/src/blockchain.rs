...
use sofcore_db::cache_manager::CacheManager;
use sofcore_db::keys::{BlockReceipts, BlockDetails, TransactionAddress, EPOCH_KEY_PREFIX, EpochTransitions};
use sofcore_db::{self as db, Writable, Readable, CacheUpdatePolicy};
use sophon_types::{H256, Bloom, BloomRef, U256};
...
use rayon::prelude::*;
use srlp::RlpStream;
use srlp_compress::{compress, decompress, blocks_swapper};
...
pub trait BlockProvider {
	...
	/// Get the header SRLP of a block.
...
impl BlockProvider for BlockChain {
	...
	fn block_body(&self, hash: &H256) -> Option<encoded::Body> {
		...
		// Check if it's the best block
		{
			let best_block = self.best_block.read();
			if &best_block.header.hash() == hash {
				return Some(encoded::Body::new(Self::block_to_body(best_block.block.srlp().as_raw())));
			}
		}
	...
	fn next(&mut self) -> Option<ExtendedHeader> {
		if self.current.is_zero() {
			...
			let header = self.chain.block_header_data(&self.current)
				.map(|h| h.decode().expect("Stored block header data is valid SRLP; qed"));
...
impl<'a> Iterator for EpochTransitionIter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			...
			let transitions: EpochTransitions = ::srlp::decode(&val[..]).expect("decode error: the db is corrupted or the data structure has changed");
...
impl BlockChain {
	...
	pub fn new(config: Config, genesis: &[u8], db: Arc<BlockChainDB>) -> BlockChain {
			...
			.expect("Low-level database error when fetching 'best' block. Some issue with disk?")
		{
			...
			None => {
				...
				batch.put(db::COL_HEADERS, &hash, block.header_srlp().as_raw());
...

		{
			...
			let best_block_total_difficulty = bc.block_details(&best_block_hash)
				...
				.total_difficulty;
			let best_block_srlp = bc.block(&best_block_hash)
			...
			*best_block = BestBlock {
				...
				header: best_block_srlp.decode_header(),
				block: best_block_srlp,
			};
		}
	...
	pub fn insert_unordered_block(&self, batch: &mut DBTransaction, block: encoded::Block, receipts: Vec<Receipt>, parent_td: Option<U256>, is_best: bool, is_ancient: bool) -> bool {
		...
		let compressed_header = compress(block.header_view().srlp().as_raw(), blocks_swapper());
	...
	///
	/// Used in snapshots to glue the chunks togsophy at the end.
	...
	pub fn block_to_body(block: &[u8]) -> Bytes {
		...
		body.append_raw(block_view.transactions_srlp().as_raw(), 1);
		body.append_raw(block_view.uncles_srlp().as_raw(), 1);
		body.out()
	}
...
mod tests {
	...
	pub fn new_db() -> Arc<BlockChainDB> {
		...
		let db = TestBlockChainDB {
			...
			key_value: Arc::new(susykv_memorydb::create(sofcore_db::NUM_COLUMNS.unwrap()))
		};
