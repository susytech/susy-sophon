...
use sofcore::CreateContractAddress;
use sofcore::client::BlockChainClient;
use sofcore::executive::{contract_address};
use sofcore::miner::Miner;
use sofcore::test_helpers::{generate_dummy_client, push_block_with_transactions};
use sofkey::{Secret, KeyPair, Signature};
use hash::keccak;

use sofcore_private_tx::{NoopEncryptor, Provider, ProviderConfig, StoringKeyProvider};
...
fn private_contract() {
	...
	let signer = Arc::new(sofcore_private_tx::KeyPairSigner(vec![key1.clone(), key3.clone(), key4.clone()]));
	...
	let io = sofcore_io::IoChannel::disconnected();
	let miner = Arc::new(Miner::new_for_tests(&::sofcore::spec::Spec::new_test(), None));
	...
	let signatures: Vec<_> = [&key3, &key4].iter().map(|k|
		Signature::from(::sofkey::sign(&k.secret(), &nonced_state_hash).unwrap().into_electrum())).collect();
	...
	let signatures: Vec<_> = [&key4].iter().map(|k|
		Signature::from(::sofkey::sign(&k.secret(), &private_state_hash).unwrap().into_electrum())).collect();
