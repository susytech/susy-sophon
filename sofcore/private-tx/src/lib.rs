...
use sophon_types::{H128, H256, U256, Address};
use hash::keccak;
use srlp::*;
use parking_lot::RwLock;
use bytes::Bytes;
use sofkey::{Signature, recover, public_to_address};
use io::IoChannel;
use sofcore::executive::{Executive, TransactOptions};
use sofcore::executed::{Executed};
use types::transaction::{SignedTransaction, Transaction, Action, UnverifiedTransaction};
use sofcore::{contract_address as sofcore_contract_address};
use sofcore::client::{
	Client, ChainNotify, NewBlocks, ChainMessageType, ClientIoMessage, BlockId,
	Call, BlockInfo
};
use sofcore::miner::{self, Miner, MinerService, pool_client::NonceCache};
use sofcore::{state, state_db};
use sofcore::trace::{Tracer, VMTracer};
use call_contract::CallContract;
use rustc_hex::FromHex;
use sofabi::FunctionOutputDecoder;

// Source avaiable at https://github.com/susy-contracts/private-tx/blob/master/contracts/PrivateContract.pol
const DEFAULT_STUB_CONTRACT: &'static str = include_str!("../res/private.svm");
...
pub trait Signer: Send + Sync {
	...
	fn sign(&self, account: Address, hash: sofkey::Message) -> Result<Signature, Error>;
}
...
impl Signer for DummySigner {
	...
	fn sign(&self, _account: Address, _hash: sofkey::Message) -> Result<Signature, Error> {
		Err("Signing is not supported.".to_owned())?
	}
}

/// Signer implementation using multiple keypairs
pub struct KeyPairSigner(pub Vec<sofkey::KeyPair>);
impl Signer for KeyPairSigner {
	fn decrypt(&self, account: Address, shared_mac: &[u8], payload: &[u8]) -> Result<Vec<u8>, Error> {
		let kp = self.0.iter().find(|k| k.address() == account).ok_or(sofkey::Error::InvalidAddress)?;
		Ok(sofkey::crypto::ecies::decrypt(kp.secret(), shared_mac, payload)?)
	}

	fn sign(&self, account: Address, hash: sofkey::Message) -> Result<Signature, Error> {
		let kp = self.0.iter().find(|k| k.address() == account).ok_or(sofkey::Error::InvalidAddress)?;
		Ok(sofkey::sign(kp.secret(), &hash)?)
	}
}
...
impl Provider {
	...
	pub fn create_private_transaction(&self, signed_transaction: SignedTransaction) -> Result<Receipt, Error> {
		...
		let data = signed_transaction.srlp_bytes();
		...
		self.broadcast_private_transaction(private.hash(), private.srlp_bytes());
		Ok(Receipt {
			hash: tx_hash,
		...
		let process_transaction = |transaction: &VerifiedPrivateTransaction| -> Result<_, String> {
			...
			match transaction.validator_account {
				None => {
					...
					self.broadcast_private_transaction(private_hash, transaction.private_transaction.srlp_bytes());
					return Ok(());
				}
				Some(validator_account) => {
					...
					self.broadcast_signed_private_transaction(signed_private_transaction.hash(), signed_private_transaction.srlp_bytes());
				}
			}
			Ok(())
		};
	...
	pub fn process_signature(&self, signed_tx: &SignedPrivateTransaction) -> Result<(), Error> {
		...
		let desc = match self.transactions_for_signing.read().get(&private_hash) {
			None => {
				...
				self.broadcast_signed_private_transaction(signed_tx.hash(), signed_tx.srlp_bytes());
				return Ok(());
			},
			Some(desc) => desc,
		};
	...
	fn iv_from_transaction(transaction: &SignedTransaction) -> H128 {
		let nonce = keccak(&transaction.nonce.srlp_bytes());
		let (iv, _) = nonce.split_at(INIT_VEC_LEN);
		H128::from_slice(iv)
	}

	fn iv_from_address(contract_address: &Address) -> H128 {
		let address = keccak(&contract_address.srlp_bytes());
			...
			V: VMTracer,
	{
		...
		let contract_address = contract_address.unwrap_or_else(|| {
			let (new_address, _) = sofcore_contract_address(engine.create_address_scheme(env_info.number), &sender, &nonce, &transaction.data);
			new_address
		});
...
pub trait Importer {
	...
	fn import_private_transaction(&self, _srlp: &[u8]) -> Result<H256, Error>;

	/// Add signed private transaction into the store
	///
	/// Creates corresponding public transaction if last required signature collected and sends it to the chain
	fn import_signed_private_transaction(&self, _srlp: &[u8]) -> Result<H256, Error>;
}
...
impl Importer for Arc<Provider> {
	fn import_private_transaction(&self, srlp: &[u8]) -> Result<H256, Error> {
		trace!(target: "privatetx", "Private transaction received");
		let private_tx: PrivateTransaction = Rlp::new(srlp).as_val()?;
	...
	fn import_signed_private_transaction(&self, srlp: &[u8]) -> Result<H256, Error> {
		let tx: SignedPrivateTransaction = Rlp::new(srlp).as_val()?;
