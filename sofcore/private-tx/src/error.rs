...
use sofcore::error::{Error as SofcoreError, ExecutionError};
use types::transaction::Error as TransactionError;
use sofkey::Error as KeyError;
use sofkey::crypto::Error as CryptoError;
...
pub enum Error {
	...
	/// General sofcore error.
	#[display(fmt = "General sofcore error {}", _0)]
	Sofcore(SofcoreError),
...
impl From<SofcoreError> for Error {
	fn from(err: SofcoreError) -> Self {
		Error::Sofcore(err).into()
	}
}
