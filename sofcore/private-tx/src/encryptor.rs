...
use sophon_types::{H128, H256, Address};
use sofjson;
use sofkey::{Signature, Public};
use crypto;
use futures::Future;
...
impl SecretStoreEncryptor {
	...
	fn retrieve_key(
		...
		let encrypted_bytes: sofjson::bytes::Bytes = result.trim_matches('\"').parse().map_err(|e| Error::Encrypt(e))?;
