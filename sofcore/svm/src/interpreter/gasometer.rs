...
enum Request<Cost: ::svm::CostType> {
	Gas(Cost),
	GasMem(Cost, Cost),
...
impl<Gas: svm::CostType> Gasometer<Gas> {
	...
	pub fn gas_provided(&self, schedule: &Schedule, needed: Gas, requested: Option<U256>) -> vm::Result<Gas> {
		...
		// but in SIP150 even if we request more we should never fail from OOG
	...
	pub fn requirements(
		...
		let cost = match instruction {
			...
			instructions::SSTORE => {
				...
				let gas = if schedule.sip1283 {
					let orig = U256::from(&*ext.initial_storage_at(&address)?);
					calculate_sip1283_sstore_gas(schedule, &orig, &val, &newval)
				} else {
...
#[inline]
fn mem_needed_const<Gas: svm::CostType>(mem: &U256, add: usize) -> vm::Result<Gas> {
	Gas::from_u256(overflowing!(mem.overflowing_add(U256::from(add))))
}

#[inline]
fn mem_needed<Gas: svm::CostType>(offset: &U256, size: &U256) -> vm::Result<Gas> {
	if size.is_zero() {
...
#[inline]
fn add_gas_usize<Gas: svm::CostType>(value: Gas, num: usize) -> (Gas, bool) {
	value.overflow_add(Gas::from(num))
}

#[inline]
fn to_word_size<Gas: svm::CostType>(value: Gas) -> (Gas, bool) {
...
#[inline]
fn calculate_sip1283_sstore_gas<Gas: svm::CostType>(schedule: &Schedule, original: &U256, current: &U256, new: &U256) -> Gas {
	Gas::from(
		if current == new {
...
pub fn handle_sip1283_sstore_clears_refund(ext: &mut vm::Ext, original: &U256, current: &U256, new: &U256) {
