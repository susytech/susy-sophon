...
#[macro_use]
#[cfg(not(feature = "svm-debug"))]
mod inner {
	macro_rules! svm_debug {
		($x: expr) => {}
	}

	pub struct SvmInformant;
	impl SvmInformant {
...
mod inner {
	...
	fn print(data: String) {
		if cfg!(feature = "svm-debug-tests") {
			println!("{}", data);
		} else {
			debug!(target: "svm", "{}", data);
		}
	}
	...
	impl Stats {
		...
		fn avg(&self) -> u64 {
			SvmInformant::as_micro(&self.total_duration) / self.count
		}
	}
}
