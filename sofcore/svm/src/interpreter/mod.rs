...
pub struct Interpreter<Cost: CostType> {
	...
	informant: informant::SvmInformant,
	do_trace: bool,
	done: bool,
...
impl<Cost: CostType> Interpreter<Cost> {
	...
	pub fn new(mut params: ActionParams, cache: Arc<SharedCache>, schedule: &Schedule, depth: usize) -> Interpreter<Cost> {
		...
		let informant = informant::SvmInformant::new(depth);
	...
	fn step_inner(&mut self, ext: &mut vm::Ext) -> Result<Never, InterpreterResult> {
		let result = match self.resume_result.take() {
			...
			None => {
				...
				svm_debug!({ self.informant.before_instruction(self.reader.position, instruction, info, &self.gasometer.as_mut().expect(GASOMETER_PROOF).current_gas, &self.stack) });
				...
				svm_debug!({ self.informant.after_instruction(instruction) });
	...
	fn exec_instruction(
		...
		match instruction {
			...
			instructions::SSTORE => {
				...
				if ext.schedule().sip1283 {
					let original_val = U256::from(&*ext.initial_storage_at(&address)?);
					gasometer::handle_sip1283_sstore_clears_refund(ext, &original_val, &current_val, &val);
				} else {
