...
SNAP_PACKAGE="susy_"$VERSION"_"$BUILD_ARCH".snap"
CARGO_TARGET="$(ls artifacts)"
# Choose snap release channel based on susy sophon version track
case ${TRACK} in
...
echo $SNAPCRAFT_LOGIN_SUSY_BASE64 | base64 --decode > snapcraft.login
snapcraft login --with snapcraft.login
snapcraft push --release $CHANNEL $SNAP_PACKAGE
snapcraft status susy
snapcraft logout
