#!/usr/bin/env bash

cargo build --release -p svmbin

# LOOP TEST
CODE1=606060405260005b620f42408112156019575b6001016007565b600081905550600680602b6000396000f3606060405200
if [ -x "$(command -v sofvm)" ]; then
  sofvm --code $CODE1
  echo "^^^^ sofvm"
fi
./target/release/susy-svm stats --code $CODE1 --gas 4402000
echo "^^^^ usize"
./target/release/susy-svm stats --code $CODE1
echo "^^^^ U256"

# RNG TEST
CODE2=
if [ -x "$(command -v sofvm)" ]; then
  sofvm --code $CODE2
  echo "^^^^ sofvm"
fi
./target/release/susy-svm stats --code $CODE2 --gas 143020115
echo "^^^^ usize"
./target/release/susy-svm stats --code $CODE2
echo "^^^^ U256"
