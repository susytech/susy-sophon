...
use sophon_types::{H256, H512};
use srlp::{self, DecoderError, RlpStream, Rlp};
...
impl srlp::Encodable for Topic {
	fn srlp_append(&self, s: &mut RlpStream) {
		s.encoder().encode_value(&self.0);
	}
}

impl srlp::Decodable for Topic {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		use std::cmp;

		srlp.decoder().decode_value(|bytes| match bytes.len().cmp(&4) {
...
fn decode_topics(srlp: Rlp) -> Result<SmallVec<[Topic; 4]>, DecoderError> {
	if srlp.is_list() {
		srlp.iter().map(|r| r.as_val::<Topic>()).collect()
	} else {
		srlp.as_val().map(|t| SmallVec::from_slice(&[t]))
	}
}
...
impl Envelope {
	/// whether the message is multi-topic. Only relay these to Susy peers.
...
impl srlp::Encodable for Envelope {
	fn srlp_append(&self, s: &mut RlpStream) {
		s.begin_list(5)
			.append(&self.expiry)
			.append(&self.ttl);
...
impl srlp::Decodable for Envelope {
	fn decode(srlp: &Rlp) -> Result<Self, DecoderError> {
		if srlp.item_count()? != 5 { return Err(DecoderError::RlpIncorrectListLen) }

		Ok(Envelope {
			expiry: srlp.val_at(0)?,
			ttl: srlp.val_at(1)?,
			topics: decode_topics(srlp.at(2)?)?,
			data: srlp.val_at(3)?,
			nonce: srlp.val_at(4)?,
		})
	}
...
impl Message {
	...
	pub fn create(params: CreateParams) -> Result<Self, Error> {
		...
		let encoded = ::srlp::encode(&envelope);
	...
	/// Decode message from SRLP and check for validity against system time.
	pub fn decode(srlp: Rlp, now: SystemTime) -> Result<Self, Error> {
		let envelope: Envelope = srlp.as_val()?;
		let encoded_size = srlp.as_raw().len();
		let hash = H256(keccak256(srlp.as_raw()));
...
mod tests {
	...
	fn round_trip() {
		...
		let encoded = ::srlp::encode(&envelope);
		let decoded = ::srlp::decode(&encoded).expect("failure decoding Envelope");
