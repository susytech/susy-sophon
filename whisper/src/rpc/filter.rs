...
use sofkey::Public;
use susy_jsonrpc_pubsub::typed::{Subscriber, Sink};
...
impl Manager {
	...
	pub fn new() -> ::std::io::Result<Self> {
		...
		let join_handle = thread::Builder::new()
			...
			.spawn(move || {
				trace!(target: "susy_whisper", "Start decryption worker");
				loop {
		...
		-> Result<(), &'static str>
	{
		...
		sub.assign_id(::susy_jsonrpc_pubsub::SubscriptionId::String(format!("{:x}", id)))
			.map(move |sink| {
...
impl Drop for Manager {
	fn drop(&mut self) {
		trace!(target: "susy_whisper", "waiting to drop FilterManager");
		self.exit.store(true, atomic::Ordering::Release);
		if let Some(guard) = self.join.take() {
			let _ = guard.join();
		}
		trace!(target: "susy_whisper", "FilterManager dropped");
	}
}
