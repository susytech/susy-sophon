...
/// Susy-whisper protocol ID
/// Current susy-specific extensions:
///   - Multiple topics in packet.
pub const SUSY_PROTOCOL_ID: ::network::ProtocolId = *b"pwh";
...
impl Peer {
	...
	fn will_accept(&self, message: &Message) -> bool {
		...
		// only susy peers will accept multitopic messages.
		if message.envelope().is_multitopic() && !self.is_susy { return false }
...
impl<T: MessageHandler> Network<T> {
		...
		-> Result<(), Error>
	{
		let mut messages_vec = {
			...
			let now = SystemTime::now();
			let mut messages_vec = message_packet.iter().map(|srlp| Message::decode(srlp, now))
	...
	fn on_connect<C: ?Sized + Context>(&self, io: &C, peer: &PeerId) {
		...
		self.peers.write().insert(*peer, Mutex::new(Peer {
			...
			is_susy: io.protocol_version(SUSY_PROTOCOL_ID, *peer).is_some(),
			_protocol_version: version,
		}));

		io.send(*peer, packet::STATUS, ::srlp::EMPTY_LIST_RLP.to_vec());
	}

	fn on_packet<C: ?Sized + Context>(&self, io: &C, peer: &PeerId, packet_id: u8, data: &[u8]) {
		let srlp = Rlp::new(data);
		let res = match packet_id {
			packet::STATUS => self.on_status(peer, srlp),
			packet::MESSAGES => self.on_messages(peer, srlp),
			packet::POW_REQUIREMENT => self.on_pow_requirement(peer, srlp),
			packet::TOPIC_FILTER => self.on_topic_filter(peer, srlp),
...
/// Dummy subprotocol used for susy extensions.
#[derive(Debug, Copy, Clone)]
pub struct SusyExtensions;

impl ::network::NetworkProtocolHandler for SusyExtensions {
