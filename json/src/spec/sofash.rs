...
/// Deserializable doppelganger of block rewards for SofashParams
...
/// Deserializable doppelganger of SofashParams.
#[derive(Clone, Debug, PartialEq, Deserialize)]
#[serde(deny_unknown_fields)]
...
pub struct SofashParams {
	...
	/// See main SofashParams docs.
	pub sip100b_transition: Option<Uint>,
...
/// Sofash engine deserialization.
#[derive(Debug, PartialEq, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Sofash {
	/// Sofash params.
	pub params: SofashParams,
}
...
mod tests {
	...
	use hash::Address;
	use spec::sofash::{Sofash, SofashParams, BlockReward};

	#[test]
	fn sofash_deserialization() {
		let s = r#"{
			"params": {
				...
				"sip100bTransition": "0x42"
			}
		}"#;

		let deserialized: Sofash = serde_json::from_str(s).unwrap();

		assert_eq!(deserialized, Sofash {
			params: SofashParams {
				...
				sip100b_transition: Some(Uint(U256::from(0x42))),
	...
	#[test]
	fn sofash_deserialization_missing_optionals() {
		let s = r#"{
			"params": {
				"difficultyBoundDivisor": "0x0800",
				"minimumDifficulty": "0x020000"
	...
	fn test_zero_value_divisor() {
		...
		let _deserialized: Sofash = serde_json::from_str(s).unwrap();
	}
}
