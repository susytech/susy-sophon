...
//! Note that all the structs and functions here are documented in `susy.h`, to avoid
//! duplicating documentation.

extern crate futures;
extern crate panic_hook;
extern crate susy_sophon;
extern crate tokio;
...
use susy_sophon::{PubSubSession, RunningClient};
...
#[no_mangle]
pub unsafe extern fn susy_config_from_cli(
	...
	panic::catch_unwind(|| {
		*output = ptr::null_mut();
		...
		match susy_sophon::Configuration::parse_cli(&args) {
			Ok(mut cfg) => {
...
#[no_mangle]
pub unsafe extern fn susy_config_destroy(cfg: *mut c_void) {
	let _ = panic::catch_unwind(|| {
		let _cfg = Box::from_raw(cfg as *mut susy_sophon::Configuration);
	});
}

#[no_mangle]
pub unsafe extern fn susy_start(cfg: *const SusyParams, output: *mut *mut c_void) -> c_int {
	panic::catch_unwind(|| {
		*output = ptr::null_mut();
		let cfg: &SusyParams = &*cfg;
		let logger = Arc::from_raw(cfg.logger as *mut susy_sophon::RotatingLogger);
		let config = Box::from_raw(cfg.configuration as *mut susy_sophon::Configuration);
		...
		let action = match susy_sophon::start(*config, logger, on_client_restart_cb, || {}) {
			Ok(action) => action,
			Err(_) => return 1,
		};

		match action {
			susy_sophon::ExecutionAction::Instant(Some(s)) => { println!("{}", s); 0 },
			susy_sophon::ExecutionAction::Instant(None) => 0,
			susy_sophon::ExecutionAction::Running(client) => {
...
#[no_mangle]
pub unsafe extern fn susy_destroy(client: *mut c_void) {
	let _ = panic::catch_unwind(|| {
...
pub unsafe extern fn susy_rpc(
	...
	panic::catch_unwind(|| {
		if let Some((client, query)) = susy_rpc_query_checker(client, query, len) {
...
#[no_mangle]
pub unsafe extern fn susy_subscribe_ws(
	client: *const c_void,
	query: *const c_char,
	len: usize,
	callback: CCallback,
	...
	panic::catch_unwind(|| {
		if let Some((client, query)) = susy_rpc_query_checker(client, query, len) {
			let callback = Arc::new(CallbackStr { user_data, function: callback});
			susy_ws_worker(client, query, callback)
		} else {
			ptr::null()
		}
	})
...
#[no_mangle]
pub unsafe extern fn susy_unsubscribe_ws(session: *const c_void) {
	let _ = panic::catch_unwind(|| {
...
#[no_mangle]
pub extern fn susy_set_panic_hook(callback: CCallback, param: *mut c_void) {
...
#[no_mangle]
pub unsafe extern fn susy_set_logger(
	...
	let mut logger_cfg = susy_sophon::LoggerConfig::default();
	...
	*logger = Arc::into_raw(susy_sophon::setup_log(&logger_cfg).expect("Logger initialized only once; qed")) as *mut _;
}

// WebSocket event loop
fn susy_ws_worker(client: &RunningClient, query: &str, callback: Arc<Callback>) -> *const c_void {
...
fn susy_rpc_worker(client: &RunningClient, query: &str, callback: Arc<Callback>, timeout_ms: u64) {
...
unsafe fn susy_rpc_query_checker<'a>(client: *const c_void, query: *const c_char, len: usize)
