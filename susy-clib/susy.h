...
#ifndef _SUSY_H_INCLUDED_
#define _SUSY_H_INCLUDED_

#include <stddef.h>

/// Parameters to pass to `susy_start`.
struct SusyParams {
	/// Configuration object, as handled by the `susy_config_*` functions.
	/// Note that calling `susy_start` will destroy the configuration object (even on failure).
	void *configuration;
	...
	/// Logger object which must be created by the `susy_config_logger` function
	void *logger;
};
...
/// if (susy_config_from_cli(args, str_lens, 2, &cfg) != 0) {
///		return 1;
/// }
/// ```
///
int susy_config_from_cli(char const* const* args, size_t const* arg_lens, size_t len, void** out);

/// Builds a new logger object which should be a member of the `SusyParams struct`
///
///	- log_mode		: String representing the log mode according to `Rust LOG` or nullptr to disable logging.
///					  See module documentation for `sofcore-logger` for more info.
...
/// susy_set_logger(logger_mode, strlen(logger_mode), nullptr, 0, &cfg.logger);
/// ```
///
int susy_set_logger(const char* log_mode, size_t log_mode_len, const char* log_file, size_t log_file_len, void** logger);

/// Destroys a configuration object created earlier.
///
/// **Important**: You probably don't need to call this function. Calling `susy_start` destroys
/// 				the configuration object as well (even on failure).
void susy_config_destroy(void* cfg);

/// Starts the susy client in background threads. Returns a pointer to a struct that represents
...
/// 				call `susy_start` (even on failure).
///
/// On success, the produced object will be written to the `void*` pointed by `out`.
///
/// Returns 0 on success, and non-zero on error.
int susy_start(const SusyParams* params, void** out);

/// Destroys the susy client created with `susy_start`.
///
/// **Warning**: `susy_start` can return NULL if execution finished instantly, in which case you
///					must not call this function.
void susy_destroy(void* susy);

/// Performs an asynchronous RPC request running in a background thread for at most X milliseconds
///
///	- susy		: Reference to the running susy client
...
///
int susy_rpc(const void *const susy, const char* rpc_query, size_t rpc_len, size_t timeout_ms,
...
///
void* susy_subscribe_ws(const void *const susy, const char* ws_query, size_t len,
		void (*subscribe)(void* ud, const char* response, size_t len), void* ud);
...
///
int susy_unsubscribe_ws(const void *const session);
...
/// the user. A panic always indicates a bug in Susy.
///
/// Note that this method sets the panic hook for the whole program, and not just for Susy. In
/// other words, if you use multiple Rust libraries at once (and not just Susy), then a panic
...
///
int susy_set_panic_hook(void (*cb)(void* param, const char* msg, size_t msg_len), void* param);
